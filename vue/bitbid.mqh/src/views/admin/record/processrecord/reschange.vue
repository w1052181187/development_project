<template>
  <!-- 结果公示 -->
  <div class="admin-layout" id="reschange-page">
    <div class="resnotice-cont">
      <div class="resnotice-tit">
        <span>结果变更</span>
      </div>
      <div class="resnitice-box">
        <div class="btn-box" style="padding-bottom: 55px;" v-if="dialogEdit">
          <el-button @click="addBtnClick" type="warning" size="medium" icon="el-icon-plus" style="float: left;">添 加</el-button>
          <el-button @click="$router.go(-1)" type="info" size="medium" style="float: right;">取 消</el-button>
        </div>
        <!-- 折叠面板 -->
        <el-collapse v-model="collapseActiveName" accordion>
          <el-collapse-item  v-for="(resultNotice, index) in resultNoticeList" :key="index" :name='resultNotice.code'>
            <template slot="title">
            <span class="title-span collapse-span" @click="getData(resultNotice, index)" style="width: 100%; display: inherit">
              <span class="middle-span">{{resultNotice.section.name}}</span>
              <span>{{resultNotice.resultNoticeChangeCount}}次结果变更</span>
              <el-button type="text" @click.stop="deleteCilck(resultNotice.objectId)"
                 style="float: right;margin-right: 40px;line-height: 25px;"
                 v-if="resultNotice.isAllowOperate === 1">删除</el-button>
            </span>
            </template>
            ...
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="140px" style="margin-top: 20px;">
              <el-form-item label="中标人:" prop="bidWinner">
                <el-button type="warning" @click="confirmBidWinner" class="confirm-btn" v-if="ruleForm.isAllowOperate === 1">确定中标人</el-button>
                <el-table
                  class="form-table"
                  border
                  :data="bidTableData"
              ...
              <el-form-item label="中标通知书:" prop="noticeFileList" ref="noticeFileList">
                <el-upload
                  v-if="ruleForm.isAllowOperate === 1"
                  class="upload-demo"
                  :action="upLoad"
                  ...
                  <el-row>
                    <el-col :span="12">
                      <span class="file-list" v-for="(item, index) in noticeFileList" :key="index">
                        ...
                        <i class="el-icon-error upload-file" @click="noticeClickDel(index)" v-if="ruleForm.isAllowOperate === 1"></i>
                      </span>
                    </el-col>
                  </el-row>
                </div>
                  ...
                  <el-row>
                    <el-col :span="12">
                      <span class="file-list" v-for="(item, index) in fileList" :key="index">
                        <el-button type="text" style="padding-right: 20px;" @click="downloadFile(item.fileName,item.relativePath)">{{item.fileName}}</el-button>
                        <i class="el-icon-error upload-file" @click="clickDel(index)" v-if="ruleForm.isAllowOperate === 1"></i>
                      </span>
                    </el-col>
                  </el-row>
                </div>
              ...
              <el-form-item label="发布媒介:" prop="resultNoticeMedia">
                <el-input v-model="ruleForm.resultNoticeMedia" style="width: 340px;" v-if="ruleForm.isAllowOperate === 1"></el-input>
                <span v-else>{{ruleForm.resultNoticeMedia}}</span>
              </el-form-item>
              <el-row :gutter="100">
                <!--<el-col :span="8">
                  <el-form-item label="质保金(万元):" prop="warrantyGold">
                    <el-input v-model="ruleForm.warrantyGold" style="width: 340px;" v-if="ruleForm.isAllowOperate === 1"></el-input>
              ...
              <!--<el-row :gutter="100">
                <el-col :span="8">
                  <el-form-item label="付款方式:" prop="payMethod">
                    <el-input v-model="ruleForm.payMethod" style="width: 340px;" v-if="ruleForm.isAllowOperate === 1"></el-input>
                    <span v-else>{{ruleForm.payMethod}}</span>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item label="质保期:" prop="warrantyPeriod">
                    <el-input v-model="ruleForm.warrantyPeriod" style="width: 340px;" v-if="ruleForm.isAllowOperate === 1"></el-input>
              ...
              <!--<el-row :gutter="100">
                <el-col :span="8">
                  <el-form-item label="项目经理:" prop="projectManager" v-if="tenderProjectType === 1">
                    <el-input v-model="ruleForm.projectManager" style="width: 340px;" v-if="ruleForm.isAllowOperate === 1"></el-input>
                    <span v-else>{{ruleForm.projectManager}}</span>
                  </el-form-item>
                  <el-form-item label="结算依据:" prop="basisSettlement" v-if="tenderProjectType === 1">
                    <el-input v-model="ruleForm.basisSettlement" style="width: 340px;" v-if="ruleForm.isAllowOperate === 1"></el-input>
                    <span v-else>{{ruleForm.basisSettlement}}</span>
                  </el-form-item>
                </el-col>
                <el-col :span="8">
                  <el-form-item label="规格、型号、数量:" prop="goodsExplain" v-if="tenderProjectType === 2">
                    <el-input v-model="ruleForm.goodsExplain" style="width: 340px;" v-if="ruleForm.isAllowOperate === 1"></el-input>
                    <span v-else>{{ruleForm.goodsExplain}}</span>
                  </el-form-item>
                  <el-form-item label="供货期:" prop="supplyPeriod" v-if="tenderProjectType === 2">
                    <el-input v-model="ruleForm.supplyPeriod" style="width: 340px;" v-if="ruleForm.isAllowOperate === 1"></el-input>
                    <span v-else>{{ruleForm.supplyPeriod}}</span>
                  </el-form-item>
                  <el-form-item label="服务期:" prop="serverPeriod" v-if="tenderProjectType === 3">
                    <el-input v-model="ruleForm.serverPeriod" style="width: 340px;" v-if="ruleForm.isAllowOperate === 1"></el-input>
                    <span v-else>{{ruleForm.serverPeriod}}</span>
                  </el-form-item>
                </el-col>
              </el-row>-->
              <el-form-item class="form-last">
                <el-button type="primary" @click="saveForm('ruleForm', index)" v-if="ruleForm.isAllowOperate === 1">保 存</el-button>
                <el-button @click="$router.go(-1)" v-if="ruleForm.isAllowOperate === 1">取 消</el-button>
              </el-form-item>
            </el-form>
          </el-collapse-item>
        </el-collapse>
    ...
    <div class="dialog-box">
      ...
      <el-dialog
        ...
        width="30%">
        <div class="dialog-box-form">
          <el-form :model="addRuleForm" ref="addRuleForm" :rules="addRules" label-width="100px">
            <el-form-item label="标段名称:" prop="sectionCode">
              <el-select v-model="addRuleForm.sectionCode" placeholder="请选择标段">
                <el-option
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="saveAddClick('addRuleForm')">保 存</el-button>
...
import {resnotice, tenderproject, candNotice, investmanage} from 'api/index'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      dialogVisible: false, // 中标人设置-弹窗
      dialogRuleForm: {}, // 弹窗-表单
      dialogTableData: [], // 弹窗-表格
      dialogAddVisible: false, // 弹窗-新增
      addRuleForm: {}, // 弹窗-新增-表单
      winBidYn: '',
      selectIndex: null,
      ruleForm: {},
      rules: {
        ...
        resultNoticeContent: [
          ...
          { min: 1, max: 600, message: '请输入1~600个字符', trigger: 'blur' }
        ],
        resultNoticeMedia: [
      ...
      currentPage: 1,
      sectionList: [], // 标段
      resultNoticeList: [], // 标段对应的结果公示信息
      tenderProjectType: null, // 招标项目类型（1.工程 2.货物 3.服务）
      index: 0,
      sectionCode: null, // 标段编号
      dialogEdit: false
    }
  },
  computed: {
  ...
  methods: {
    ...
    /** 删除-折叠面板 */
    deleteCilck (objectId) {
      if (objectId) {
        resnotice.deleteById(objectId).then((res) => {
          this.getResultNoticeList()
        })
      } else {
        let newRes = this.resultNoticeList.filter(resultNotice => {
          if (resultNotice.objectId) {
            return resultNotice
          }
        })
        this.resultNoticeList = newRes
      }
    },
    /** 确定中标人-按钮 */
    confirmBidWinner () {
    ...
    updateSectionInfos () {
      ...
      this.ruleFormProInfo.suggestedPlanTableData.map(item => {
        ...
        sectionInfo.sectionCode = this.sectionCode
    ...
    saveAddClick (val) {
      this.$refs[val].validate((vaild) => {
        if (vaild) {
          this.sectionCode = this.addRuleForm.sectionCode
    ...
    getSections () {
      ...
      resnotice.getLastResult(query).then((res) => {
        ...
        if (resultNoticeList) {
          let newRes = resultNoticeList.filter(resultNotice => {
            if (resultNotice.isConfirm === 0) {
              return resultNotice
            }
          })
          if (newRes.length) {
            this.dialogEdit = true
            this.sectionList = newRes.map(resultNotice => {
    ...
    /** 通过标段查询对应中标结果公示信息(新增时调用) */
    getResultNotices () {
      this.resultNoticeList = []
      let obj = {
        tenderProjectCode: this.$route.query.tenderProjectCode,
        noticeCode: this.noticeCode,
        sectionCode: this.sectionCode
      }
      resnotice.getList(obj).then((res) => {
        if (res.data.resultNoticeList) {
          let result = res.data.resultNoticeList
          if (result.length === 1) {
            this.resultNoticeList[0] = {}
            this.resultNoticeList[0].tenderProjectCode = result[0].tenderProjectCode
            this.resultNoticeList[0].noticeCode = result[0].noticeCode
            this.resultNoticeList[0].sectionCode = result[0].sectionCode
            this.resultNoticeList[0].resultNoticeMedia = result[0].resultNoticeMedia
            this.resultNoticeList[0].section = result[0].section
            this.resultNoticeList[0].bidWinner = result[0].bidWinner
            this.resultNoticeList[0].code = '111'
            this.resultNoticeList[0].resultNoticeType = 2
            this.resultNoticeList[0].resultNoticeChangeCount = result[0].resultNoticeChangeCount + 1
            this.resultNoticeList[0].isAllowOperate = 1
            result[0].suggestedPlanTree.map(item => {
              item.suggestedPlan.sectionInfoId = null
              return item
            })
            this.resultNoticeList[0].suggestedPlanTree = result[0].suggestedPlanTree
            this.getSuggestedPlans(this.resultNoticeList[0])
          } else {
            let newRes = result.filter(resultNotice => {
              if (resultNotice.resultNoticeType === 2) {
                return resultNotice
              }
            })
            // 新增一条结果变更数据
            let resultNotice = {}
            resultNotice.tenderProjectCode = result[0].tenderProjectCode
            resultNotice.noticeCode = result[0].noticeCode
            resultNotice.sectionCode = result[0].sectionCode
            resultNotice.resultNoticeMedia = newRes[0].resultNoticeMedia
            resultNotice.section = newRes[0].section
            resultNotice.bidWinner = newRes[0].bidWinner
            resultNotice.code = '111'
            resultNotice.resultNoticeType = 2
            resultNotice.resultNoticeChangeCount = newRes[0].resultNoticeChangeCount + 1
            resultNotice.isAllowOperate = 1
            newRes[0].suggestedPlanTree.map(item => {
              item.suggestedPlan.sectionInfoId = null
              return item
            })
            resultNotice.suggestedPlanTree = newRes[0].suggestedPlanTree
            this.getSuggestedPlans(resultNotice)
            this.resultNoticeList.push(resultNotice)
            newRes.map(resultNotice => {
              this.resultNoticeList.push(resultNotice)
            })
          }
        }
      }).then(res => {
        if (this.resultNoticeList.length > 0) {
          this.resultNoticeList.map(resultNotice => {
            if (resultNotice.objectId && resultNotice.isAllowOperate === 1) {
              resultNotice.isAllowOperate = 0
              resnotice.updateResultNotice(resultNotice)
            }
          })
          this.getData(this.resultNoticeList[0], 0)
        }
      })
    },
    getSuggestedPlans (resultNotice) {
      if (resultNotice.suggestedPlanTree) {
        if (resultNotice.suggestedPlanTree) {
          resultNotice.suggestedPlanTree.map(item => {
            item.dialogEdit = resultNotice.isAllowOperate
          })
          this.ruleFormProInfo.suggestedPlanTableData = this.transTreeData(resultNotice.suggestedPlanTree)
        }
      }
    },
    /**
    ...
    getResultNoticeList () {
      ...
      resnotice.getList(obj).then((res) => {
        if (res.data.resultNoticeList) {
          this.resultNoticeList = res.data.resultNoticeList
          this.collapseActiveName = this.resultNoticeList[0].code
        }
      }).then((res) => {
        if (this.resultNoticeList.length) {
          this.getData(this.resultNoticeList[0], 0)
        }
      })
    },
    /** 查询折叠面板信息 */
    getData (resultNotice, index) {
      this.ruleForm = {}
      this.bidTableData = []
      this.dialogTableData = []
      this.fileList = []
      this.noticeFileList = []
      this.collapseActiveName = resultNotice.code
      this.index = index
      this.getTenderProject()
      this.getResultNotice(resultNotice, index)
      this.getPublicity()
    },
    /** 查询折叠面板中结果公示信息 */
    getResultNotice (resultNotice, index) {
      this.ruleForm = resultNotice
      this.dialogRuleForm.sectionName = resultNotice.section.name
      if (this.ruleForm.fileInformationList) {
      ...
      if (resultNotice.isAllowOperate === 1) {
        this.readonly = false
      } else {
        window.UE.getEditor(`${this.$refs.ueditor[index].id}`).setDisabled()
      }
      this.bidTableData.push(this.ruleForm.bidWinner.winCandidate)
      // 项目信息
      this.getSuggestedPlans(this.ruleForm)
    },
    /** 中标候选人公示对应信息 */
    getPublicity () {
      let query = {
