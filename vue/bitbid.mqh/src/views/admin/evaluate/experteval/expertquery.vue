<template>
  <!-- 后台主容器 -->
  <div class="admin-layout" id="expertquery-page">
    <div v-if="this.$route.path == '/admin/evaluate/expertquery'">
      <div class="breadcrumb-mqh-tit">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
          <el-breadcrumb-item>考评系统</el-breadcrumb-item>
          <el-breadcrumb-item :to="{ path: '/admin/evaluate/expertquery' }">评标专家考评查询</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!--搜索区域start-->
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button @click="handleTableClick(scope.row,'look')" type="text" size="medium" v-if="$store.getters.permissions.includes('/evaluate/expertquery/detail')">查看</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!-- 表格区域end -->
...
export default {
  ...
  methods: {
    ...
    handleTableClick (row, type) {
      this.$router.push({path: '/admin/evaluate/expertquery/details', query: {code: row.code}})
    }
  },
  computed: {},
  // 实例创建完成
