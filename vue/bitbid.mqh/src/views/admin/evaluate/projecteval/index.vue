<template>
  <!-- 后台主容器 -->
  <div class="admin-layout" id="projecteval-page">
    <div v-if="this.$route.path == '/admin/evaluate/projecteval'">
      <div class="breadcrumb-mqh-tit">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
          <el-breadcrumb-item>考评系统</el-breadcrumb-item>
          <el-breadcrumb-item :to="{ path: '/admin/evaluate/projecteval' }">项目单位考评</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!--搜索区域start-->
      <div class="search-box projecteval-search">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button @click="handleTableClick(scope.row,'eval')" type="text" size="medium" v-if="$store.getters.permissions.includes('/evaluate/projecteval/operation')">考评</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!-- 表格区域end -->
...
export default {
  ...
  methods: {
    ...
    /** 获取表单数据 */
    getTableData () {
      tenderproject.getListInEvaluate({
        pageNo: this.pageNo,
    ...
    handleTableClick (row, type) {
      if (Object.is(type, 'eval')) {
        this.$router.push({path: '/admin/evaluate/agenteval/evalpage', query: {code: row.code}})
      }
    }
  },
  computed: {},
