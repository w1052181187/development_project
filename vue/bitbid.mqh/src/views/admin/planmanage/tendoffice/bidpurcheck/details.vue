<template>
  <!-- 后台主容器 -->
  <div class="admin-layout" id="planapp-details-page">
    <div class="breadcrumb-mqh-tit">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/admin/planmanage/bidpurcheck/details' }">详情页</el-breadcrumb-item>
      </el-breadcrumb>
      <el-button type="default" icon="el-icon-back" @click="$router.go(-1)" class="go-back-btn">
        返回
      </el-button>
    </div>
    <div class="planapp-cont">
      <div class="planapp-cont-tit">
        <h3>{{this.$route.query.year}}年招标项目计划表</h3>
        <el-row :gutter="20">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button type="text" @click="clickProPlan(scope.row)">{{scope.row.projectPlanName}}</el-button>
          </template>
        </el-table-column>
        <el-table-column
          prop="projectName"
          ...
          width="240">
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark" :content="item.projectName" v-for="(item,index) in scope.row.projectList" :key="index" v-if="item.projectName">
              <p class="dresult-span">
                <el-button
                  type="text"
                  style="width:100%;line-height: 0;overflow: hidden;text-align: left;"
                  size="mini"
        ...
        <el-table-column
          ...
          width="200">
          <template slot-scope="scope">
            <el-tooltip class="item" effect="dark" :content="item.content" placement="top" v-for="(item,index) in scope.row.projectList" :key="index" v-if="item.content">
              <p class="dresult-span" >{{item.content}}
              </p>
            </el-tooltip>
            <p class="dresult-blank-span" v-else></p>
          </template>
...
import {planproject, investmanage, plansubmit} from 'api/index'
...
export default {
  ...
  methods: {
    ...
    annualPlanProject () {
      ...
      planproject.getList(query).then((res) => {
    ...
    getApproveRecordList () {
      ...
      plansubmit.getApproveRecord(query).then((res) => {
  ...
  // 实例创建完成
  mounted () {
    this.annualPlanProject()
  }
}
</script>
<style lang="less">
#planapp-details-page {
  /*表格区域css*/
  .planapp-cont {
    margin: 20px;
    padding: 20px;
