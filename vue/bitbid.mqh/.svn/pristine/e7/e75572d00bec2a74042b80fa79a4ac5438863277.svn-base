<template>
  <!-- 后台主容器 -->
  <div class="admin-layout" id="agentquery-page">
    <div v-if="this.$route.path == '/admin/evaluate/agentquery'">
      <div class="breadcrumb-mqh-tit">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
          <el-breadcrumb-item>考评系统</el-breadcrumb-item>
          <el-breadcrumb-item :to="{ path: '/admin/evaluate/agentquery' }">代理机构考评查询</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!--搜索区域start-->
      <div class="search-box agenteval-search">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button @click="handleTableClick(scope.row,'look')" type="text" size="medium" v-if="$store.getters.permissions.includes('/evaluate/agentquery/detail')">查看</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!-- 表格区域end -->
  ...
  methods: {
    ...
    handleTableClick (row, type) {
      if (Object.is(type, 'look')) {
        this.$router.push({path: '/admin/evaluate/agentquery/details', query: {code: row.code}})
      }
    }
  },
  computed: {},
