<template>
  ...
  <div class="admin-layout" id="coalsafety-page">
    <div v-if="this.$route.path == '/admin/statistics/coalsafety-pass'">
      <div class="breadcrumb-mqh-tit">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
          <el-breadcrumb-item>统计分析</el-breadcrumb-item>
          <el-breadcrumb-item>年度汇总表</el-breadcrumb-item>
          <el-breadcrumb-item :to="{ path: '/admin/statistics/coalsafety' }">煤炭安全资金汇总表</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      <!-- 搜索区域start -->
      ...
      <div class="summary-box">
        ...
        <div class="summary-table">
          <div class="summary-table-tit">
            <h2>{{year}}年煤炭安全资金汇总表计划完成汇总表</h2>
          </div>
          <div class="summary-unit">
            <span>单位：万元</span>
          </div>
          <div v-if="typeFlag==4&&randomNum">
            <el-table
              ref="tableSum1"
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <el-button type="text" v-if="scope.$index!=0" :title="scope.row.enterpriseName">{{scope.row.enterpriseName}}</el-button>
                  <span v-if="scope.$index==0" :title="scope.row.enterpriseName">{{scope.row.enterpriseName}}</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="planPrice"
        ...
        <el-row :gutter="20">
          <el-col :span="7">
            <div class="charts-pic charts-pie">
              ...
              <div id="myChartLeft" class="pie-box"></div>
            </div>
          </el-col>
          <el-col :span="7">
            <div class="charts-pic charts-pie">
              <span class="charts-tit">各项目单位累计完成金额所占比例</span>
              <div id="myChartMiddle" class="pie-box"></div>
            </div>
          </el-col>
          <el-col :span="10">
  ...
  watch: {
    ...
    typeFlag: function (newV, oldV) {
      if (newV) {
        ...
        this.histogramChart = {} // 柱状图
        this.rightChart = {} // 饼状图右
        this.leftChart = {} // 饼状图左
        this.drawPieLeft()
      }
    }
  ...
  methods: {
    ...
    cellClick (row, column, cell, event) {
      if (column.label === '单位名称' && row.enterpriseName !== '总计') {
        this.$router.push({path: '/admin/statistics/coalsafety/coalsafety-details', query: {enterpriseId: row.enterpriseId, enterpriseName: row.enterpriseName, capitalSourceId: this.capitalSourceId, type: this.type, year: this.year, menuId: this.typeFlag}})
      }
    },
    handleChart (data) {
      if (data.length === 0) {
    ...
    /** 饼状图-中 */
    drawPieMiddle () {
      let myChart = Echarts.init(document.getElementById('myChartMiddle'))
      // 获得数据
    ...
    getCapitalSourceList () {
      if (this.typeFlag === 4) {
        this.capitalSourceName = '煤炭安全资金汇总表'
        this.capitalSourceId = 10004
        this.type = 1
      }
      // 表格表头数据
