<template>
  <!-- 后台主容器 -->
  <div class="admin-layout" id="fundscheck-page">
    <div class="breadcrumb-mqh-tit">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/admin/planmanage/balancecheck/check' }">审核
        </el-breadcrumb-item>
      </el-breadcrumb>
    ...
    <div class="maincont-box">
      ...
      <div class="handle-btn">
        <el-button type="primary" @click="handleBtnEvents('edit')">修 改</el-button>
    ...
    <div class="dialog-box">
      <el-dialog
        ...
        width="60%"
        center>
        <p><strong>{{this.chooseYear - 1}}年项目预计完成情况</strong></p>
          ...
          <el-row :gutter="20" v-if="isShowFlag">
            <el-col :span="11">
              <el-form-item
                ...
                <el-select v-model="projectExpectedForm.miningRightsType" placeholder="请选择" @change="changeMinRight" v-if="!lookFlag" style="width:190px">
                  <el-option label="探矿权" value="0"></el-option>
                  <el-option label="采矿权" value="1"></el-option>
                </el-select>
                <span v-if="lookFlag">{{projectExpectedForm.miningRightsType === '0' ? '探矿权': '采矿权'}}</span>
              </el-form-item>
            </el-col>
            <el-col :span="11">
              <el-form-item
                ...
                <el-input v-model="projectExpectedForm.prospectingRightsBenefits" style="width:190px" v-if="!lookFlag"></el-input><span v-if="!lookFlag">万元</span>
                <span v-if="lookFlag">{{projectExpectedForm.prospectingRightsBenefits}}&nbsp;&nbsp;万元</span>
              </el-form-item>
              <el-form-item
                v-if="isShowMineral"
                label="深加工产品销售收入"
                ...
                <el-input v-model="projectExpectedForm.deepProcessingProductsIncome" style="width:190px" v-if="!lookFlag"></el-input><span v-if="!lookFlag">万元</span>
                <span v-if="lookFlag">{{projectExpectedForm.deepProcessingProductsIncome}}&nbsp;&nbsp;万元</span>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20"  v-if="isShowMineral && isShowFlag">
            <el-col :span="11">
              <el-form-item
                label="矿种系数"
                prop="mineralTypeCoefficient"
                :rules="rules.mineralTypeCoefficient">
                <el-input v-model="projectExpectedForm.mineralTypeCoefficient" style="width:190px"  v-if="!lookFlag"></el-input>
                <span v-if="lookFlag">{{projectExpectedForm.mineralTypeCoefficient}}</span>
              </el-form-item>
            </el-col>
            <el-col :span="11">
              <el-form-item
                ...
                <el-input v-model="projectExpectedForm.influenceCoefficient" style="width:190px" v-if="!lookFlag"></el-input>
                <span v-if="lookFlag">{{projectExpectedForm.influenceCoefficient}}</span>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
        <p><strong>{{this.chooseYear}}年项目计划情况（本年度专项资金风险比例为{{this.riskRatio}}%）</strong></p>
          ...
          <el-row :gutter="20" v-if="isShowSecondFlag">
              <el-col :span="11">
                <el-form-item
                  ...
                  prop="miningRightsType"
                  :rules="rules.miningRightsType">
                  <el-select v-model="projectPlanForm.miningRightsType" placeholder="请选择" @change="changeSecondMinRight" style="width:190px" v-if="!lookFlag">
                    <el-option label="探矿权" value="0"></el-option>
                    <el-option label="采矿权" value="1"></el-option>
                  </el-select>
                  <span v-if="lookFlag">{{projectPlanForm.miningRightsType === '0' ? '探矿权': '采矿权'}}</span>
                </el-form-item>
              </el-col>
              <el-col :span="11">
                <el-form-item
                  ...
                  prop="prospectingRightsBenefits"
                  :rules="rules.prospectingRightsBenefits">
                  <el-input v-model="projectPlanForm.prospectingRightsBenefits" style="width:190px" v-if="!lookFlag"></el-input><span v-if="!lookFlag">万元</span>
                  <span v-if="lookFlag">{{projectPlanForm.prospectingRightsBenefits}}&nbsp;&nbsp;万元</span>
                </el-form-item>
                <el-form-item
                  v-if="isShowSecondMineral"
                  label="深加工产品销售收入"
                  prop="deepProcessingProductsIncome"
                  :rules="rules.deepProcessingProductsIncome">
                  <el-input v-model="projectPlanForm.deepProcessingProductsIncome" style="width:190px" v-if="!lookFlag"></el-input><span v-if="!lookFlag">万元</span>
                  <span v-if="lookFlag">{{projectPlanForm.deepProcessingProductsIncome}}&nbsp;&nbsp;万元</span>
                </el-form-item>
              </el-col>
            </el-row>
            <el-row :gutter="20"  v-if="isShowSecondMineral && isShowSecondFlag">
              <el-col :span="11">
                <el-form-item
                  label="矿种系数"
                  prop="mineralTypeCoefficient"
                  :rules="rules.mineralTypeCoefficient">
                  <el-input v-model="projectPlanForm.mineralTypeCoefficient" style="width:190px" v-if="!lookFlag"></el-input>
                  <span v-if="lookFlag">{{projectPlanForm.mineralTypeCoefficient}}</span>
                </el-form-item>
              </el-col>
              <el-col :span="11">
                <el-form-item
                  ...
                  prop="influenceCoefficient"
                  :rules="rules.influenceCoefficient">
                  <el-input v-model="projectPlanForm.influenceCoefficient" style="width:190px" v-if="!lookFlag"></el-input>
                  <span v-if="lookFlag">{{projectPlanForm.influenceCoefficient}}</span>
                </el-form-item>
              </el-col>
            </el-row>
        </el-form>
        <span slot="footer" class="dialog-footer">
          ...
          <el-button @click="dialogVisible=false" v-if="!lookFlag">取消</el-button>
        </span>
      </el-dialog>
    </div>
    <!--弹窗区域end-->
...
import {balanceSheet, investmanage, agendaMessage} from 'api/index'
...
export default {
  data () {
    ...
    return {
      ...
      enterpriseClassify: this.$route.query.enterpriseClassify, // 当前用户所在企业的企业类型
  ...
  methods: {
    ...
    /** 获取当前列表上的数据 */
    getCurrentData () {
      balanceSheet.getOneByCode(this.$route.query.code).then((res) => {
    ...
    getFormInfo (fundsInfo) {
      ...
      if (this.flag === 1) {
        agendaMessage.updateConsumedStatusByrealatedCode(this.$route.query.relatedCode).then((res) => {
        })
      }
    },
    ...
    createTableFundsType () {
      if (this.enterpriseClassify === null || this.enterpriseClassify === '') {
        ...
        investmanage.selectCapitalSourceByEnterpriseType(this.enterpriseClassify).then((res) => {
    ...
    getAllCapitalSourceByEnterpriseType () {
      if (this.enterpriseClassify !== null && this.enterpriseClassify !== '') {
        investmanage.selectCapitalSourceByEnterpriseType(this.enterpriseClassify).then((res) => {
          this.getDialogInfo(res.data.enterpriseTypeCapitalSourceList, this.enterpriseClassify)
        })
      } else {
        investmanage.getCapitalSourceList({}).then((res) => {
          ...
          this.getDialogInfo(capitalSourceList, this.enterpriseClassify)
        })
      }
    },
    /** 构建弹框中专项资金部分 */
    ...
    createBalanceSheet (specialFundsArr, constructionFundsArr) {
      ...
      balanceSheetInfo.objectId = null
      balanceSheetInfo.relatedBalanceCode = this.currentBalanceSheetInfo.balanceSheet.code
      balanceSheetInfo.fixedQuota = Number(fixedQuota).toFixed(2) // 专项资金额度
      balanceSheetInfo.constructionQuota = Number(constructionQuota).toFixed(2) // 工程建设资金额度
      balanceSheetInfo.previousYearBalanceInfoList = previousYearBalanceInfoList
      ...
      balanceSheet.save(balanceSheetInfo).then((res) => {
        if (res.data.resCode === '0000') {
          this.$router.push({path: '/admin/planmanage/balancecheck'})
        }
      })
    },
    /** 针对上一年度资金信息的总结 */
    createPreviousYearBalanceInfo (form, sourceType) {
      let previousYearBalanceInfo = {
        relatedCode: this.currentBalanceSheetInfo.balanceSheet.code,
