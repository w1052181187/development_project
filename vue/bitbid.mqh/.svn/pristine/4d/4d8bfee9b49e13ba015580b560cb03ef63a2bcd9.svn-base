  ...
  <div class="admin-layout" id="investmanage-page">
    <div class="breadcrumb-mqh-tit">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/admin/planmanage/investmanage/investedit' }">建议计划</el-breadcrumb-item>
      </el-breadcrumb>
    ...
    <div class="maincont-box planaccept-cont">
      <el-tabs v-model="activeName" type="card" @tab-click="handleTabsClick">
        <el-tab-pane label="创建建议计划" name="first">
          <component ref="mychild" :is="currentView" @getChildValue="getChildValue" :classifyCode="classifyCode" :recycleFlag="recycleFlag" @childToParent="childToParent" @newCode="newCode">
        ...
        <el-tab-pane label="项目储备库" name="third">
          ...
          <div class="table">
            <el-table
              ...
              <el-table-column
                v-if="this.$route.query.handleType === 'edit'"
                align="center"
                label="操作"
                width="100">
                <template slot-scope="scope">
                  <el-button @click="handleRecycleClick(scope.row,'recall')" type="text" size="medium">恢复</el-button>
                  <el-button @click="handleRecycleClick(scope.row,'remove')" type="text" size="medium">移除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
          ...
          <div class="table">
            <el-table
              ...
              <el-table-column
                ...
                width="100">
                <template slot-scope="scope">
                  <el-button @click="handleRecycleClick(scope.row,'back')" type="text" size="medium">恢复</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
    ...
    <div class="dialog-box">
      ...
      <el-dialog
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="recallTarget('pro')">项目储备库</el-button>
          <el-button type="primary" @click="recallTarget('plan')">投资计划</el-button>
        </span>
      </el-dialog>
      <!--恢复弹窗end-->
      <!-- 工程建设建议计划申报表start -->
...
export default {
  data () {
    return {
      ...
      fourthEquipForm: {},
      fifthOtherForm: {},
      recallDialogVisible: false, // 恢复弹窗
      searchRecycleForm: {}, // 项目储备库-查询
      searchReserveForm: {}, // 回收站-查询
      recycleTableData: [],
      reserveTableData: [],
      currentView: 'investcard',
  ...
  methods: {
    ...
    /** 恢复 */
    recallTarget (target) {
      if (Object.is(target, 'pro')) {
        // 项目储备库
        this.reserveRow.isInRecycleBin = 1
      } else if (Object.is(target, 'plan')) {
        // 投资计划
    ...
    /** 返回 */
    goBack () {
      if (Object.is(this.currentView, 'investedit')) {
        this.currentView = 'investcard'
      } else {
        this.$router.go(-1)
      }
    },
    /** 接受子组件的值 */
    getChildValue (val) {
      if (Object.is(val.investedit, 'investedit')) {
        this.currentView = 'investedit'
      }
  ...
  components: {
    ...
    treeGridQuery,
    // 卡片
    investcard: resolve => require(['./investcard'], resolve),
    // 编辑
    investedit: resolve => require(['./investedit'], resolve)
  }
}
</script>
<style lang="less">
