<template>
  <!-- 后台主容器 -->
  <div class="admin-layout" id="investedit-page">
    <!--创建建议计划-搜索区域start-->
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-form :model="scope.row" :rules="projectNameRules" :ref="'projectNameForm'+scope.$index" class="demo-ruleForm" @submit.native.prevent>
        ...
        <el-table-column
          ...
          align="center">
          <template slot-scope="scope">
            <el-form :model="scope.row" :rules="contentRules" :ref="'contentForm'+scope.$index" class="demo-ruleForm" @submit.native.prevent>
        ...
        <el-table-column
          ...
          align="center">
          <template slot-scope="scope">
            <el-form :model="scope.row" :rules="quantitiesUnitRules" :ref="'quantitiesUnitForm'+scope.$index" class="demo-ruleForm" @submit.native.prevent>
        ...
        <el-table-column
          ...
          <el-table-column
            ...
            align="center">
            <template slot-scope="scope">
              <el-form :model="scope.row" :rules="budgetQuantitiesRules" :ref="'budgetQuantitiesForm'+scope.$index" class="demo-ruleForm" @submit.native.prevent>
          ...
          <el-table-column
            ...
            align="center">
            <template slot-scope="scope">
              <el-form :model="scope.row" :rules="budgetAmountRules" :ref="'budgetAmountForm'+scope.$index" class="demo-ruleForm" @submit.native.prevent>
        ...
        <el-table-column
          ...
          <el-table-column
            ...
            align="center">
            <template slot-scope="scope">
              <el-form :model="scope.row" :rules="quantitiesRules" :ref="'quantitiesForm'+scope.$index" class="demo-ruleForm" @submit.native.prevent>
          ...
          <el-table-column
            ...
            align="center">
            <template slot-scope="scope">
              <el-form :model="scope.row" :rules="amountRules" :ref="'amountForm'+scope.$index" class="demo-ruleForm" @submit.native.prevent>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-dropdown @command="handleCommand" v-show="scope.row.objectId  && capitalSourceClassify.status === 0">
              <span class="el-dropdown-link" style="color: #409eff;">
                转出<i class="el-icon-arrow-down el-icon--right"></i>
              </span>
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item :command="{'row': scope.row, 'classify':item}" v-for="(item, index) in  optionalclassifyList" :key="index">
...
export default {
  ...
  watch: {
    ...
    autoSearch: function (newV, oldV) {
      ...
      investmanage.getDeclarationForm(query).then(res => {
        ...
        if (declarationForm) {
          this.dialogBaseInfoForm.fundPurposeFirst = declarationForm.fundPurposeFirst
  ...
  methods: {
    /** 转出 */
    handleCommand (val) {
      let arr = ['该项目将转到“' + val.classify.capitalSourceName + '”中, 确认转出吗?', '项目提交后，将不能跨资金来源转换']
      ...
      this.$confirm('提示', {
        ...
        if (row.declarationForm) {
          row.declarationForm.objectId = null
          ...
          row.declarationForm.equipmentActualityList.map((equipmentActuality) => {
            equipmentActuality.objectId = null
            return equipmentActuality
          })
          row.declarationForm.accessoriesList.map((accessories) => {
            accessories.objectId = null
            return accessories
          })
          row.declarationForm.investmentCompositionList.map((investmentComposition) => {
    ...
    addDialogTableRow (type) {
      ...
      if (Object.is(type, 'equip')) {
        this.dialogBaseInfoForm.equipmentTableData.push(obj1)
      } else if (Object.is(type, 'enginer')) {
        this.dialogBaseInfoForm.engineerTableData.push(obj2)
      }
    },
    /** 取消-申报表 */
    cancelBtn () {
    ...
    submitCreatePlan () {
      if (Number(this.capitalSourceClassify.surplusUsageQuota) < 0) {
        this.$confirm('已使用额度大于可使用额度，提交后可能被退回，是否继续？', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then((res) => {
          this.$confirm('提交后，其他项目无法转入该资金来源，是否继续？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then((res) => {
            this.capitalSourceClassify.status = 1
            this.capitalSourceClassify.isLocked = 1
            this.capitalSourceClassify.operate = 1
            ...
            investmanage.saveClassify(this.capitalSourceClassify).then((res) => {
              if (Object.is(res.data.resCode, '0000')) {
                this.$emit('childToParent', 'investcard')
              } else {
                this.capitalSourceClassify.status = 0
    ...
    getClassifyList () {
      ...
      investmanage.getClassifyList(query).then((res) => {
        ...
        if (data) {
          ...
          this.optionalclassifyList = data.filter(classify => {
            if (classify.capitalSourceId === 0) {
              classify.capitalSourceName = '工程建设'
            }
            if (classify.status === 0 && classify.capitalSourceId !== this.capitalSourceClassify.capitalSourceId) {
              return classify
            }
          })
        }
