<template>
  <!-- 后台主容器 -->
  <div class="admin-layout" id="agentcheck-page">
    <div v-if="this.$route.path == '/admin/evaluate/agentcheck'">
      ...
      <!--搜索区域start-->
      <div class="search-box agentcheck-search">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button @click="handleTableClick(scope.row,'check')" type="text" size="medium" v-if="$store.getters.permissions.includes('/evaluate/agentcheck/approval')">审核</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!-- 表格区域end -->
  ...
  methods: {
    ...
    handleTableClick (row, type) {
      if (Object.is(type, 'check')) {
        this.$router.push({path: '/admin/evaluate/agentcheck/check', query: {code: row.code}})
      }
    }
  },
  computed: {},
