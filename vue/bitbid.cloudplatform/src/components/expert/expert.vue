...
<!--                     <el-button
 ...
 -->                <el-button
    ...
    <div id="tingxjwdfather">
      <el-dialog title="新增专家" :visible.sync="dialogTableVisible" width="80%">
        <!--新增专家-->
...
import {CityInfo} from '../../assets/js/citydata'
export default {
  ...
  // ********************************************************************专家列表********************************************************************
    ...
    expertData (search) {
      let url = null
      if (search) {
        url = `expert-infos?pageNo=${this.pageNo}&pageSize=${this.pageSize}&messageLike=${this.seacher_input}&expertClassifySelf=${this.classificationvalue}`
      } else {
        url = `expert-infos?pageNo=${this.pageNo}&pageSize=${this.pageSize}`
      }
    ...
    search () {
      ...
      this.pageNo = 0
      this.loading = true
      this.axios.get(`expert-infos?pageNo=${this.pageNo}&pageSize=${this.pageSize}&messageLike=${this.seacher_input}&expertClassifySelf=${this.classificationvalue}`).then((res) => {
        this.loading = false
        this.tableData = res.data.experts.list
        this.total = res.data.experts.total
        this.currentPage = 1
    ...
    // 修改
    editFun (scope) {
      this.$router.push({path: '/expert/update', query: {data: scope}})
    },
    // 单条查看
    seeData (scope) {
