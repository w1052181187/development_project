...
export default{
  ...
  methods: {
    ...
    editList () {
      this.axios.get(`charge/${this.$route.query.objectIds}?_t=${new Date().getTime()}`).then((res) => {
        ...
        res.data.chargeManagement.operatingRecords.map((ite) => {
          ...
          let operator = ite.operator
          this.operators.map((item) => {
            if (item.value === operator) {
              ite.operator = item.label
            }
          })
    ...
    operatorUser () {
      ...
      useradmin.henanuserEditlook(this.$store.getters.authUser.userId).then((res) => {
        operator.value = res.data.user.objectId
        operator.label = res.data.user.name
