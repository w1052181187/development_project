...
<script>
import {projectFlowInfo} from '@/api/cloudplatform/index'
export default {
  components: {
  },
  data () {
  ...
  methods: {
    ...
    // 查看
    lookBtn (scope) {
      this.$router.push({path: `/processManage/proProcessSet/detail/${scope.row.flowType}`, query: {flowType: scope.row.flowType}})
    },
    // 流程管理
    updateBtn (scope) {
      this.$router.push({path: `/processManage/proProcessSet/update`, query: {flowType: scope.row.flowType}})
    },
    // 删除
    delBtn () {
