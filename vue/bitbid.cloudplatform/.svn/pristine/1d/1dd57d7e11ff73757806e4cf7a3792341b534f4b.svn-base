...
export default {
  ...
  methods: {
    ...
    // 流程管理
    processtBtn (scope) {
      this.$router.push({path: `/processManage/projectProcess/project/process/${scope.row.code}`, query: {code: this.$route.query.code}})
    },
    // 修改
    updateBtn (scope) {
    ...
    /** 流程状态:1.已保存；2.待审批；3.已审批；4.已驳回；5.已提交（不需要审批）；6.招标公告; 7.资格预审公告; 8.招标邀请书;  9.资格预审文件; 10.响应资格预审; 11.资格审查结果; 12.招标文件; 13.响应招标; 14.开标评标; 15.废标结果公告; 16.定标结果; 17.完结  */
