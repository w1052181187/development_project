<template>
  <div>
    ...
    <div class="content_bigbox" id="content_bigbox">
      ...
      <div class="da_xz_ckbigbox" style="border: none">
        ...
        <div class="seacher_box">
            <el-input placeholder="请输入部门名称关键词" v-model="deptseainput"></el-input>
            <el-input placeholder="请输入项目经理关键词" v-model="prjectseainput"></el-input>
    ...
    <!--新增文档弹框-->
    <div id="xjwdfather">
      <el-dialog title="选择归档标段" :visible.sync="dialogTableVisible" width="80%">
        <!--线上线下选择-->
        <div class="xingzhi_box">
          ...
          <div class="xinzhiright_box" v-show="tab1">
            <el-select class="xxgdxz" v-model="value2" style="vertical-align: top" placeholder="请选择">
              <el-option
            ...
            <el-input placeholder="请输入内容" v-model="input5" class="input-with-select" v-show="tab1">
        ...
        <div class="select_tabbox" v-show="tab1">
          ...
          <el-table :data="gridData" border>
            <el-table-column
              ...
              width="80">
              <template slot-scope="scope">
                <input type="radio" v-model="radios" :value="scope.row.tk_bdbh">
              </template>
            </el-table-column>
            <el-table-column
              prop="sectionNumber"
        ...
        <!--线上项目数据-->
        <!--线下项目数据-->
        <div class="selectxxxm_bigbox" v-show="tab2">
...
export default {
  ...
  methods: {
    ...
    getData (search) {
      let url = null
      if (search) {
        url = `project-infos?pageNo=${this.pageNo}&pageSize=${this.pageSize}&enterpriseId=${this.$store.getters.authUser.enterpriseId}&userId=${this.$store.getters.authUser.userId}&isDelete=${0}&sectionName=${encodeURIComponent(this.biaoduanseainput)}&userNameLike=${encodeURIComponent(this.prjectseainput)}&deptNameLike=${encodeURIComponent(this.deptseainput)}&_t=${new Date().getTime()}`
      } else {
        url = `project-infos?pageNo=${this.pageNo}&pageSize=${this.pageSize}&enterpriseId=${this.$store.getters.authUser.enterpriseId}&userId=${this.$store.getters.authUser.userId}&isDelete=${0}&_t=${new Date().getTime()}`
      }
    ...
    search () {
      ...
      this.dataIndex = 1
      this.pageNo = 0
      this.axios.get(`project-infos?pageNo=${this.pageNo}&pageSize=${this.pageSize}&enterpriseId=${this.$store.getters.authUser.enterpriseId}&userId=${this.$store.getters.authUser.userId}&sectionName=${encodeURIComponent(this.biaoduanseainput)}&userNameLike=${encodeURIComponent(this.prjectseainput)}&deptNameLike=${encodeURIComponent(this.deptseainput)}&_t=${new Date().getTime()}`).then((res) => {
        this.tableData = res.data.projectInformationInfo.list
        this.tableData.map((item) => {
          if (!item.agentStructure) {
            item.agentStructure = '-----------'
    ...
    // 查看
    lookFun (scope) {
      this.$router.push({path: `/archives/detail/${scope.row.objectId}`, query: {objectId: scope.row.objectId, deptName: scope.row.deptName.name, managerName: scope.row.managerName.name}})
    },
    // 编辑
    deleteFun (scope) {
      this.$router.push({path: '/archives/update', query: {objectId: scope.row.objectId, btn: true, flangshow: true, deptName: scope.row.deptName.name, managerName: scope.row.managerName.name, editStatus: true}})
    },
    // 线下线上切换
    changeValue (value) {
      this.tab1 = !this.tab1
    ...
    // 导出excel
    excelBtn () {
      window.open(`${loadUrl}project-infos/createExcel?enterpriseId=${this.$store.getters.authUser.enterpriseId}&userId=${this.$store.getters.authUser.userId}&isDelete=${0}`)
    }
  },
  mounted () {
    this.getData()
