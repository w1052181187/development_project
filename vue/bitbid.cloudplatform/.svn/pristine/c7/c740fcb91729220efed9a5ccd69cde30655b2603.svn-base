    ...
    <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb_box">
      ...
      <template v-if="isApproved">
        <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb_box">
          ...
          <el-breadcrumb-item :to="{ path: '/dailyManage/onlineapprove/initiate' }" v-if="isApproved === '3'">我发起的</el-breadcrumb-item>
          <el-breadcrumb-item :to="{ path: '/dailyManage/onlineapprove/need' }" v-else>待我审批</el-breadcrumb-item>
          <el-breadcrumb-item class="active_bread">{{isApproved === '1' ? '审批 ': isApproved === '2' ? '审批详情 ': '查看申请'}}</el-breadcrumb-item>
        </el-breadcrumb>
      </template>
      <template v-else>
    ...
    <div class="main">
      <el-form :model="updateForm" :validate-on-rule-change="true">
              ...
              <el-table
                ...
                <el-table-column
                  ...
                  <template slot-scope="scope">
                    ...
                    <span  v-if="scope.row.auditStatus === 0">不通过</span>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="auditorName"
        ...
        <el-form-item class="submit-radio">
          <el-button type="primary" @click="cancleApprove" v-if="isApproved === '3'" :loading="isSubmiting">撤回</el-button>
          <template v-else>
            <el-button type="primary" @click="submitApprove" v-if="isApproved === '1'" :loading="isSubmiting">提交</el-button>
...
import {bulletinInfo, approvalRecord, approvalTask} from '@/api/cloudplatform/index'
...
export default {
  ...
  methods: {
    ...
    submitApprove () {
      ...
      this.updateForm.relatedCode = this.objectId
    ...
    cancleApprove () {
      this.isSubmiting = true
      approvalTask.recallMyTask(this.objectId).then((res) => {
        this.isSubmiting = false
        if (res.data.resCode === '0000') {
          // this.$route.go(-1)
        }
