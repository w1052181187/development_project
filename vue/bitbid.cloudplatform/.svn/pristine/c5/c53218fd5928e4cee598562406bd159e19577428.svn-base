<template>
  <div>
    ...
    <div id="zztxjwdfather">
      <el-dialog title="选择业主" :visible.sync="dialogTableVisible" width="80%">
        <!--线上线下选择-->
        <div class="xingzhi_box">
          ...
          <div class="xinzhiright_box" v-show="tab2">
            ...
            <el-input placeholder="请输入内容" v-model="inputseacher" class="input-with-select" v-show="tab2">
        ...
        <!--线上线下选择-->
        <!--线下项目数据-->
        <div class="selectxxxm_bigbox" v-show="tab1">
        ...
        <!--线下项目数据-->
        <!--线上项目数据-->
        <div class="select_tabbox" v-show="tab2">
          <!--表格数据-->
...
export default {
  ...
  // ********************************************************************业主列表********************************************************************
    ...
    ownerData (search) {
      let url = null
      if (search) {
        url = `owner-infos?pageNo=${this.pageNo}&pageSize=${this.pageSize}&messageLike=${this.seacher_input}&ownerClassify=${this.classificationvalue}`
      } else {
        url = `owner-infos?pageNo=${this.pageNo}&pageSize=${this.pageSize}`
      }
    ...
    // 修改
    editFun (scope) {
      this.$router.push({path: '/owner/update', query: {data: scope}})
    },
    // 搜索功能
    search () {
      this.currentPage = 1
      ...
      this.pageNo = 0
      this.loading = true
      this.axios.get(`owner-infos?pageNo=${this.pageNo}&pageSize=${this.pageSize}&messageLike=${this.seacher_input}&ownerClassify=${this.classificationvalue}`).then((res) => {
        this.loading = false
        this.tableData = res.data.ownerInfo.list
        this.total = res.data.ownerInfo.total
        this.currentPage = 1
    ...
    // 跳转项目管理
    ownerPro (scope) {
      this.$router.push({path: '/owner/project', query: {objectId: scope.row.objectId, code: scope.row.code, enterpriseName: scope.row.enterpriseName}})
    },
    // 线下线上切换
    changeValue (value) {
      this.tab1 = !this.tab1
    ...
    // open2 () {
    // this.$message({
    //     message: '您已成功邀请：*****公司 激活账户，请等待对方处理。',
    //        type: 'success'
    //      })
    //    }
  },
