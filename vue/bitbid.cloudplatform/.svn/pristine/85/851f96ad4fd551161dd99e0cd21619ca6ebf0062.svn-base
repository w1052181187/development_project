      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button
              type="text"
              size="small" @click="returnOne(scope)" v-if="scope.row.paymentProjress === 0">
              退还
            </el-button>
            <el-button
              type="text"
      ...
      <el-dialog title="批量退还" :visible.sync="dialogFormVisible" width="30%">
        ...
        <el-form :model="ruleForm" ref="ruleForm" :rules="rules"  >
          ...
          <el-form-item label="备注：" class="expertRemarkbox" :label-width="formLabelWidth" style="margin-bottom: 0">
            ...
            <span style="float: right;font-size: 12px;color: red">* 退还金额与缴纳金额相同</span>
          </el-form-item>
        </el-form>
...
export default{
  ...
  methods: {
    ...
    addCharge () {
      this.$router.push({path: '/projectpayments/manage/bondlist/add', query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, costTypes: this.$route.query.costTypes, expenseId: this.$route.query.expenseId}})
    },
    editBtn (scope) {
      this.$router.push({path: '/projectpayments/manage/bondlist/update', query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, costTypes: this.$route.query.costTypes, objectIds: scope.row.objectId, expenseId: this.$route.query.expenseId}})
    },
    lookBtn (scope) {
      this.$router.push({path: `/projectpayments/manage/bondlist/detail/${scope.row.objectId}`, query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, costTypes: this.$route.query.costTypes, objectIds: scope.row.objectId, expenseId: this.$route.query.expenseId}})
    },
    returnOne (scope) {
      this.$router.push({path: '/projectpayments/manage/bondlist/return', query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, costTypes: this.$route.query.costTypes, objectIds: scope.row.objectId, expenseId: this.$route.query.expenseId}})
    },
    // 删除
    deleteObject (scope) {
