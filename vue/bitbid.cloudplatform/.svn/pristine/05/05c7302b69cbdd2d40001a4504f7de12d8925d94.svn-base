<template>
  <div class="cloudcontent" id="cloud_processedit">
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb_box">
      ...
      <el-breadcrumb-item class="active_bread">添加中标候选人公示疑义及澄清</el-breadcrumb-item>
    </el-breadcrumb>
    <!--面包屑-->
    <div class="main">
...
import {questionAndAnswerInfo, bidSection} from '@/api/cloudplatform'
export default {
  components: {
    editor,
    uploadFile
  ...
  methods: {
    ...
    // 提交/保存
    submit (form, status) {
      //      let fileDoubtList = this.updateForm.fileInformationList.filter(item => item.businessType === '1')
      //      let fileClarifyList = this.updateForm.fileInformationList.filter(item => item.businessType === '2')
      //      if (fileDoubtList.length < 1) {
      //        this.$message({
      //          message: '请上传疑义附件！',
      //          type: 'warning'
      //        })
