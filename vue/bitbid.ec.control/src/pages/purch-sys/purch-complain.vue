  ...
  <div id="order-statis-sys-page" class="cont-view">
    ...
    <div class="bread-box">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>采购投诉</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <!-- 面包屑区域end -->
    <!-- 搜索区域start -->
      ...
      <el-dialog
        ...
        <el-form :model="complainForm" :rules="rules" ref="complainForm" label-width="140px" class="demo-ruleForm">
          <el-form-item label="电商平台名称：" prop="beComplainantId">
            <el-select v-model="complainForm.beComplainantId"  @change="selectObj" clearable placeholder="请选择">
              <el-option
                v-for="item in ecPlatform"
                :key="item.id"
                :label="item.userName"
...
export default {
  ...
  methods: {
    ...
    selectObj (vId) {
      this.selectEcName = this.ecPlatform.find((item) => {
        return item.id === vId
      })
    },
    /** 搜索 */
    ...
    submitComplain () {
      this.$refs['complainForm'].validate(valid => {
        if (valid) {
          ...
          this.complainForm.beComplainantName = this.selectEcName.userName
          this.complainForm.beComplainantId = this.selectEcName.id
    ...
    getTableData () {
      ...
      this.searchForm.complainantId = this.$store.getters.authUser.userId
