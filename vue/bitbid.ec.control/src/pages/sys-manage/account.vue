  ...
  <div id="account-page"  class="cont-view">
    ...
    <div class="dialog-box">
      ...
      <el-dialog
        ...
        @closed="close('roleForm')"
        width="30%"
        center>
        <el-form :model="roleForm" :rules="roleRules" ref="roleForm" label-width="110px" class="demo-ruleForm">
  ...
  methods: {
    ...
    handleTableBtn (row, type) {
      if (Object.is(type, 'role')) {
        ...
        if (row.userRoles && row.userRoles.length > 0) {
          this.roleForm.roleId = row.userRoles[0].roleId
        }
    ...
    submit () {
      this.$refs['submitForm'].validate(valid => {
        if (valid) {
          delete this.submitForm.rePassword
          this.submitForm.type = 5
    ...
    submitRole () {
      this.$refs['roleForm'].validate(valid => {
        if (valid) {
          role.saveUserRoleAssociation(this.roleForm).then(res => {
            if (res.data.resCode === '0000') {
