    ...
    <div class="fileBox">
      <ul v-for="(item, index) in fileList" class="ul-class" :key="index">
        <li style="width: 100%; overflow:hidden; float: left;">
          ...
          <i class="iconfont icon-chahao chahao" :file-id="item.relativePath" @click="deleteFile($event)"></i>
        </span>
        </li>
      </ul>
    </div>
...
<style>
  ...
  .chahao {
    ...
    float: right;
    margin-top: 10px;
    background: url("../../../static/images/upload_cha.png") top left no-repeat;
    display: block;
  }
  .chahao:hover{
    background: url("../../../static/images/upload_cha.png") bottom left no-repeat;
  }
  .fileBox{
    width: 100%;
    overflow: hidden;
