      ...
      <div class="forgetPwd_main">
        ...
        <el-form :model="stepFirstForm" :rules="rules" ref="stepFirstForm" label-width="110px" :validate-on-rule-change="true" v-if="this.active === 1">
          ...
          <el-form-item label="短信验证码" prop="smsCheckCode">
            ...
            <el-button type="primary" class="getSmsCheckCode" v-show="show" @click="getCode('stepFirstForm')">获取验证码</el-button>
    ...
    return {
      ...
      rules: {
        ...
        smsCheckCode: [
          ...
          {min: 6, max: 6, message: '短信验证码为6位数', trigger: ['blur']}
        ]
      },
      rules2: {
        newPwd: [
    ...
    getCode (formName) {
      if (checkRepeatClick) {
        if (!this.stepFirstForm.users) {
          ...
          this.$refs[formName].validateField('users', (errMsg) => {
            if (errMsg) {
              this.$message({
                type: 'warning',
                message: '请输入正确的手机号'
