      ...
      <div class="mainContent">
        ...
        <ul class="qualificationList">
          <li v-if="dataForm.qualificationInformationList && dataForm.qualificationInformationList.length !== 0"
            ...
            <el-form-item :prop="'qualificationInformationList.'+index+'.qualificationName'" :rules="rules.qualificationName" class="itemRule">
              <el-input v-if="isEdit" v-model="item.qualificationName" placeholder="资质名称"></el-input>
            </el-form-item>
            <p v-if="!isEdit" :title="item.qualificationName">{{item.qualificationName}}</p>
          </li>
        ...
        <div class="nodata" v-if="!dataForm.qualificationInformationList || dataForm.qualificationInformationList.length === 0">
          <img src="../../../static/images/enterprise/noqualification.png"/>
          <p>暂无资质信息</p>
        </div>
      </div>
    </el-form>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        qualificationName: [
          {message: '请输入资质名称', required: true, trigger: 'blur'}
        ]
      }
    }
  },
