<template>
  <div class="">
    <form ref="webform" name="webform" id="webform" action="" method="post" style="height: 400px">
...
export default {
  ...
  methods: {
    ...
    saveSignPDF () {
      var count = this.$refs.webform.WebPDF.SignatureCount()
      if (count > 0) {
        if (confirm('确定要发布吗？')) {
          this.$refs.webform.WebPDF.WebSetMsgByName('objectId', this.pdfData.objectId)
          this.$refs.webform.WebPDF.WebSetMsgByName('signType', this.pdfData.signType)
          this.$refs.webform.WebPDF.FileName = '已签章的评标报告'
          this.$refs.webform.WebPDF.ExtParam = 'bidOpenReport'
          if (this.$refs.webform.WebPDF.WebSave()) {
    ...
    pdfList () {
      reviewControl.pdfDetail(this.objectId).then((res) => {
      })
    }
  },
  mounted () {}
