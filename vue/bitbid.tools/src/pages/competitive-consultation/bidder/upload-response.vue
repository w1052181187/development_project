<template>
  <div id="negBri" class="content_bgibox">
    <div class="setup_bigbox">
      <div class="proinfobox" v-if="(oldFileName === '' || oldFileName) && !subStatus">
    ...
    <div class="score_bigbox" style="padding-left: 20px;padding-right: 20px;box-sizing: border-box">
      <div style="padding-top: 20px;">
        ...
        <div style="display: inline-block;width: 100%">
          <commonFileUpload @upLoadFile="upLoadFile" :oldFileName="oldFileName" :fileTypeStatus="fileTypeStatus" v-if="!subStatus"></commonFileUpload>
          <div v-else>
            <span style="font-size: 14px;color: #606266;" v-if="oldFileName">澄清与承诺文件：</span>
            <fileDownload :oldFileName="oldFileName" :filePath="briefFilePath" class="cqcn_file"></fileDownload>
          </div>
        </div>
      </div>
      <!--<el-dialog-->
      ...
      <!--</div>-->
      <div class="proinfobox" style="margin-top: 20px">
        <span class="line_bigbox"></span>
        ...
        <div style="text-align: center;margin-top: 35px">
          <el-form :model="ruleForm" :rules="expertRules" ref="ruleForm" label-width="180px" class="demo-ruleForm">
            ...
            <el-form-item label="提交截止时间：" style="text-align: left">
              <span>{{submitTimePice}}</span><span style="color: red">（请在截止时间前提交文件及报价，否则无法进入下一轮磋商）</span>
            </el-form-item>
          </el-form>
...
export default {
  ...
  methods: {
    ...
    viewPageList () {
      OnLoad()
      consultingSummary.viewClarifyPage(this.$route.params.num).then((res) => {
    ...
    list () {
      ...
      this.pdfLink = ''
      this.objectIdQuery = ''
      consultingSummary.obtainResponse(this.$route.params.num).then((res) => {
        ...
        if (res.data.BidOpenNegotiatingAnswer) {
          if (res.data.BidOpenNegotiatingAnswer.offerFile) {
            ...
            this.briefFilePath = res.data.BidOpenNegotiatingAnswer.offerFilePath
            this.isFile = true
          }
    ...
    // 下一轮
    nextRound () {
      consultingSummary.gainOffer(this.$route.params.num).then((res) => {
        console.log(res)
    ...
    upLoadFile (file) {
      ...
      //        this.loading = true
      //        console.log(url)
      //        consultingSummary.uploadResponse(url).then((res) => {
      //          console.log(res)
      //          if (res.data.resCode === '0000') {
      //            this.loading = false
      //            this.list()
    ...
    inputChange (scope) {
      ...
      if (reg.test(scope)) {
        consultingSummary.offerUpdate(this.ruleForm).then((res) => {
          this.nextRound()
        })
      }
    },
      ...
      this.$refs[ruleForm].validate((valid) => {
        if (valid) {
          if (SignOnClick(this.loginName)) {
            ...
            consultingSummary.viewClarifySub(this.$route.params.num).then((res) => {
              this.viewPageList()
            })
          }
        } else {
