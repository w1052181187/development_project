  ...
  <div id="goDownStartEview" class="content_bgibox">
    ...
    <div class="setup_bigbox">
      <div class="proinfobox">
        ...
        <span class="title_bigbox">下达开始磋商命令</span>
      </div>
      <el-button type="success" class="go_down_btnbox" @click="downBtn" v-if="!orderStatus">下达开始磋商命令</el-button>
      <el-button type="success" class="go_down_btnbox" disabled v-else>已下达磋商命令</el-button>
      <!--操作说明-->
      <div class="operation_bigbox">
        <div class="operation_title">操作说明</div>
        <div class="operation_content">
          <span>如果所有评委都已准备就绪，就可以下达开始磋商的命令。这时候评委可以开始评标。</span>
        </div>
      </div>
      <!--操作说明-->
      <!--专家签到情况-->
...
export default {
  ...
  methods: {
    // 查询下达状态
    inquiryDownStatus () {
      consultingReviewControl.inquiryCommandStatus().then((res) => {
    ...
    // 下达开始命令
    downBtn () {
      consultingReviewControl.douwnStartEvaluation().then((res) => {
