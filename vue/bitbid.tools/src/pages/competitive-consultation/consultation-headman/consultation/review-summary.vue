...
import { consultingPoints, consultingReview } from '@/api'
export default {
  components: {
    fileDownload,
    editor
  ...
  methods: {
    ...
    // 复核按钮
    btnReview () {
      consultingReview.numhReview(this.$route.params.num).then((res) => {
        this.init()
      })
    },
    // 确认按钮
    btnConfirmt () {
      consultingReview.numConfirm(this.$route.params.num).then((res) => {
        this.consultationHeadSummaryLists()
      })
    }
  },
