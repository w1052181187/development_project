<template>
  <div id="examinationQuaSumm" class="content_bgibox">
    <!--评审-->
...
export default {
  ...
  methods: {
    ...
    tabReviewJudgesListLists (objectId) {
      ...
      consultingReview.ReviewProjectSummaryList(objectId, params).then((res) => {
    ...
    tabReviewJudgesLists (tenderId, itemId) {
      ...
      consultingReview.tabReviewJudgesList(tenderId, itemId, params).then((res) => {
    ...
    // 复核按钮
    btnReview () {
      consultingReview.zgshReview().then((res) => {
        this.init()
      })
    },
    // 确认按钮
    btnConfirmt () {
      consultingReview.zgshConfirm().then((res) => {
        this.reviewSummaryLists()
      })
    }
  },
