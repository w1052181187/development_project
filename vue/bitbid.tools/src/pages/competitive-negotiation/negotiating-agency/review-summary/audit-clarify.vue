        ...
        <div class="zztdemo_ruleForm">
          ...
          <div style="padding: 20px 0;width: 90%">
            ...
            <div style="text-align: center;margin-top: 50px" v-if="isData">
              <el-button type="primary" class="sub_btn_next go_addbtnbox" v-if="isUltimately && !startStatus && isMoreRound === 1" @click="addNumBtn">增加一轮谈判</el-button>
              <el-button type="primary" class="sub_btn_next" @click="startClickBtn" v-if="isUltimately && !startStatus">进入最终轮谈判</el-button>
              <el-button type="primary" class="sub_btn_next" @click="startClickBtn" v-if="!isUltimately && !startStatus">开始下一轮谈判</el-button>
              <el-button type="primary" class="sub_btn_next" disabled v-if="startStatus">已开始下一轮谈判</el-button>
            </div>
          </div>
        </div>
        <!--评审-->
...
export default {
  ...
  methods: {
    ...
    nextOfferList () {
      ...
      agencyReviewSummary.nextOffers(url).then((res) => {
    ...
    downOfferFileBtn (scope) {
      ...
      downloadFile('澄清与承诺文件' + suffixName, scope.row.pdfPath)
    },
    getWinWH () {
