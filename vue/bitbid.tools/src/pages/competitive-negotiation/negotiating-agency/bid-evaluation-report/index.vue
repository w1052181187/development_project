...
export default {
  ...
  methods: {
      ...
      reviewControl.reportView().then((res) => {
        if (res.data.resCode === '0000') {
          ...
          if (report && report.fileId) {
            ...
            reviewControl.pdfDetail(url).then((res) => {
    ...
    // 是否开始签章
    isStartReport () {
      reviewControl.isStartEndReport(this.objectIdQuery).then((res) => {
        if (res.data.isLock === 0) {
          reviewControl.startSignature(this.objectIdQuery).then((res) => {
            this.isStart = true
            this.reportLists()
          })
        } else {
    ...
    // 解锁签章
    unlockedBtn () {
      reviewControl.unlockReport(this.objectIdQuery).then((res) => {
        reviewControl.isStartEndReport(this.objectIdQuery).then((res) => {
          this.reload()
          if (res.data.isLock === 0) {
            this.isStart = false
