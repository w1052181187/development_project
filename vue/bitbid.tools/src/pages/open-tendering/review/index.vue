        ...
        <div class="setup_bigbox">
          ...
          <div class="tableDataSupplier_box">
            <el-table
              ...
              <el-table-column
                prop="checkDesc"
                label="是否合格" align="left" width="120" v-if="!this.reviewTypeStatus">
              ...
              <el-table-column
                ...
                width="80"
                align="center"
                show-overflow-tooltip v-if="this.reviewTypeStatus">
              </el-table-column>
              <el-table-column
                label="得分"
                align="center" v-if="this.reviewTypeStatus">
                <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    negotiatingSuppliersSeleList () {
      openTenderReview.negotiatingSuppliersSele(this.reviewType).then((res) => {
    ...
    negotiatingSuppliersTable () {
      ...
      openTenderReview.negotiatingSuppliersList(url).then((res) => {
        if (res.data.ration === 0) {
          this.reviewTypeStatus = true
        } else {
          this.reviewTypeStatus = false
        }
        this.tableDataSupplier = res.data.BidOpenCheckList
        this.tableDataSupplier.map((its) => {
          this.subStatus = its.checkStatus === 1
    ...
    changeStatusQualified (scope) {
      ...
      if (scope.row.checkDesc === '不合格') {
        ...
        openTenderReview.preservationUnqualified(reasonData).then(() => {
          this.negotiatingSuppliersTable()
        })
      }
    },
    handleScore (id, score, totalScore) {
      ...
      if (reg.test(score)) {
        ...
        openTenderReview.preservationScore(params).then((res) => {
        })
      } else {
        this.$message({
          type: 'warning',
    ...
    confirmationBtns (unqualifiedForm) {
      this.$refs[unqualifiedForm].validate((valid) => {
        if (valid) {
          ...
          openTenderReview.preservationUnqualified(reasonData).then((res) => {
            this.negotiatingSuppliersTable()
            this.dialogFormVisible = false
    ...
    // pdf文件选择列表
    pdfSelectLists () {
      openTenderReview.pdfSelectList(this.reviewType).then((res) => {
