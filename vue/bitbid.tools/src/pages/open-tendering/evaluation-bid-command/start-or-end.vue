<template>
  <div id="goDownStartEview" class="content_bgibox">
    <div class="setup_bigbox">
      ...
      <el-button type="success" class="go_down_btnbox" @click="downBtn" v-if="!orderStatus">{{handleBtnTitle()}}</el-button>
...
<script>
import { openTenderEvaBidCommand } from '@/api'
...
export default {
  ...
  methods: {
    ...
    inquiryDownStatus () {
      ...
      openTenderEvaBidCommand[`${action}`]().then((res) => {
    ...
    downBtn () {
      ...
      openTenderEvaBidCommand[`${action}`]().then(() => {
    ...
    loginInformation () {
      ...
      openTenderEvaBidCommand.loginInfo(parameter).then((res) => {
        this.tableDataSignIn = res.data.activeUserList
      })
    },
    // 下载评标报告
    downloadClick () {
      openTenderEvaBidCommand.downloadReport().then((res) => {
