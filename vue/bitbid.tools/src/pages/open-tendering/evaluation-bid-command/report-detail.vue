<template>
  <div class="" id="gongkai">
    <div style="margin-top: 7vh;padding: 0 20px;box-sizing: border-box">
      <commonFileUpload @upLoadFile="upLoadFile" :oldFileName="oldFileName" v-if="this.$route.params.role === 'agent'"></commonFileUpload>
    </div>
...
export default {
  ...
  methods: {
    // 查询评标报告
    reportLists () {
      openTenderEvaBidCommand.reportView().then((res) => {
        if (res.data.resCode === '0000') {
          ...
          if (report && report.fileId) {
            ...
            openTenderEvaBidCommand.creatReport().then((res) => {
              if (res.data.resCode === '0000') {
                this.reportLists()
                this.loading = false
              }
    ...
    upLoadFile (file) {
      ...
      this.loading = true
      openTenderEvaBidCommand.uploadResponse(url).then((res) => {
        if (res.data.resCode === '0000') {
          this.loading = false
        }
        this.reportLists()
