      ...
      <el-row>
        <el-col :span="4" class="left_scorebox">
          ...
          <div class="score_navbigbox">
            <div v-for="(scoreProject,index) in scoreProjectAggregate" :key="index" :class="{'curr' : index === addrSelected}" @click="chooseAddr(index, scoreProject.objectId, scoreProject.typeOrder)">
...
export default {
  ...
  methods: {
    ...
    navReviewProjectLists () {
      ...
      openTenderReviewProject.navReviewProjectList(url).then((res) => {
        ...
        openTenderReviewProject.tableProjectList(projectUrl).then((res) => {
    ...
    chooseAddr (index, objectId, typeOrder) {
      ...
      openTenderReviewProject.tableProjectList(url).then((res) => {
        this.tableDataScore = res.data.BidOpenCheckItemList
      })
    }
  },
