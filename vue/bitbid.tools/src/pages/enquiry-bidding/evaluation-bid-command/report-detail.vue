<template>
  <div class="" id="look_baopdf">
    ...
    <div style="padding: 0 15px;box-sizing: border-box">
      <commonFileUpload @upLoadFile="upLoadFile" :oldFileName="oldFileName" v-if="!isStart && this.$route.params.role === 'agent'"></commonFileUpload>
      <el-button type="success" class="go_down_endbtnbox" @click="restoreBtn" size="mini" v-if="!isStart && this.$route.params.role === 'agent'">恢复初始模板</el-button>
      <!--操作说明-->
      <div class="operation_bigbox" v-if="!isStart && this.$route.params.role === 'agent'">
      ...
      <div style="margin-top: 15px">
        ...
        <el-button type="success" class="go_down_endbtnbox filedownbtn" size="mini" @click="unlockedBtn" v-if="this.$route.params.role === 'agent'">解除签章锁定</el-button>
      </div>
    </div>
...
export default {
  ...
  methods: {
      ...
      evaBidCommand.reportView().then((res) => {
        if (res.data.resCode === '0000') {
          ...
          if (report && report.fileId) {
            ...
            evaBidCommand.pdfDetail(url).then((res) => {
    ...
    // 是否开始签章
    isStartReport () {
      evaBidCommand.isStartEndReport(this.objectIdQuery).then((res) => {
        if (res.data.isLock === 0) {
          evaBidCommand.startSignature(this.objectIdQuery).then((res) => {
            this.isStart = true
            this.reportLists()
          })
        } else {
    ...
    // 解锁签章
    unlockedBtn () {
      evaBidCommand.unlockReport(this.objectIdQuery).then((res) => {
        evaBidCommand.isStartEndReport(this.objectIdQuery).then((res) => {
          this.reload()
          if (res.data.isLock === 0) {
            this.isStart = false
