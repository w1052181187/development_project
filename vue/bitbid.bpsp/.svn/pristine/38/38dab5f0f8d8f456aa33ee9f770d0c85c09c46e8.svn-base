  ...
  <div id="ad-manage">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>违规公示</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
    <div class="contentbigbox">
      <!--搜索 & 添加按钮-->
      <el-row>
        <el-button type="success"  class="addbutton" @click="addbutton"> + 添加违规公示</el-button>
...
export default {
  ...
  methods: {
    list () {
      ...
      infoManageViolation.queryList(params).then((res) => {
        this.tableData = res.data.violationNoticePageInfo.list
        this.total = res.data.violationNoticePageInfo.total
      })
    },
    // 搜索
    search () {
    ...
    /** 新增 */
    addbutton () {
      this.$router.push({path: '/index/violation/add'})
    },
    /** 表格操作按钮 */
    handleTableBtn (row, type) {
      if (Object.is(type, 'del')) {
        ...
        this.$router.push({path: `/index/violation/detail/${row.objectId}`})
      } else if (Object.is(type, 'edit')) {
        this.$router.push({path: '/index/violation/update', query: {objectId: row.objectId}})
      }
    },
    /** 删除 */
    delete (row) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        // 执行删除
        infoManageViolation.remove(row.objectId).then((res) => {
          this.list()
        })
      }).catch(() => {
        this.$message({
