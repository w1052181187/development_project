  ...
  <div id="rotate-add" class="smaincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>编辑下载</el-breadcrumb-item>
      </el-breadcrumb>
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="下载类型:" prop="type">
          <el-select v-model="ruleForm.type" class="select" style="width:300px">
            <el-option label="操作指南" value="1"></el-option>
            <el-option label="资料下载" value="2"></el-option>
          </el-select>
        </el-form-item>
  ...
  methods: {
    detail () {
      download.detail(this.$route.query.objectId).then((res) => {
        ...
        this.ruleForm.type = res.data.downloadInfo.type.toString()
        this.oldFileName = res.data.downloadInfo.fileInformations[0].fileName
        this.relativePath = res.data.downloadInfo.fileInformations[0].relativePath
      })
    },
    submitForm (name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          download.update(this.ruleForm).then((res) => {
    ...
    upLoadFile (file) {
      ...
      this.ruleForm.fileInformations[0].businessType = 4
    }
  },
  mounted () {
    this.detail()
