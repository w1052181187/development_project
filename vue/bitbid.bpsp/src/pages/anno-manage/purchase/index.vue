<template>
  <div id="purchase" class="smaincontent">
    ...
    <div class="contentbigbox">
      <template>
        <!--搜索 & 添加按钮-->
        <el-row>
          <el-button type="success"  class="addbutton"  @click="addBtn"> + 添加采购公告</el-button>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <div class="statusbigbox" v-if="scope.row.bulletin_type === 3">
                <span class="statusbox">询价公告</span>
              </div>
              <div class="statusbigbox" v-if="scope.row.bulletin_type === 4">
                <span class="statusbox">竞争性谈判公告</span>
              </div>
              <div class="statusbigbox" v-if="scope.row.bulletin_type === 5">
                <span class="statusbox">竞争性磋商公告</span>
              </div>
            ...
            <template slot-scope="scope">
              <div class="statusbigbox">
                <div class="statusbigbox">
                  ...
                  <el-button type="text" size="small" v-if="(scope.row.status === 2) && scope.row.enterprise_id === $store.getters.authUser.enterpriseId" @click="addResultAnno(scope)">
                    添加成交公示
                  </el-button>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    // 修改
    updateBtn (scope) {
      this.$router.push({path: '/annomanage/purchase/update', query: {objectId: scope.row.object_id, status: this.status}})
    },
    /** 删除 */
    deleteBtn (scope) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        // 执行删除
        purchase.remove(scope.row.object_id).then((res) => {
          this.list()
        })
      }).catch(() => {
        this.$message({
    ...
    // 添加
    addBtn () {
      this.$router.push({path: '/annomanage/purchase/add', query: {status: this.status}})
    },
    // 查看
    detailBtn (scope) {
      this.$router.push({path: `/annomanage/purchase/detail/${scope.row.object_id}`, query: {status: this.status}})
    },
    // 跳转成交结果公示添加
    addResultAnno (scope) {
