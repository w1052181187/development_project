<template>
  <div id="resultanno" class="smaincontent">
    <div class="headertitle">
      <!--面包屑-->
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/main' }">首页</el-breadcrumb-item>
        <el-breadcrumb-item>公告管理</el-breadcrumb-item>
        <el-breadcrumb-item>中标公告</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
    <div class="contentbigbox">
      <template>
        <!--搜索 & 添加按钮-->
        <el-row>
          <el-button type="success"  class="addbutton"  @click="addBtn"> + 添加中标公告</el-button>
...
export default {
  ...
  methods: {
    ...
    // 修改
    updateBtn (scope) {
      this.$router.push({path: '/annomanage/resultanno/update', query: {objectId: scope.row.object_id, status: this.status}})
    },
    changeBtn (scope) {
      this.$router.push({path: '/annomanage/resultanno/change', query: {objectId: scope.row.object_id, status: this.status}})
    },
    /** 删除 */
    deleteBtn (scope) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        // 执行删除
        resultAnno.remove(scope.row.object_id).then((res) => {
          this.list()
        })
      }).catch(() => {
        this.$message({
    ...
    // 添加
    addBtn () {
      this.$router.push({path: '/annomanage/resultanno/add/0', query: {status: this.status}})
    },
    // 查看
    detailBtn (scope) {
      this.$router.push({path: `/annomanage/resultanno/detail/${scope.row.object_id}`, query: {status: this.status}})
    },
    // 分页
