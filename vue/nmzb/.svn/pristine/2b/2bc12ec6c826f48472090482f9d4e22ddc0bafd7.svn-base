<template>
  <div id="industry-supervision">
    <div class="headertitle">
      <!--面包屑-->
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/main' }">首页</el-breadcrumb-item>
        <el-breadcrumb-item>信息管理</el-breadcrumb-item>
        <el-breadcrumb-item>行业监督</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
    <div class="contentbigbox">
      <!--搜索 & 添加按钮-->
      <el-row>
        <el-button type="success"  class="addbutton" @click="addbutton"> + 添加行业监督</el-button>
        <div class="anno_seacher_box">
          <span>类型：</span>
          <el-select v-model="industryType" class="select">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <span v-if="scope.row.type === 6">水利</span>
            <span v-if="scope.row.type === 7">交通</span>
            <span v-if="scope.row.type === 8">铁路</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="createDate"
...
export default {
  ...
  methods: {
    ...
    /** 新增 */
    addbutton () {
      this.$router.push({path: '/index/industry-supervision/add'})
    },
    /** 表格操作按钮 */
    handleTableBtn (row, type) {
      if (Object.is(type, 'del')) {
        ...
        this.$router.push({path: `/index/industry-supervision/detail/${row.objectId}`})
      } else if (Object.is(type, 'edit')) {
        this.$router.push({path: '/index/industry-supervision/update', query: {objectId: row.objectId}})
      }
    },
    /** 删除 */
    delete (row) {
