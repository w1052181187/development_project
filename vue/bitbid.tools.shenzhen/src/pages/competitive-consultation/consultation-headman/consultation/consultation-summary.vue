...
export default {
  ...
  methods: {
    // 页面访问
    viewPageList () {
      consultingSummary.viewSummaryPage(this.$route.params.num).then((res) => {
      })
    },
    // 磋商纪要
    consultationBriefLists () {
      consultingPoints.consultingBriefList(this.$route.params.num).then((res) => {
    ...
    // 发布
    publish () {
      consultingPoints.publish(this.$route.params.num).then((res) => {
        this.consultationBriefLists()
      })
    },
    // 驳回
    back () {
    ...
    determineBtn (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          consultingPoints.backNeoBrief(this.$route.params.num, reasonParameter).then((res) => {
            this.consultationBriefLists()
