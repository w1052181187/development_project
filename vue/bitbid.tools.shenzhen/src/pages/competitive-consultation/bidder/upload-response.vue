<template>
  <div id="negBri" class="content_bgibox">
    ...
    <div class="score_bigbox" style="padding-left: 20px;padding-right: 20px;box-sizing: border-box">
      <div style="padding-top: 20px;" v-if="!subStatus">
      ...
      <div class="proinfobox">
        <span class="line_bigbox"></span>
        <span class="title_bigbox">下一轮报价</span>
      </div>
...
import {OnLoad, SignOnClick} from '@/assets/js/signature-sz'
export default {
  components: {
    commonFileUpload,
    fileDownload
  ...
  methods: {
    ...
    inputChange (scope) {
      ...
      if (scope >= 1) {
        if (flagRex.test(scope)) {
          ...
          this.piceStatus = false
          return false
        }
      }
      if (reg.test(scope)) {
        // 验证通过
        this.piceStatus = true
      } else {
        this.piceStatus = false
      }
    },
    ...
    submitBtn (ruleForm) {
      ...
      this.$refs[ruleForm].validate((valid) => {
        if (valid) {
          SignOnClick(this.loginName, this.piceSub)
        } else {
          return false
        }
      })
