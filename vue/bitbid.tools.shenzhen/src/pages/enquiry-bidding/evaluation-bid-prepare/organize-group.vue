  ...
  <div id="orgGroupEview" class="content_bgibox">
    ...
    <div class="setup_bigbox" style="padding-bottom: 70px">
      <div class="proinfobox">
        ...
        <span class="title_bigbox">组建{{this.groupTypeDatas.name}}小组</span>
...
<script>
import commonProject from '@/components/common/competitive-negotiation/project'
import {isvalidId} from '@/assets/js/validate'
import { reviewProject } from '@/api'
const leaderActions = {
  'enquiry': {name: '询价', leaderRole: 2, memberRole: 3},
  'trial': {name: '资审', leaderRole: 4, memberRole: 5}
}
const leaderType = {
  'enquiry': 0,
  'trial': 1
...
export default {
  ...
  methods: {
    ...
    setUpNegotiationLists () {
      ...
      reviewProject.setUpNegotiationList(url).then((res) => {
      ...
      this.$refs[expertForm].validate((valid) => {
        if (valid) {
          if (flag) { // 增加
            ...
            reviewProject.setUpNegotiationAdd(this.expertForm).then((res) => {
              this.handleData(res.data.resCode)
            })
          } else { // 编辑
            reviewProject.setUpNegotiationUpdate(this.expertForm).then((res) => {
              this.handleData(res.data.resCode)
            })
          }
        } else {
    ...
    // 编辑时查看人员信息
    editBtn (scope) {
      reviewProject.setUpNegotiationLook(scope.row.objectId).then((res) => {
        this.flag = false
        this.titileName = '编辑'
    ...
    deleteBtn (scope) {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        reviewProject.setUpNegotiationDelete(scope.row.objectId).then((res) => {
          this.setUpNegotiationLists()
    ...
    subExpert () {
      ...
      reviewProject.setLeader(this.expertId, this.groupTypeDatas.memberRole, this.groupTypeDatas.leaderRole).then((res) => {
        this.setUpNegotiationLists()
      })
    },
    init () {
