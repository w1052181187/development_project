      ...
      <el-col :span="14">
        ...
        <div class="setup_bigbox">
          <div class="proinfobox" style="margin-bottom: 25px">
            ...
            <span class="title_bigbox">磋商</span>
          </div>
          <el-form :model="ruleForm" ref="ruleForm" label-width="160px" class="demo-ruleForm">
            <el-form-item label="磋商供应商：" prop="content" class="bitianicon">
            ...
            <el-form-item label="磋商要点：" prop="content" class="bitianicon">
              <div class="editor">
                <editor ref="ueditor" class="ueditor" :content="content" :editread="true"></editor>
              </div>
            </el-form-item>
            <el-form-item label="报价：" prop="content" class="bitianicon">
...
import { consultingReview, consultingPoints } from '@/api'
export default {
  components: {
    editor
  },
  ...
  methods: {
    ...
    consultingSuppliersSeleList () {
      consultingReview.consultingSele(this.$route.params.num).then((res) => {
        this.reviewSelect = res.data.BidOpenUserList
        this.reviewSelId = this.reviewSelect[0].objectId
        this.consultingLogLists() // 根据供应商id和谈判轮数获取数据
      })
    },
    // 上一家
    previousBtn () {
    ...
    radioChange (val) {
      if (val === 0) {
        ...
        consultingPoints.consultingLogUpdate(params).then((res) => {
          this.consultingLogLists()
        })
      }
    },
    // 查看数据
    consultingLogLists () {
      consultingPoints.negotiatingLogList(this.reviewSelId, this.$route.params.num).then((res) => {
        ...
        if (res.data.BidOpenNegotiatingLog.logStatus === 1) {
  ...
  watch: {
    '$route': 'consultingSuppliersSeleList'
  },
  mounted () {
