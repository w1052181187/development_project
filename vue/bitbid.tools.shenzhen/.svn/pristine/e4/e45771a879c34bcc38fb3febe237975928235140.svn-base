              ...
              <el-table
                ...
                <el-table-column
                  ...
                  <template slot-scope="scope">
                    ...
                    <el-button type="text" size="small" v-if="scope.row.offerFile && scope.row.isSubmit === 1" @click="downOfferFileBtn(scope)">下载澄清与承诺文件</el-button>
                    <el-button type="text" size="small" disabled v-else>下载澄清与承诺文件</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </el-form-item>
  ...
  methods: {
    ...
    reviewSummaryLists () {
      consultingSummary.resultList(this.frequency).then((res) => {
        ...
        if (this.tableDataConsultation.length === 0) {
          this.tableDataJudges = []
        } else {
          this.tableReviewRadios = this.tableDataConsultation[0].tenderId
          this.tenderIds = this.tableDataConsultation[0].tenderId
    ...
    tabReviewJudgesLists () {
      consultingSummary.tabReviewJudgesList(this.tenderIds, this.frequency).then((res) => {
