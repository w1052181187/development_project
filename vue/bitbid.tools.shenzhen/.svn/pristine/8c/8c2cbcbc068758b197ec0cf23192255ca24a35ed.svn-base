          ...
          <div class="tableDataSupplier_sonbox">
            <el-table
              ...
              <el-table-column
                ...
                label="结论"
                width="120"
                show-overflow-tooltip v-if="!this.judgeTypeStatus">
              </el-table-column>
              <el-table-column
                prop="price"
                label="报价"
...
export default {
  ...
  methods: {
    ...
    reviewSummaryLists () {
      ...
      openTenderReview.reviewSummaryList(checkType).then((res) => {
        console.log(res)
        if (res.data.ration === 0) {
          this.judgeTypeStatus = true
        } else {
          this.judgeTypeStatus = false
        }
    ...
    tabReviewJudgesListLists () {
      let checkType = this.checkType
      openTenderReview.ReviewProjectSummaryList(this.tableReviewSummaryId, checkType).then((res) => {
    ...
    tabReviewJudgesLists (tenderId, itemId) {
      let checkType = this.checkType
      openTenderReview.tabReviewJudgesList(tenderId, itemId, checkType).then((res) => {
