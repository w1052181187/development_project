<template>
  <div id="orgGroupEview" class="content_bgibox">
    <!--项目信息-->
    ...
    <div class="setup_bigbox" style="padding-bottom: 70px">
      <div class="proinfobox">
        ...
        <span class="title_bigbox">组建{{this.groupTypeDatas.name}}</span>
        <div class="formmian_projectbox">
          ...
          <!--添加按钮-->
          <!--添加弹窗-->
          <el-dialog :title="titileName" :visible.sync="dialogFormVisible" width="30%" :before-close="cancelBtn">
...
<script>
import commonProject from '@/components/common/open-tendering/project'
import {isvalidId} from '@/assets/js/validate'
import { openTenderReviewProject } from '@/api'
const leaderActions = {
  'enquiry': {name: '评标委员会', leaderRole: 2, memberRole: 3},
  'trial': {name: '资审小组', leaderRole: 4, memberRole: 5}
}
const leaderType = {
  'enquiry': 0,
  'trial': 1
  ...
  methods: {
    ...
    setUpNegotiationLists () {
      let url = {
        ...
        groupBy: leaderType[`${this.$route.params.type}`]
      }
      openTenderReviewProject.setUpNegotiationList(url).then((res) => {
        this.tableDataExpert = res.data.BidOpenUserList
        let isLeader
        this.tableDataExpert.map((its) => {
          isLeader = its.initRoleId === this.groupTypeDatas.leaderRole
          if (this.$route.params.type === 'enquiry') {
            isLeader = its.initRoleId === this.groupTypeDatas.leaderRole || its.initRoleId === 9
          }
          if (isLeader) {
      ...
      this.$refs[expertForm].validate((valid) => {
        if (valid) {
          if (flag) { // 增加
            ...
            this.expertForm.initRoleId = this.groupTypeDatas.memberRole
            openTenderReviewProject.setUpNegotiationAdd(this.expertForm).then((res) => {
              this.handleData(res.data.resCode)
            })
          } else { // 编辑
            openTenderReviewProject.setUpNegotiationUpdate(this.expertForm).then((res) => {
              this.handleData(res.data.resCode)
            })
          }
        } else {
    ...
    // 编辑时查看人员信息
    editBtn (scope) {
      openTenderReviewProject.setUpNegotiationLook(scope.row.objectId).then((res) => {
        this.flag = false
        this.titileName = '编辑'
    ...
    deleteBtn (scope) {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        openTenderReviewProject.setUpNegotiationDelete(scope.row.objectId).then((res) => {
          this.setUpNegotiationLists()
    ...
    subExpert () {
      ...
      openTenderReviewProject.setLeader(this.expertId, this.groupTypeDatas.memberRole, this.groupTypeDatas.leaderRole).then((res) => {
        this.setUpNegotiationLists()
      })
    },
    init () {
      this.groupTypeDatas = leaderActions[`${this.$route.params.type}`]
      this.setUpNegotiationLists()
    }
  },
  watch: {
