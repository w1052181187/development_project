  ...
  <div id="orgGroupEview" class="content_bgibox">
    ...
    <div class="setup_bigbox" style="padding-bottom: 70px">
      <div class="proinfobox">
        ...
        <span class="title_bigbox">组建谈判小组</span>
...
export default {
  ...
  methods: {
    ...
    setUpNegotiationLists () {
      ...
      reviewControl.setUpNegotiationList(url).then((res) => {
        this.tableDataExpert = res.data.BidOpenUserList
        this.tableDataExpert.map((its) => {
          if (its.initRoleId === 2 || its.initRoleId === 9) {
      ...
      this.$refs[expertForm].validate((valid) => {
        if (valid) {
          if (flag) { // 增加
            ...
            this.expertForm.initRoleId = 3
            reviewControl.setUpNegotiationAdd(this.expertForm).then((res) => {
              this.repeatDataErr(res.data.resCode)
            })
          } else { // 编辑
            reviewControl.setUpNegotiationUpdate(this.expertForm).then((res) => {
              this.repeatDataErr(res.data.resCode)
            })
          }
        } else {
    ...
    // 编辑时查看人员信息
    editBtn (scope) {
      reviewControl.setUpNegotiationLook(scope.row.objectId).then((res) => {
        this.flag = false
        this.titileName = '编辑'
    ...
    deleteBtn (scope) {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        reviewControl.setUpNegotiationDelete(scope.row.objectId).then((res) => {
          this.setUpNegotiationLists()
    ...
    subExpert () {
      ...
      reviewControl.setLeader(this.expertId, 3, 2).then((res) => {
        this.setUpNegotiationLists()
      })
    }
  },
