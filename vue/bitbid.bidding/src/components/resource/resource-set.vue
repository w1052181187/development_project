  ...
  <div>
    <div id="yzfl" class="content_bigbox">
      ...
      <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb_box">
        ...
        <el-breadcrumb-item :to="{ path: '/owner-set'}">资源分类设置</el-breadcrumb-item>
        <el-breadcrumb-item class="active_bread">{{this.$route.query.nav}}</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
...
export default{
  ...
  methods: {
    // 列表获取：
    getData () {
      this.axios.get(`classify-infos?pageNo=${this.dataIndex - 1}&pageSize=${this.pageSize}&enterpriseId=${this.$store.getters.authUser.enterpriseId}&userId=${this.$store.getters.authUser.userId}&relatedModuleType=${this.$route.query.moduleType}&_t=${new Date().getTime()}`).then((res) => {
    ...
    addClassify (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.addForm.creator = this.$store.getters.authUser.userId
          this.addForm.isDelete = 0
    ...
    handleEdit (scope) {
      ...
      this.axios.get(`classify-infos/${scope.row.objectId}?_t=${new Date().getTime()}`).then((res) => {
