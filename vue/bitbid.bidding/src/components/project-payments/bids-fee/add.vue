  ...
  <div>
    <div id="addItembid" class="content_bigbox">
      ...
      <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb_box">
        ...
        <el-breadcrumb-item :to="{path:'/projectpayments/manage/biddocumentlist',query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, costTypes:  this.$route.query.costTypes, expenseId: this.$route.query.expenseId, incomeStatus: this.$route.query.incomeStatus, expensesType: this.$route.query.expensesType}}">
          {{this.$route.query.name}}
        </el-breadcrumb-item>
        <el-breadcrumb-item class="active_bread">新增收费</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
      <!-- form表单 -->
      <el-row style="margin-top:48px">
        <el-form :model="ruleForm"  ref="ruleForm" :rules="rules"  label-width="160px">
          <el-col :span="24" v-if="this.$route.query.incomeStatus">
            <el-form-item label="收入项目" class="bitianicon" :label-width="formLabelWidth">
              <el-input v-model.trim="ruleForm.paymentUnitName" auto-complete="off"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="24" v-else>
            <el-form-item label="缴费单位：" class="bitianicon">
              <el-input class="el-inputbox" v-model="ruleForm.paymentUnitName" readonly></el-input>
              <div class="xuanzhebtn" @click="choiceBtn">选择</div>
              <el-dialog title="缴费单位" :visible.sync="dialogFormVisible" width="40%">
                <!--<div class="xinzhileft_box">-->
                  <!--<span>激活单位状态：</span>-->
                  <!--<el-select class="xxgdxz" v-model="value" placeholder="请选择">-->
                    <!--<el-option-->
                ...
                <el-form :model="Companyform" :rules="Companyrules" ref="Companyform">
                  <el-form-item label="缴费单位" prop="name" :label-width="formLabelWidth">
                    <el-input v-model.trim="Companyform.name" auto-complete="off"></el-input>
                  </el-form-item>
                </el-form>
                <div slot="footer" class="dialog-footer">
                  <el-button @click="jiaofeiQuxiao">取 消</el-button>
                  <el-button type="primary" @click="jiaofeiQueding('Companyform')">确 定</el-button>
                </div>
              </el-dialog>
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="应缴金额(元)：" prop="shouldPayment">
              <el-input v-model="ruleForm.shouldPayment"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="24">
          ...
          <el-col :span="24" v-if="this.$route.query.name === '标书费'">
            <el-form-item label="标书类型：" prop="tenderType">
              <el-select class="el-selectbox" v-model="ruleForm.tenderType" placeholder="请选择" v-if="tendertypeStayus">
                <el-option
                  v-for="item in tenderTypefrom"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
              </el-select>
              <el-select class="el-selectbox" v-model="ruleForm.tenderType" placeholder="请选择" v-else>
                <el-option
                  v-for="item in tenderTypefromShow"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value">
                </el-option>
...
export default{
  ...
  methods: {
    // 标书类型选择判断接口
    tenderTypeshow () {
      this.axios.get(`charge/checkTenderType/${this.$route.query.incExpenseIds}?_t=${new Date().getTime()}`).then((res) => {
        if (res.data.tenderFlag === 1) {
          this.tendertypeStayus = false
        }
      })
    },
    // 选择缴费单位弹框
    ...
    // 提交
    jiaofeiQueding (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.ruleForm.paymentUnitName = this.Companyform.name
          this.dialogFormVisible = false
          this.Companyform = {}
        } else {
          return false
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (this.ruleForm.paymentUnitName === '') {
              ...
              this.axios.post('charge', this.ruleForm).then((res) => {
                this.$router.push({path: '/projectpayments/manage/biddocumentlist', query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, costTypes: this.$route.query.costTypes, expenseId: this.$route.query.expenseId, incomeStatus: this.$route.query.incomeStatus, expensesType: this.$route.query.expensesType}})
              })
            }
          } else {
            return false
    ...
    sectioninformationList () {
      let url = null
      if (this.searchIs) {
        if (this.searchSel == 'sectionName') {
          url = `section-information/listPage?pageNo=${this.pageNo}&pageSize=${this.pageSize}&incExpenseId=${this.$route.query.incExpenseIds}&sectionName=${encodeURIComponent(this.roleseacher)}&_t=${new Date().getTime()}`
        } else if (this.searchSel == 'sectionNumber') {
          url = `section-information/listPage?pageNo=${this.pageNo}&pageSize=${this.pageSize}&incExpenseId=${this.$route.query.incExpenseIds}&sectionNumber=${encodeURIComponent(this.roleseacher)}&_t=${new Date().getTime()}`
        }
      } else {
        url = `section-information/listPage?pageNo=${this.pageNo}&pageSize=${this.pageSize}&incExpenseId=${this.$route.query.incExpenseIds}&_t=${new Date().getTime()}`
      }
    ...
    handleCurrentChange (nowNum) {
      ...
      this.sectioninformationList(this.searchIs)
    }
  },
  mounted () {
    this.tenderTypeshow()
