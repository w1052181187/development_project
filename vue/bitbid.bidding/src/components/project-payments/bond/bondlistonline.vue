...
export default{
  ...
  methods: {
    ...
    lookBtn (scope) {
      this.$router.push({path: `/projectpayments/manage/bondlist/detail/${scope.row.objectId}`, query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, bitbidObjectId: this.$route.query.bitbidObjectId, costTypes: this.$route.query.costTypes, objectIds: scope.row.objectId, expenseId: this.$route.query.expenseId, expensesType: this.$route.query.expensesType, paymentProjress: scope.row.paymentProjress}})
    },
    returnOne (scope) {
