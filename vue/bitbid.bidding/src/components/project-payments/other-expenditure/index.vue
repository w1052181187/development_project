      ...
      <div class="da_xz_ckbigbox">
        <el-row>
          <el-col :span="12"  v-if="$store.getters.permissions.includes('/projectpayments/charge/add')">
            <el-button icon="el-icon-plus" size='medium' type="primary" class="gongneng_btn" @click="addCharge">新增支出</el-button>
          </el-col>
          <div class="seacher_box">
            <el-input placeholder="支出项目关键字" v-model="deptseainput"></el-input>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small" @click="editBtn(scope)" v-if="$store.getters.permissions.includes('/projectpayments/charge/update')">
              修改
            </el-button>
            <el-button
              type="text"
            ...
            <el-button
              type="text"
              size="small" @click="deleteObject(scope)" v-if="$store.getters.permissions.includes('/projectpayments/charge/delete')">
              删除
            </el-button>
          </template>
        </el-table-column>
...
export default{
  ...
  methods: {
    ...
    addCharge () {
      this.$router.push({path: '/projectpayments/manage/other-expenditure/add', query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, costTypes: this.$route.query.costTypes, expenseId: this.$route.query.expenseId, expensesType: this.$route.query.expensesType}})
    },
    editBtn (scope) {
      this.$router.push({path: '/projectpayments/manage/other-expenditure/update', query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, costTypes: this.$route.query.costTypes, objectIds: scope.row.objectId, expenseId: this.$route.query.expenseId, expensesType: this.$route.query.expensesType}})
    },
    lookBtn (scope) {
      this.$router.push({path: `/projectpayments/manage/other-expenditure/detail/${scope.row.objectId}`, query: {name: this.$route.query.name, incExpenseIds: this.$route.query.incExpenseIds, costTypes: this.$route.query.costTypes, objectIds: scope.row.objectId, expenseId: this.$route.query.expenseId, expensesType: this.$route.query.expensesType}})
    },
    // 删除
    deleteObject (scope) {
