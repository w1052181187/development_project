<template>
  <div>
    <!--内容开始-->
    <div class="content_bigbox" id="zztcontentEditss">
      <!--面包屑-->
      <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb_box">
        <el-breadcrumb-item :to="{ path: '/index' }">首页</el-breadcrumb-item>
        <el-breadcrumb-item :to="{ path: '/owner' }">业主管理</el-breadcrumb-item>
        <el-breadcrumb-item class="active_bread">编辑业主</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
      <!--新增归档信息内容-->
      <div class="ckdan_contentbox">
        <el-row type="flex" class="row-bg" style="overflow: hidden">
          <el-form :model="form" :rules="rules" ref="form">
            ...
            <el-col :span="12">
              <el-form-item label="法人姓名：" :label-width="formLabelWidth" :rules="[
                { required: false, message: '', trigger: 'blur' },
                {min: 1, max: 100, message: '请输入1~100个字符',trigger: ['blur', 'change'] }
              ]">
        ...
        <el-row class="gongneng_btnbox">
          <el-button class="gongneng_btn" type="primary" @click="editOwner('form')">
            保存
          </el-button>
...
export default {
  ...
  methods: {
    // 加载数据
    initDatas () {
      this.axios.get(`owner-infos/${this.$route.query.objectId}?_t=${new Date().getTime()}`).then((res) => {
        this.form = res.data.ownerInfo
        let provincename = res.data.ownerInfo.provinceId
        let cityname = res.data.ownerInfo.cityId
        let countyname = res.data.ownerInfo.countyId
        this.oldenterpriseName = res.data.ownerInfo.enterpriseName
    ...
    editOwner (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.oldenterpriseName === this.form.enterpriseName) {
            this.axios.put('owner-infos', this.form).then((res) => {
