...
<div>
  ...
  <div class="content_bigbox" id="zhangcontent_bigbox">
    ...
    <div class="da_xz_ckbigbox" style="border: none">
        <el-button type="text" class="da_xzgd_btn"  @click="dialogTableVisible = true" v-if="$store.getters.permissions.includes('/owners/projects/add')">
         ...
         <el-table-column
           ...
           <template slot-scope="scope">
             <el-button
               type="text"
               size="small"
               @click="deleteProject(scope)" v-if="$store.getters.permissions.includes('/owners/delete') && scope.row.projectNature === 0">
               删除
             </el-button>
           </template>
         </el-table-column>
  ...
  <div id="zhangxjwdfather">
    <el-dialog title="增加招标项目" :visible.sync="dialogTableVisible" width="60%" :before-close="closeDialog">
      <!--线下项目数据-->
        <el-form :model="projectform" class="formsbox" :rules="rules" ref="projectform" @submit.native.prevent>
          ...
          <el-form-item label="项目经理：" prop="projectManager"  :label-width="formLabelWidth">
            <el-select class="zzt_zbfs_selectbox"  v-model="projectform.projectManager" placeholder="请选择">
              <el-option
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="closeBtn('projectform')">取 消</el-button>
...
export default {
  ...
  methods: {
    // 列表展示
    projectCon () {
      this.axios.get(`owner-projects?pageNo=${this.pageNo}&pageSize=${this.pageSize}&ownerCode=${this.$route.query.code}&_t=${new Date().getTime()}`).then((res) => {
        console.log(res)
    ...
    deptManagerhebibi () {
      ...
      useradmin.deptManagerlistbibi(this.$store.getters.authUser.userId).then((res) => {
        bibimanagerObj.value = res.data.user.id
        bibimanagerObj.label = res.data.user.nameZH
