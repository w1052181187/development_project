...
<div>
  ...
  <div class="content_bigbox" id="zhaotingcontent_bigbox">
    ...
    <div class="da_xz_ckbigbox" style="border: none">
        <el-button type="text" class="da_xzgd_btn" @click="dialogTableVisible = true" v-if="$store.getters.permissions.includes('/experts/projects/add')">
         ...
         <el-table-column
           ...
           <template slot-scope="scope">
             <el-button
               type="text"
               size="small"
               @click="deleteProject(scope)" v-if="$store.getters.permissions.includes('/experts/delete') && scope.row.projectNature === 0">
               删除
             </el-button>
           </template>
         </el-table-column>
  ...
  <div id="zhaotingxjwdfather">
    <el-dialog title="增加标段" :visible.sync="dialogTableVisible" width="60%" :before-close="closeDialog">
      <!--线下项目数据-->
        <el-form :model="projectform" class="formsbox" :rules="rules" ref="projectform" @submit.native.prevent>
          ...
          <el-form-item label="招标项目编号：" prop="tenderProjectNumber" :label-width="formLabelWidth">
            <el-input class="xxxm_xinxi" v-model="projectform.tenderProjectNumber" auto-complete="off"></el-input>
          </el-form-item>
