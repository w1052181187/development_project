<template>
  <div>
    <!--内容开始-->
    <div class="content_bigbox" id="expertcontents">
      <!--面包屑-->
      <el-breadcrumb separator-class="el-icon-arrow-right" class="breadcrumb_box">
        <el-breadcrumb-item :to="{ path: '/index' }">首页</el-breadcrumb-item>
        <el-breadcrumb-item :to="{ path: '/expert' }">专家管理</el-breadcrumb-item>
        <el-breadcrumb-item class="active_bread">编辑专家</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
      <!--新增归档信息内容-->
            ...
            <el-col :span="24">
              ...
              <el-col :span="12">
                <div class="add-select-btn">
                  <el-button type="primary" icon="el-icon-plus" @click="addSelectClick">新增专业</el-button>
                </div>
              </el-col>
            </el-col>
            <el-col :span="24">
              <el-form-item>
                <div class="edit-select-box">
                  <el-table
                    :show-header="false"
                    :data="backData"
                    style="width: 100%;background: none;padding: 0px;">
                    <el-table-column
                      prop="professional"
                      label="专业">
            ...
            <div class="bdlx_box">
              <el-col :span="24">
                <el-form-item label="职业资格：" class="bdneirong bdneirongzhiye" :label-width="formLabelWidth">
                  ...
                  <el-dialog title="职业资格" :visible.sync="dialogFormVisible" :show-close = "false">
                  ...
                  { min: 1, max: 50, message: '请输入1~50个字符', trigger: ['blur', 'change']}
                        ...
                        <el-input v-model.trim="qualificationsform.name" auto-complete="off"></el-input>
                      </el-form-item>
                      <el-form-item label="职业资格等级：" prop="grade" :label-width="formLabelWidth" :rules="[
                  { required: true, message: '职业资格等级', trigger: 'blur' },
                  { min: 1, max: 50, message: '请输入1~50个字符', trigger: ['blur', 'change']}
                ]">
                        <el-input v-model.trim="qualificationsform.grade" auto-complete="off"></el-input>
                      </el-form-item>
                    </el-form>
...
<script>
import {isvalidPhone, isvalidId, Zipcode, wholeNumbertype} from '../../assets/js/validate'
...
export default {
  data () {
    ...
    // 数字
    var wholeNumbertypes = (rule, value, callback) => {
      if (value === '' || value === null || value === undefined) {
        callback()
      } else if (!wholeNumbertype(value)) {
        callback(new Error('请输入1~2位整数'))
      } else {
        callback()
      }
    }
    return {
      ...
      rules: {
        ...
        workAge: [
          { required: false, message: '', trigger: 'blur' },
          {min: 1, max: 2, message: '请输入1~2位整数', trigger: ['blur', 'change']},
          { validator: wholeNumbertypes, trigger: ['blur', 'change'] }
        ]
      },
      zhiyerules: {
        name: [
  ...
  methods: {
    ...
    _getFormInfo () {
      this.axios.get(`expert-infos/${this.$route.query.objectId}`).then((res) => {
        this.form = res.data.expert
        this.dynamicTags = this.form.professionalQualifications
        // console.log(res.data.expert.expertClassifyStandard)
        let arr = this.form.expertClassifyStandard.split(';')
        // 表单-专业
        this.selectedOptions = arr[arr.length - 1].split(',')
        // 表格-专业
        this.selectTableData = arr.filter((item, index) => {
        ...
        this.selectTableData = this.selectTableData.map((item) => {
          let obj = {}
          obj.professional = item.split(',')
          return obj
        })
        var provincename = res.data.expert.provinceId
        var cityname = res.data.expert.cityId
        var countyname = res.data.expert.countyId
        this.selectedcityOptions = [provincename, cityname, countyname]
      })
    },
    addExpert (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          // this.form.expertClassifyStandard = this.form.expertClassifyStandard.toString()
          // delete this.form.expertClassifyStandards
          this.saveClassify()
          this.axios.put('expert-infos', this.form).then((res) => {
    ...
    saveClassify () {
      if (this.selectTableData.length) {
        ...
        for (let i = 0; i < this.selectTableData.length; i++) {
          let str = this.selectTableData[i].professional.join(',')
          langStr += `${str};`
          console.log(langStr)
        }
        // 合并数据
        console.log(this.selectedOptions.toString())
        this.form.expertClassifyStandard = `${langStr}${this.selectedOptions.toString()}`
      }
    },
    /** 校验是否存在重复的专业 */
    checkClassify () {
