              ...
              <el-col :span="24">
                <el-form-item label="职业资格：" class="bdneirong bdneirongzhiye" :label-width="formLabelWidth">
                  ...
                  <el-dialog title="职业资格" :visible.sync="dialogFormVisible" :show-close = "false">
                    <el-form :model="qualificationsform" :rules="zhiyerules" ref="qualificationsform" :show-close = "false">
...
export default {
  ...
  methods: {
    ...
    // 合并表格数据
    saveClassify () {
      // this.form.expertClassifyStandard = this.selectTableData.map(item => { return item })
    ...
    checkClassify () {
      ...
      if (this.selectTableData.length) {
        let expertClassesList = this.form.expertClassifyStandard.split(';')
        let newList = new Set()
        for (let i = 0; i < expertClassesList.length; i++) {
          let expertClassList = expertClassesList[i].split(',')
          newList.add(expertClassList[3])
        }
        if (newList.size < expertClassesList.length) {
