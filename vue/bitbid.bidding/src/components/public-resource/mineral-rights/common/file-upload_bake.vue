<template>
    <div class="upload">
      <div class="upload" v-if="IEVersion === 1">
        <input type="file"  class="file" name="file"  id="produceFile"/>
        <span class="upload_btn">上传文件</span>
        <span class="fileNames" style="margin-left: 20px;margin-right: 20px" v-if="ieFileObject.name !== ''">
          {{ieFileObject.name}}
        </span>
        <span class="fileNames" style="margin-left: 20px;margin-right: 20px" v-else></span>
      </div>
      <el-upload
        class="upload-demo"
        ref="upload"
...
export default {
  ...
  mounted () {
    let that = this
    $('#produceFile').on('change', function (e) {
      console.log('#produceFile : ' + JSON.stringify($('#produceFile')))
      // 上传时先把原有的删除掉
      alert(0)
      var iedel = []
      iedel.push(that.ieFileObject)
      that.$emit('ieDelectFile', iedel)
      alert(1)
      // 开始从新创建新的from
      var $form = $('<form></form>')
      ...
      $form.ajaxSubmit({
        ...
        success: function (data) {
          ...
          if (result.resCode === '0000') {
            $('.fileNames').html(data.fileInformation.fileName)
            var arryFile = []
            data.fileInformation.businessType = that.fileType
            arryFile.push(data.fileInformation)
            console.log(arryFile)
            that.$emit('upLoadFile', arryFile)
          }
        },
        error: function (a, b) {
          console.log('a : ' + a)
