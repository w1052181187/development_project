  ...
  methods: {
    ...
    lastStep () {
      if (this.$route.query.bitbidObjectId === null) {
        this.$router.push({path: '/archives/add', query: {objectId: this.$route.query.objectId, btn: true, flangshow: true}})
      } else {
        this.$router.push({path: '/archives/onlineadd', query: {objectId: this.$route.query.bitbidObjectId, btn: true, flangshow: true}})
      }
    },
    // 取消
    cancelClick () {
    ...
    submitFun () {
      var flag = this.isAlert()
      if (flag) {
        this.$confirm('提交后即归档，不可再编辑，是否确定提交?', '提示', {
