...
export default {
  ...
  methods: {
    ...
    submitForm (sectionForm) {
      ...
      this.salesConfirmation.affirmTime = new Date().getFullYear() + '-' + (new Date().getMonth() + 1) + '-' + new Date().getDate() + ' ' + new Date().getHours() + ':' + new Date().getMinutes() + ':' + new Date().getSeconds()
      salesCon.salesConUpdate(this.salesConfirmation).then((res) => {
        this.$router.push({path: '/mr/assignee/business-confirmation/receive-sales-confirmation?roledsType=2'})
      })
    },
    // 富文本
    setContent () {
