<template>
  <div id="downloadclarindexbox"  class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>下载澄清/修改文件</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
...
export default {
  ...
  methods: {
    ...
    downloadBtn (scope) {
      this.$router.push({path: '/mr/assignee/transfer-documents/download-clarify-file/download', query: {clarifyFileSectionid: scope.row.clarifyFileSectionid, roledsType: 2}})
    },
    // 列表显示
    queryList () {
      let url = null
      ...
      clarifyFile.queryList(url).then((res) => {
        let sectionLists = []
        res.data.clarifyFileList.list.forEach(function (item, index) {
          let sectionList = []
          sectionList = item.refclarifyFileList.map(function (refTransDocu) {
