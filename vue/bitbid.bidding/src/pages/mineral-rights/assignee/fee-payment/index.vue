...
export default {
  ...
  methods: {
    ...
    // 详情
    detailBtn (scope) {
      this.$router.push({path: '/mr/assignee/fee-payment/detail', query: {objectId: scope.row.objectId, sectionId: scope.row.sectionId, roledsType: 2}})
    },
    // 查看回执
    lookReceipt (scope) {
      this.$router.push({path: '/mr/assignee/fee-payment/look-receipt', query: {objectId: scope.row.objectId, sectionId: scope.row.sectionId, roledsType: 2}})
    },
    // 支付
    paymentBtn (scope) {
      ...
      this.$router.push({path: '/mr/assignee/fee-payment/payment', query: {objectId: scope.row.objectId, bidBond: scope.row.section.bidBond, times: times, roledsType: 2}})
    },
    // 根据标段id查询所有报名信息
    queryList () {
      let url = null
