...
export default {
  data () {
    return {
      ...
      // 支付方式自定义
      paymentStatus: true,
      imgurl: '../../../../../static/images/mineral/bank/', // 银行图片地址
      // 银行
      bankRadioAggregate: [
        {
          label: 1,
  ...
  methods: {
    ...
    list () {
      apply.applyDetail(this.$route.query.objectId).then((res) => {
        ...
        this.strikePrice = res.data.Apply.section.salesConfirmation.strikePrice
        this.createDate = res.data.Apply.createDate
      })
    },
    cancelBtn () {
      this.$router.push({path: '/mr/assignee/fee-payment?roledsType=2'})
    }
  },
  mounted () {
    this.list()
