          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <div class="statusbigbox" v-if="scope.row.auctionStatus !== 5 && scope.row.auctionStatus !== 8">
...
export default {
  ...
  methods: {
    ...
    // 进入竞价室
    biddingBtn (scope) {
      this.$router.push({path: '/mr/assignee/quotation/check-listed-bids/bidding-room',
        query: {
          quaConObjectId: scope.row.quaConObjectId,
          roledsType: 2
        }
      })
    },
    ...
    listAfter () {
      this.auctiontableData.forEach(item => {
        let auctionHandler = factory.create({
          ...
          userJob: factory.AuctionType['bidder'],
          isApply: item.isApply,
          section: item
        })
        item.hangStartTime = auctionHandler.getHangStartTime()
        item.hangEndTime = auctionHandler.getHangEndTime()
        item.limitStartTime = auctionHandler.getLimitStartTime()
