        ...
        <template>
          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <!--<div class="statusbigbox" v-if="scope.row.pricestatus === 0">
                  <el-button type="text" size="small" @click="createbiddingBtn(scope)">
                    进入竞价室
                  </el-button>
                </div>-->
                <div class="statusbigbox" v-if="scope.row.pricestatus === 1">
...
export default {
  ...
  methods: {
    ...
    // 查看
    detailBtn (scope) {
      this.$router.push({path: '/mr/transferor/quotation/check-time-limit-bids/detail', query: {name: scope.row.Selltype, roledsType: 1}})
    },
    // 进入竞价室
    biddingBtn (scope) {
      this.$router.push({path: '/mr/transferor/quotation/check-time-limit-bids/bidding-room', query: {status: scope.row.Status, roledsType: 1}})
    },
    // 创建竞价室
    createbiddingBtn (scope) {
      this.$router.push({path: '/mr/transferor/quotation/check-time-limit-bids/create-bidding-room', query: {status: scope.row.Status, roledsType: 1}})
    },
    list () {
      let params = null
      params = {
