<template>
    <div id="otherprospectupdatebox"  class="maincontent">
      <div class="headertitle">
        ...
        <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item>修改公示</el-breadcrumb-item>
        </el-breadcrumb>
        <!--面包屑-->
      </div>
...
import commonOtherFileUpload from '@/components/public-resource/mineral-rights/common/file-other-upload.vue'
  ...
  methods: {
    ...
    contentFrom () {
      otherProject.queryOne(this.$route.query.objectId).then((res) => {
        ...
        this.otherPublicityForm.region = Number(res.data.selectotherPublicity.region)
        // 三级联动赋值
        ...
        if (res.data.selectotherPublicity.fileInformations) {
          this.oldFileName = res.data.selectotherPublicity.fileInformations.fileName
          this.originalId = res.data.selectotherPublicity.fileInformations.objectId
        }
        // 附件
        if (res.data.selectotherPublicity.fileInformation) {
          this.oldOtherFileName = res.data.selectotherPublicity.fileInformation.fileName
          this.originalOtherId = res.data.selectotherPublicity.fileInformation.objectId
        }
      })
    },
    // 批准文件
    ...
    submitForm (otherPublicityForm, flag) {
      ...
      this.$refs[otherPublicityForm].validate((valid) => {
        if (valid) {
          otherProject.update(this.otherPublicityForm).then((res) => {
            let allFile = null
            let deleteId = null
            if (that.fileArry.length !== 0 || that.fileOtherArry.length !== 0) {
              if (that.fileOtherArry.length === 0 && that.fileArry !== 0) { // 修改批准文件时
                that.fileArry[0].relatedCode = that.otherPublicityForm.code
                allFile = [...that.fileArry]
                deleteId = this.originalId
              } else if (that.fileOtherArry.length !== 0 && that.fileArry.length === 0) { // 修改附件时
                that.fileOtherArry[0].relatedCode = that.otherPublicityForm.code
