  ...
  <div id="subaddindexbox"  class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/mr/transferor/submission/subminssion-notice/selection-project?roledsType=1' }">选择项目</el-breadcrumb-item>
...
<script>
import {anno, prProject, fileInfo} from '@/api'
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        saleDataRange: [
          {required: true, message: '拍卖开始时间不能为空', trigger: ['blur', 'change']}
        ]
      },
      // 文件上传保存的数组
      fileArry: [],
  ...
  methods: {
    ...
    list () {
      let url = {
        ...
        includeIds: this.$route.query.idArry.toString()
      }
      prProject.queryList(url).then((res) => {
        this.ruleForm.sections = res.data.sectionPageInfo.list
        // 三级联动赋值
        res.data.sectionPageInfo.list.map((ite) => {
          ite.allAddress = ''
