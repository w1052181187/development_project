<template>
  <div id="supmissionindexbox"  class="maincontent">
      ...
      <template>
        ...
        <el-table
          ...
          <el-table-column
            ...
            show-overflow-tooltip>
            <template slot-scope="scope">
              <span class="jfbdbox" v-for="(item,index) in scope.row.suppSections" :key="index">
...
export default {
  ...
  methods: {
    ...
    // 添加
    addBtn () {
      this.$router.push({path: '/mr/transferor/submission/supplement-notice/selection-project'})
    },
    // 编辑
    updateBtn (scope) {
      this.$router.push({path: '/mr/transferor/submission/supplement-notice/update', query: {landUse: scope.row.landUse, ObjectId: scope.row.objectId, roledsType: 1}})
    },
    // 查看
    lookBtn (scope) {
    ...
    // 发布
    releaseBtn (scope) {
      this.$router.push({path: '/mr/transferor/submission/supplement-notice/release', query: {ObjectId: scope.row.objectId, landUse: scope.row.landUse, roledsType: 1}})
    }
  },
  mounted () {
    this.list()
