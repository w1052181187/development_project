...
export default {
  ...
  methods: {
    ...
    // 退款
    refundBtn (scope) {
      this.$router.push({path: '/mr/transferor/post-bid-business/margin-return/refund', query: {objectId: scope.row.objectId, sectionId: scope.row.section.objectId, assigneeId: scope.row.assigneeId, roledsType: 1}})
    },
    // 审批
    approveBtn (scope) {
      this.$router.push({path: '/mr/transferor/post-bid-business/margin-return/approve', query: {objectId: scope.row.refundedSecurity.objectId, payMethod: scope.row.payMethod, roledsType: 1}})
    },
    // 查看
    lookeBtn (scope) {
      this.$router.push({path: `/mr/transferor/post-bid-business/margin-return/detail/${scope.row.refundedSecurity.objectId}`, query: {payMethod: scope.row.payMethod, roledsType: 1}})
    }
  },
  mounted () {
    this.list()
