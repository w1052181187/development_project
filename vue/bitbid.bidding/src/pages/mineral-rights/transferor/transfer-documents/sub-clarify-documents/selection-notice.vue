<template>
  <div id="clarselmissionindexbox"  class="maincontent">
...
<script>
import $ from 'jquery'
import {anno, clarifyFile} from '@/api'
export default {
  data () {
    return {
      seacher_input: '',
  ...
  methods: {
    ...
    addFile (scope) {
      ...
      clarifyFile.statusCount(url).then((res) => {
        this.addStatus = res.data.status
        if (this.addStatus) {
          this.$message({
            type: 'warning',
            message: '没有要添加澄清文件的标段'
          })
          return false
        } else {
          this.$router.push({path: '/mr/transferor/transfer-documents/sub-clarify-documents/add', query: {objectId: scope.row.objectId, roledsType: 1}})
        }
      })
    },
    queryList () {
