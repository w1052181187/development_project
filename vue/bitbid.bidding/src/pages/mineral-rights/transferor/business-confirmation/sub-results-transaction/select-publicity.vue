...
<script>
import {anno, transResAnno} from '@/api'
import $ from 'jquery'
export default {
  data () {
    return {
  ...
  methods: {
    ...
    addBtn (scope) {
      ...
      transResAnno.selectRelation(url).then((res) => {
        ...
        this.$router.push({path: '/mr/transferor/business-confirmation/sub-results-transaction/add', query: {objectId: scope.row.objectId, code: scope.row.code, roledsType: 1}})
      })
    },
    queryList () {
      let url = null
