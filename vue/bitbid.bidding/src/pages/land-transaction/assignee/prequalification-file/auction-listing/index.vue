  ...
  <div id="land_pro_patchReg" class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>下载资格预审文件</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
    <div class="contentbigbox">
      <template>
        ...
        <el-table
          ...
          <el-table-column
            prop="landAnno.preDocStartTime"
            label="资格预审文件获取开始时间"
            align="left"
            show-overflow-tooltip>
          </el-table-column>
          <el-table-column
            prop="landAnno.preDocEndTime"
            label="资格预审文件获取截止时间"
            align="left"
            show-overflow-tooltip>
          </el-table-column>
  ...
  methods: {
    ...
    list () {
      ...
      landPreDoc.queryList(url).then((res) => {
        ...
        let sectionLists = []
        res.data.landPreDocPageInfo.list.forEach(function (item) {
          let sectionList = []
    ...
    // -----------------------------------------查看----------------------------------------------
    downBtn (scope) {
      this.$router.push({path: `/lt/assignee/prequalification-file/auction-listing/detail/${scope.row.objectId}`, query: {roledsType: 2, isClarify: scope.row.isClarify, transDocSectionid: scope.row.transDocSectionid}})
    }
  },
  mounted () {
    this.list()
