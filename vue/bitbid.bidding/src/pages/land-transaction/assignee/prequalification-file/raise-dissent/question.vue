...
export default {
  ...
  methods: {
    ...
    addBtn () {
      this.$router.push({path: '/lt/assignee/prequalification-file/raise-dissent/add', query: {roledsType: 2, sectionId: this.$route.query.objectId}})
    },
    detailBtn (scope) {
      this.$router.push({path: `/lt/assignee/prequalification-file/raise-dissent/detail/${scope.row.objectId}`, query: {roledsType: 2}})
    }
  },
  mounted () {
    this.list()
