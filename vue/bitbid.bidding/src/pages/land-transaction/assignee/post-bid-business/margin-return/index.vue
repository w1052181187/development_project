...
export default {
  ...
  methods: {
    ...
    // 申请
    refundBtn (scope) {
      this.$router.push({path: '/lt/assignee/post-bid-business/margin-return/refund', query: {objectId: scope.row.objectId, sectionId: scope.row.landInformation.objectId, assigneeId: scope.row.assigneeId, roledsType: 2}})
    },
    // 查看
    lookeBtn (scope) {
      this.$router.push({path: `/lt/assignee/post-bid-business/margin-return/detail/${scope.row.refundedSecurity.objectId}`, query: {payMethod: scope.row.payMethod, roledsType: 2}})
    },
    // 修改
    updateBtn (scope) {
      this.$router.push({path: '/lt/assignee/post-bid-business/margin-return/update', query: {objectId: scope.row.refundedSecurity.objectId, payMethod: scope.row.payMethod, roledsType: 2}})
    }
  },
  mounted () {
    this.list()
