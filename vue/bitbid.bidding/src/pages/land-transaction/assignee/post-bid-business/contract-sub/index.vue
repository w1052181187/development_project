...
export default {
  ...
  methods: {
    ...
    list () {
      ...
      contract.queryAllData(url).then((res) => {
    ...
    // 提交
    subBtn (scope) {
      this.$router.push({path: '/lt/assignee/post-bid-business/submit', query: {objectId: scope.row.objectId, roledsType: 2}})
    },
    // 修改
    updateBtn (scope) {
      this.$router.push({path: '/lt/assignee/post-bid-business/update', query: {sectionId: scope.row.landInformation.objectId, objectId: scope.row.objectId, roledsType: 2}})
    },
    // 查看
    detailBtn (scope) {
      this.$router.push({path: `/lt/assignee/post-bid-business/detail/${scope.row.objectId}`, query: {roledsType: 2}})
    }
  },
  mounted () {
    this.list()
