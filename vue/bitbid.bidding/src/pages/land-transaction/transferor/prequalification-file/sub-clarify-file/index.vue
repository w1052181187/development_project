...
export default {
  ...
  methods: {
    ...
    addBtn () {
      this.$router.push({path: '/lt/transferor/prequalification-file/sub-clarify-file/select-announ', query: {roledsType: 1}})
    },
    updateBtn (scope) {
      this.$router.push({path: '/lt/transferor/prequalification-file/sub-clarify-file/update', query: {roledsType: 1, objectId: scope.row.objectId, annoId: scope.row.annoId}})
    },
    detailBtn (scope) {
      this.$router.push({path: `/lt/transferor/prequalification-file/sub-clarify-file/detail/${scope.row.objectId}`, query: {roledsType: 1}})
    },
    deleteBtn (scope) {
      landClarifyFile.delete(scope.row.objectId).then((res) => {
        this.list()
      })
    },
    releaseBtn (scope) {
      this.$router.push({path: `/lt/transferor/prequalification-file/sub-clarify-file/release/${scope.row.objectId}`, query: {roledsType: 1}})
    }
  },
  mounted () {
    this.list()
