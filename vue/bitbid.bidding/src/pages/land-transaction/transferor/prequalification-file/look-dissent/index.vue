  ...
  <div id="issuemissionindexbox"  class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>查看异议</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
...
<script>
import {landProject, question} from '@/api'
export default {
  data () {
    return {
      seacherInput: '',
  ...
  methods: {
    ...
    list () {
      ...
      landProject.queryListForQues(url).then(res => {
        ...
        this.projecttableData.map((item) => {
          let acceptUrl = {
            isDelete: 0,
            sectionId: item.objectId,
            isAccept: 1
          }
          question.count(acceptUrl).then(res1 => {
            console.log(res1)
            item.acceptCount = res1.data.count
          })
          let questionUrl = {
            isDelete: 0,
            sectionId: item.objectId
          }
          question.count(questionUrl).then(res1 => {
            item.questionCount = res1.data.count
          })
        })
    ...
    questionBtn (scope) {
      this.$router.push({path: '/lt/transferor/prequalification-file/look-dissent/dissent', query: {roledsType: 1, objectId: scope.row.objectId}})
    }
  },
  mounted () {
    this.list()
