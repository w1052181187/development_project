  ...
  <div id="issuemissionindexbox"  class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>提交资格预审文件</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
  ...
  methods: {
    ...
    list () {
      ...
      landPreDoc.queryList(url).then((res) => {
        console.log(res)
        this.projecttableData = res.data.landPreDocPageInfo.list
    ...
    addBtn () {
      this.$router.push({path: '/lt/transferor/prequalification-file/sub-tender-file/select-announ', query: { roledsType: 1 }})
    },
    updateBtn (scope) {
      this.$router.push({path: '/lt/transferor/prequalification-file/sub-tender-file/update', query: {roledsType: 1, objectId: scope.row.objectId, annoId: scope.row.annoId}})
    },
    detailBtn (scope) {
      this.$router.push({path: `/lt/transferor/prequalification-file/sub-tender-file/detail/${scope.row.objectId}`, query: {roledsType: 1, objectId: scope.row.objectId}})
    },
    releaseBtn (scope) {
      this.$router.push({path: `/lt/transferor/prequalification-file/sub-tender-file/release/${scope.row.objectId}`, query: {roledsType: 1, objectId: scope.row.objectId}})
    },
    deleteBtn (scope) {
      this.$confirm('确认删除吗?', '提示', {
        ...
        landPreDoc.delete(scope.row.objectId).then((res) => {
          this.list()
        })
      }).catch(() => {
        return false
