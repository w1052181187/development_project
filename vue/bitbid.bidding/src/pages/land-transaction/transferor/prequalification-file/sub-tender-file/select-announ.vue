...
<script>
import $ from 'jquery'
import { landAnno, landPreDoc } from '@/api'
export default {
  data () {
    return {
  ...
  methods: {
    ...
    addFileBtn (scope) {
      ...
      console.log(scope.row.objectId)
      landPreDoc.selectStatus(url).then((res) => {
        this.addStatus = res.data.status
        if (this.addStatus === false) {
          this.$message({
            type: 'warning',
            message: '没有要添加资格预审文件的标段'
          })
          return false
        } else {
          this.$router.push({
            path: '/lt/transferor/prequalification-file/sub-tender-file/add',
            query: {roledsType: 1, annoId: scope.row.objectId, code: scope.row.code}
          })
        }
      })
    },
