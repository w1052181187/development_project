          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span class="jfbdbox" v-for="(item,index) in scope.row.refTransDocuList" :key="index">
                <span class="xmbh_box" :title="item.sectionNumber">{{item.landInformation.sectionNumber}}</span>
              </span>
            </template>
          </el-table-column>
          <el-table-column
  ...
  methods: {
    ...
    list () {
      ...
      tdocuments.queryLandList(url).then((res) => {
        console.log(res)
        this.projecttableData = res.data.transferDocumentsList.list
    ...
    // -----------------------------------------添加----------------------------------------------
    addBtn () {
      this.$router.push({path: '/lt/transferor/auction-listed-file/selection-announ?roledsType=1&showStatus=false'})
    },
    // -----------------------------------------修改 变更----------------------------------------------
    updateBtn (scope) {
      this.$router.push({path: '/lt/transferor/auction-listed-file/sub-auc-listed/update', query: {objectId: scope.row.objectId, annoId: scope.row.annoId, roledsType: 1, showStatus: false}})
    },
    // -----------------------------------------查看----------------------------------------------
    detailBtn (scope) {
      this.$router.push({path: `/lt/transferor/auction-listed-file/sub-auc-listed/detail/${scope.row.objectId}`, query: {annoId: scope.row.annoId, roledsType: 1, showStatus: false}})
    },
    // -----------------------------------------发布----------------------------------------------
    releaseBtn (scope) {
      this.$router.push({path: `/lt/transferor/auction-listed-file/sub-auc-listed/release/${scope.row.objectId}`, query: {annoId: scope.row.annoId, roledsType: 1, showStatus: false}})
    },
    // -----------------------------------------删除----------------------------------------------
    deleteBtn (scope) {
    ...
    // -----------------------------------------澄清修改----------------------------------------------
    clarifyBtn (scope) {
      this.$router.push({path: `/lt/transferor/auction-listed-file/sub-clarify?roledsType=1&showStatus=false`})
    },
    // ***************************************************************自己方法**********************************************************************
    // 搜索选择
    selectSeacher (val) {
