  ...
  <div id="addbox" class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>修改地块</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="地块编号：" prop="sectionNumber">
                  ...
                  <el-input v-model="sectionForm.objectId" type="hidden"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
  ...
  methods: {
    ...
    detail () {
      landProject.detail(this.$route.query.objectId).then(res => {
        ...
        this.linkageArray.push(res.data.landInformation.provinceId)
        this.linkageArray.push(res.data.landInformation.cityId)
        this.linkageArray.push(res.data.landInformation.countyId)
        let url = null
        url = {
        ...
        fileInfo.get(url).then((res) => {
          this.oldFileName = res.data.fileInformation.fileName
          this.fileObjectId = res.data.fileInformation.objectId
        })
      })
    },
    // 将子组件获取到的数据传给父组件
      ...
      this.$refs[sectionForm].validate((valid) => {
        if (valid) {
          ...
          landProject.update(this.sectionForm).then((res) => {
            // 将附件信息入库
            if (this.fileArry.length !== 0) {
              this.fileArry[0].relatedCode = this.sectionForm.code
