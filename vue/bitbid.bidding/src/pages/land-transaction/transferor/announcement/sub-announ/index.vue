          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <div class="statusbigbox" v-if="scope.row.status === 4">
                ...
                <el-button type="text" size="small" @click="suppBtn(scope)">
                  补充公告
                </el-button>
              </div>
            </template>
...
export default {
  ...
  methods: {
    ...
    // -----------------------------------------添加----------------------------------------------
    addBtn () {
      this.$router.push({path: '/lt/transferor/announcement/sub-announ/selection-project?roledsType=1&showStatus=false'})
    },
    // -----------------------------------------修改   变更----------------------------------------------
    updateBtn (scope) {
      this.$router.push({path: '/lt/transferor/announcement/sub-announ/update', query: {roledsType: 1, showStatus: false, methodOfTransfer: scope.row.type, objectId: scope.row.objectId}})
    },
    // -----------------------------------------查看----------------------------------------------
    detailBtn (scope) {
      this.$router.push({path: `/lt/transferor/announcement/sub-announ/detail/${scope.row.objectId}`, query: {roledsType: 1, showStatus: false, methodOfTransfer: scope.row.type}})
    },
    // -----------------------------------------发布----------------------------------------------
    releaseBtn (scope) {
      this.$router.push({path: `/lt/transferor/announcement/sub-announ/release/${scope.row.objectId}`, query: {roledsType: 1, showStatus: false, methodOfTransfer: scope.row.type}})
    },
    // -----------------------------------------补充公告----------------------------------------------
    suppBtn (scope) {
