  ...
  <div id="land_pro_subAnnAdd" class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>提交补充公告</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
...
export default {
  ...
  methods: {
    ...
    list () {
      ...
      if (this.seacherInput !== '') { // 搜索后执行
        if (this.seacherName === '补充公告名称') {
          url.suppleAnnoNumber = this.seacherInput
    ...
    // -----------------------------------------添加----------------------------------------------
    addBtn () {
      this.$router.push({path: '/lt/transferor/announcement/supp-announ/selection-announ?roledsType=1&showStatus=false'})
    },
    // -----------------------------------------修改  变更----------------------------------------------
    updateBtn (scope) {
      this.$router.push({path: '/lt/transferor/announcement/supp-announ/update', query: {roledsType: 1, showStatus: false, methodOfTransfer: scope.row.type, objectId: scope.row.objectId}})
    },
    // -----------------------------------------查看----------------------------------------------
    detailBtn (scope) {
      this.$router.push({path: `/lt/transferor/announcement/supp-announ/detail/${scope.row.objectId}`, query: {roledsType: 1, showStatus: false, methodOfTransfer: scope.row.type}})
    },
    // -----------------------------------------发布----------------------------------------------
    releaseBtn (scope) {
      this.$router.push({path: `/lt/transferor/announcement/supp-announ/release/${scope.row.objectId}`, query: {roledsType: 1, showStatus: false, methodOfTransfer: scope.row.type}})
    },
    // -----------------------------------------删除----------------------------------------------
    deleteBtn (scope) {
