<template>
  <div id="land_ann_suppaddbox" class="maincontent">
    <div class="headertitle">
      <!--面包屑-->
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/lt/index?roledsType=1&showStatus=false' }">首页</el-breadcrumb-item>
        <el-breadcrumb-item>项目管理</el-breadcrumb-item>
        <el-breadcrumb-item :to="{ path: '/lt/transferor/announcement/supp-announ?roledsType=1' }">提交补充公告</el-breadcrumb-item>
      ...
      <el-form :model="sectionForm" :rules="rules" ref="sectionForm" label-width="200px" class="demo-ruleForm">
        ...
        <!--相关地块信息-->
          ...
          <div class="proinfobox" v-if="Number(this.$route.query.methodOfTransfer) === 1">
            ...
            <div class="formmian">
              <div v-for="(relevantMassif,index) in sectionForm.landInformations" :key="index" v-if="relevantMassif.isCheckedProject">
                <el-row>
            ...
            <el-row>
              <el-col :span="8">
                <el-form-item label="补充公告名称：" prop="suppleAnnoNumber">
                  <el-input v-model="sectionForm.suppleAnnoNumber" placeholder="请输入补充公告名称"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
  ...
  methods: {
    ...
    landAnnoInfo () {
      landAnno.detail(this.$route.query.objectId).then((res) => {
        ...
        this.toFormat()
        this.prjectSelect = res.data.landAnno.landInformations
        this.sectionForm.landInformations.forEach((itm) => {
          itm.isCheckedProject = true
          this.valuePrject.push(itm.objectId)
        })
      })
    },
    // ***************************************************************数据接口方法**********************************************************************
    ...
    submitForm (sectionForm, flag) {
      this.sectionForm.landInformations.forEach((item) => {
        if (item.isCheckedProject) {
          this.landInformations.push(item)
        }
      })
      this.sectionForm.landInformations = this.landInformations
      if (this.sectionForm.landInformations.length > 0) {
        this.selectProStatus = true
      } else {
        this.selectProStatus = false
      }
      ...
      this.sectionForm.type = this.$route.query.methodOfTransfer
      this.sectionForm.annoType = 4
      this.sectionForm.oldAnnoId = this.sectionForm.objectId
      this.sectionForm.objectId = null
      this.sectionForm.isSupplement = 1
      this.$refs[sectionForm].validate((valid) => {
        if (valid) {
          this.sectionForm.status = flag
          this.sectionForm.content = this.$refs.ueditor.getContent()
          this.sectionForm.landInformations.forEach((item) => {
            this.landInfoIds.push(item.objectId)
          })
          ...
          landAnno.save(this.sectionForm).then((res) => {
            ...
            this.$router.push({path: '/lt/transferor/announcement/supp-announ?roledsType=1&showStatus=false'})
          })
        } else {
          return false
        }
    ...
    // 取消
    cancelBtn () {
      this.$router.push({path: '/lt/transferor/announcement/supp-announ?roledsType=1'})
    },
    // ***************************************************************自己方法**********************************************************************
    // 关联块地信息
    selectPrject (val) {
      this.aelectArry = val
      this.sectionForm.landInformations.map((itm, index) => {
