  ...
  <div id="land_pro_subAnnAdd" class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>提交协议出让公告</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
      ...
      <template>
        ...
        <el-table
          ...
          <el-table-column
            ...
            show-overflow-tooltip>
            <template slot-scope="scope">
              <div class="statusbigbox" v-if="scope.row.annoType === 3">
                <span class="statusbox">协议出让公告</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column
...
export default {
  ...
  methods: {
    ...
    // -----------------------------------------添加----------------------------------------------
    addBtn () {
      this.$router.push({path: '/lt/transferor/sub-agree-ann/selection-project?roledsType=1&showStatus=false'})
    },
    // -----------------------------------------修改----------------------------------------------
    updateBtn (scope) {
      this.$router.push({path: '/lt/transferor/sub-agree-ann/update', query: {roledsType: 1, showStatus: false, objectId: scope.row.objectId}})
    },
    // -----------------------------------------查看----------------------------------------------
    detailBtn (scope) {
      this.$router.push({path: `/lt/transferor/sub-agree-ann/detail/${scope.row.objectId}`, query: {roledsType: 1, showStatus: false}})
    },
    // -----------------------------------------发布----------------------------------------------
    releaseBtn (scope) {
      this.$router.push({path: `/lt/transferor/sub-agree-ann/release/${scope.row.objectId}`, query: {roledsType: 1, showStatus: false}})
    },
    // -----------------------------------------删除----------------------------------------------
    deleteBtn (scope) {
