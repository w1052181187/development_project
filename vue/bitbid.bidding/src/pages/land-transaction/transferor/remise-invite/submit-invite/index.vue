<template>
  <div id="land_pro_subInviteAdd" class="maincontent">
    <div class="headertitle">
      <!--面包屑-->
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/lt/index?roledsType=1&showStatus=false' }">首页</el-breadcrumb-item>
        <el-breadcrumb-item>出让邀请</el-breadcrumb-item>
        <el-breadcrumb-item>提交出让邀请</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
      ...
      <template>
        ...
        <el-table
          ...
          <el-table-column
            ...
            show-overflow-tooltip>
            <template slot-scope="scope">
              <div class="statusbigbox" v-if="scope.row.createDate === ''">
                <span class="statusbox">-----</span>
              </div>
              <div class="statusbigbox" v-else>
                <span class="statusbox">{{scope.row.createDate}}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column
...
export default {
  ...
  methods: {
    ...
    list () {
      ...
      if (this.seacherInput !== '') {
        if (this.seacherName === '邀请书名称') {
          url.messageLike = this.seacherInput === '' ? null : this.seacherInput
        } else if (this.seacherName === '地块编号') {
          url.sectionNumber = this.seacherInput === '' ? null : this.seacherInput
        }
      }
    ...
    // -----------------------------------------添加----------------------------------------------
    addBtn () {
      this.$router.push({path: '/lt/transferor/remise-invite/submit-invite/selection-project?roledsType=1&showStatus=false'})
    },
    // -----------------------------------------修改----------------------------------------------
    updateBtn (scope) {
      this.$router.push({path: '/lt/land-transaction/transferor/remise-invite/submit-invite/update?roledsType=1&showStatus=false', query: {objectId: scope.row.objectId}})
    },
    // -----------------------------------------查看----------------------------------------------
    detailBtn (scope) {
      this.$router.push({path: `/lt/land-transaction/transferor/remise-invite/submit-invite/detail/${scope.row.objectId}`, query: {roledsType: 1, showStatus: false}})
    },
    // -----------------------------------------删除----------------------------------------------
    deleteBtn (scope) {
      landAnno.remove(scope.row.objectId).then((res) => {
        scope.row.landInformations.map((item) => {
          landProject.delRefByObjectId(item.objectId).then((resp) => {
            this.list()
          })
        })
      })
