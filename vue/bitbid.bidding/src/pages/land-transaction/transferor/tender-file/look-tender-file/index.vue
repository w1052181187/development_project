          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <div class="statusbigbox" v-if="scope.row.status === 3">
                <span class="statusbox red"><i></i>审批通过</span>
              </div>
              <div class="statusbigbox" v-if="scope.row.status === 4">
                <span class="statusbox yellow"><i></i>审批不通过</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column
...
export default {
  ...
  methods: {
    ...
    detailBtn (scope) {
      this.$router.push({path: `/lt/transferor/tender-file/look-tender-file/detail/${scope.row.objectId}`, query: {roledsType: 1}})
    },
    // 展开隐藏信息
    btnOpen (event) {
