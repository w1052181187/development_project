  ...
  <div id="issuemissionindexbox"  class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>提交招标文件</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <div class="statusbigbox" v-if="scope.row.status === 2">
                <span class="statusbox green"><i></i>已发布</span>
              </div>
              <div class="statusbigbox" v-if="scope.row.status === 3">
                <span class="statusbox yellow"><i></i>审批通过</span>
              </div>
...
export default {
  ...
  methods: {
    ...
    addBtn () {
      this.$router.push({path: '/lt/transferor/tender-file/sub-tender-file/select-announ', query: {roledsType: 1}})
    },
    updateBtn (scope) {
      this.$router.push({path: '/lt/transferor/tender-file/sub-tender-file/update', query: {roledsType: 1, objectId: scope.row.objectId, annoId: scope.row.annoId}})
    },
    detailBtn (scope) {
      this.$router.push({path: `/lt/transferor/tender-file/sub-tender-file/detail/${scope.row.objectId}`, query: {roledsType: 1}})
    },
    releaseBtn (scope) {
      this.$router.push({path: `/lt/transferor/tender-file/sub-tender-file/release/${scope.row.objectId}`, query: {roledsType: 1}})
    },
    deleteBtn (scope) {
