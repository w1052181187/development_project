  ...
  <div id="prefilelist" class="smaincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>资格预审文件备案</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
...
export default {
  ...
  methods: {
    ...
    list () {
      ...
      pushData.ysFile(url).then((res) => {
        this.projecttableData = JSON.parse(res.data.ysFileString)
    ...
    search () {
      ...
      //   searchs.name = this.seacher_input
      // }
      // pushData.ysFile(searchs).then((res) => {
      //   this.projecttableData = JSON.parse(res.data.ysFileString)
      // })
    },
    // 搜索选择
    ...
    detailBtn (scope, type) {
      this.$router.push({path: `/system-management/government-procurement/pre-qualification-documents-file/detail-or-check/${type}/${scope.row.id}`})
    },
    // 查看下载情况
    downloadBtn (item) {
      this.$router.push({path: `/system-management/government-procurement/pre-qualification-documents-file/check-download/${item.bianDuanId}`})
    }
  },
  mounted () {
    this.list()
