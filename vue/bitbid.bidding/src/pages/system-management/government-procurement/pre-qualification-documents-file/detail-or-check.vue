<template>
  <div id="prequadocfilebox" class="smaincontent">
    <div class="headertitle">
      ...
      <div class="returnboxbig" v-if="type === 'detail'">
        <router-link tag='span' to="/system-management/government-procurement/pre-qualification-documents-file">返回</router-link>
      </div>
    </div>
...
export default {
  ...
  methods: {
    detail () {
      pushData.ysFileDetail(this.$route.params.objectId).then((res) => {
        this.proForm = JSON.parse(res.data.ysFileDetail)
        this.grouptableData = this.proForm.zgysjglbdList
