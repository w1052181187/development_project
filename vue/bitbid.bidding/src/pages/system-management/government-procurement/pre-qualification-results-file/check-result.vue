  ...
  <div id="projectfileindexbox" class="smaincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>查看审查结果</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
    <div class="contentbigbox">
      <template>
        ...
        <el-table
          ...
          <el-table-column
            prop="ziGeShenChaJieGuo"
            label="是否通过"
            align="left"
            width="100">
            <template slot-scope="scope">
              <div class="statusbigbox" v-if="scope.row.ziGeShenChaJieGuo === 2">
                <span class="statusbox ">是</span>
              </div>
              <span class="statusbox " v-else>否</span>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
export default {
  ...
  methods: {
    ...
    list () {
      ...
      pushData.zgscBgResult(url).then((res) => {
        this.projecttableData = JSON.parse(res.data.zgscBgResString)
    ...
    // // 查看
    // detailBtn (scope) {
    //   this.$router.push({path: `/system-management/government-procurement/pre-qualification-results-file/detail/${scope.row.objectId}`, query: {type: scope.row.transferType}})
    // },
    // // 查看审查结果
    // checkResultBtn (scope) {
    //   this.$router.push({path: `/system-management/government-procurement/pre-qualification-results-file/check-result`})
    // }
  },
  mounted () {
    this.list()
