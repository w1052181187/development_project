  ...
  <div id="projectdetailbox" class="smaincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item :to="{ path: '/system-management/government-procurement/procurement-documents-file' }">采购文件备案</el-breadcrumb-item>
        <el-breadcrumb-item>{{type === "check" ? '审批' : '详情'}}</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
      <div class="returnboxbig" v-if="type === 'detail'">
        <router-link tag='span' to="/system-management/government-procurement/procurement-documents-file">返回</router-link>
      </div>
    </div>
    <div class="contentbigbox">
      ...
      <table class="detailtable">
        ...
        <tr v-if="isShow">
          <td>保证金：</td>
          <td colspan="3"> <span v-if="proForm.isJiaoNaTouBiaoBaoZhengjin === 1">
              是
            </span>
        ...
        <tr v-if="isShow">
          ...
          <td>响应有效期：</td>
          <td colspan="4">{{proForm.validity_of_bids}}</td>
        </tr>
        <tr v-if="isShow">
          <td>开标地点：</td>
        ...
        <tr>
          <td>备注：</td>
          <td colspan="7">{{proForm.zbwjBeiZhu}}</td>
        </tr>
...
export default {
  ...
  methods: {
    detail () {
      pushData.zbwjFileDetail(this.$route.params.objectId).then((res) => {
        this.proForm = JSON.parse(res.data.zbwjFileDetail)
        this.grouptableData = this.proForm.zbwjglbdList
