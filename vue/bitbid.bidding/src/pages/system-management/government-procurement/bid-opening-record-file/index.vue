        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div class="statusbigbox" v-if="scope.row.createTime === ''">
                <span class="statusbox ">--</span>
              </div>
              <div class="statusbigbox" v-if="scope.row.createTime !== ''">
                <span class="statusbox ">{{scope.row.createTime}}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column
...
export default {
  ...
  methods: {
    ...
    list () {
      ...
      pushData.kbLog(url).then((res) => {
        this.projecttableData = JSON.parse(res.data.kbLogList)
    ...
    // 查看
    detailBtn (scope) {
      this.$router.push({path: `/system-management/government-procurement/bid-opening-record-file/detail/${scope.row.id}`, query: {type: scope.row.transferType}})
    },
    // 确认进度查看
    confirmBtn (scope) {
      this.$router.push({path: `/system-management/government-procurement/bid-opening-record-file/check-progress/${scope.row.id}/${scope.row.biaoDuanId}`})
    }
  },
  mounted () {
    this.list()
