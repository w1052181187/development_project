  ...
  <div id="indexbox" class="smaincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>澄清/修改文件备案</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
...
export default {
  ...
  methods: {
    ...
    queryList (search) {
      ...
      sclarifyFile.queryList(url).then((res) => {
        this.clarDocuTableData = res.data.clarifyFileList.list
        this.clarDocuTableData.map((ite) => {
    ...
    search () {
      ...
      sclarifyFile.queryList(searchs).then((res) => {
        this.clarDocuTableData = res.data.clarifyFileList.list
        this.clarDocuTableData.map((ite) => {
          if (ite.announcement.releaseTime === null) {
            ite.announcement.releaseTime = '-------------'
    ...
    // 查看
    detailBtn (scope) {
      this.$router.push({path: `/system-management/mineral-rights/clarify-documents-file/detail/${scope.row.objectId}`, query: {name: scope.row.Sellmethods}})
    }
  },
  mounted () {
    this.queryList()
