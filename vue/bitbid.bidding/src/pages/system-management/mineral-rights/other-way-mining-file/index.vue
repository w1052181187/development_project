  ...
  <div id="indexbox" class="smaincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>其他方式采矿权出让公示备案</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
...
export default {
  ...
  methods: {
    ...
    // 查看
    detailBtn (scope) {
      this.$router.push({path: `/system-management/mineral-rights/other-way-mining-file/detail/${scope.row.objectId}`, query: {objectId: scope.row.objectId, name: this.$route.query.name, type: this.$route.query.type}})
    },
    // 审核
    checkBtn (scope) {
      this.$router.push({path: '/system-management/mineral-rights/other-way-mining-file/check', query: {objectId: scope.row.objectId, name: this.$route.query.name, type: this.$route.query.type}})
    }
  },
  mounted () {
    this.list()
