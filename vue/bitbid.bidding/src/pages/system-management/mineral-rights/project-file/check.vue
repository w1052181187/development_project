<template>
  <div id="detailbox" class="smaincontent">
      ...
      <el-form :model="sectionForm"  label-width="100px" class="demo-sectionForm">
        ...
        <div class="proinfobox">
          ...
          <div class="formmian"  v-if="this.$route.query.type === 2">
            ...
            <el-row>
              <el-col :span="24" class="" prop="areaCoordinates">
                <el-form-item label="区域坐标：">
                  <span>{{sectionForm.areaCoordinates}}</span>
                </el-form-item>
              </el-col>
...
<script>
import {sprProject, sApproveRecord} from '@/api'
...
export default {
  ...
  methods: {
    approveForm (val) {
      ...
      this.checkResult.approvalType = 1
    },
    // 内容获取
    contentFrom () {
      sprProject.queryOne(this.$route.query.objectId).then((res) => {
    ...
    submitForm () {
      ...
      sApproveRecord.save(this.checkResult).then((res) => {
        ...
        sprProject.update(this.sectionForm).then((res) => {
          this.$router.push({path: '/system-management/mineral-rights/project-file'})
        })
      })
    },
    // 取消
