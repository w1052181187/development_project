...
export default {
  ...
  methods: {
    ...
    list () {
      ...
      contract.queryListByTrans(url).then((res) => {
        this.projectData = res.data.contractPageInfo.list
    ...
    // -----------------------------------------查看----------------------------------------------
    detailBtn (scope) {
      this.$router.push({path: `/system-management/land-transaction/contract/detail/${scope.row.objectId}`})
    },
    // -----------------------------------------审批----------------------------------------------
    examineBtn (scope) {
      this.$router.push({path: `/system-management/land-transaction/contract/examine/${scope.row.objectId}`})
    },
    // ***************************************************************自己方法**********************************************************************
    // 搜索选择
    selectSeacher (val) {
