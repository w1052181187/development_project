  ...
  <div id="land_ann_lookdetailbox" class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>澄清/修改文件审批</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
...
import {sApproveRecord, landClarifyFile} from '@/api'
export default {
  components: {
    commonFileDownload,
    examineApprove,
  ...
  methods: {
    ...
    approveForm (val) {
      ...
      this.checkResult.approvalType = 10
    ...
    submitForm () {
      ...
      sApproveRecord.save(this.checkResult).then((res) => {
        ...
        // 注：下边放修改状态接口(修改接口)
        landClarifyFile.updateDocuStatus(this.sectionForm).then((res) => {
          this.$router.push({path: '/system-management/land-transaction/clarify/index'})
        })
      })
    },
    detail () {
      landClarifyFile.detail(this.$route.params.objectId).then((res) => {
        ...
        // }
        this.sectionForm = res.data.landClarifyFile
