  ...
  methods: {
    ...
    list () {
      ...
      if (this.seacherInput !== '') {
        ...
        } else if (this.seacherName === '邀请书名称') {
    ...
    // -----------------------------------------查看----------------------------------------------
    detailBtn (scope) {
      this.$router.push({path: `/system-management/land-transaction/remise-invite/detail/${scope.row.objectId}`})
    },
    // -----------------------------------------审批----------------------------------------------
    examineBtn (scope) {
      this.$router.push({path: `/system-management/land-transaction/remise-invite/examine/${scope.row.objectId}`})
    },
    // 搜索选择
    selectSeacher (val) {
      if (val === 1) {
