  ...
  methods: {
    ...
    list () {
      ...
      stransResAnno.queryList(url).then((res) => {
        this.projectData = res.data.transResultsAnnoList.list
    ...
    // -----------------------------------------查看----------------------------------------------
    detailBtn (scope) {
      this.$router.push({path: `/system-management/land-transaction/make-result/detail/${scope.row.objectId}`})
    },
    // -----------------------------------------审批----------------------------------------------
    examineBtn (scope) {
      this.$router.push({path: `/system-management/land-transaction/make-result/examine/${scope.row.objectId}`})
    },
    // ***************************************************************自己方法**********************************************************************
    // 搜索选择
    selectSeacher (val) {
