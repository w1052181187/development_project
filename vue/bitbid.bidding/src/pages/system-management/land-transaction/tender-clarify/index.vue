  ...
  <div id="systemlandsublookindex" class="maincontent">
    <div class="headertitle">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>招标澄清/修改文件备案</el-breadcrumb-item>
      </el-breadcrumb>
      <!--面包屑-->
    </div>
      ...
      <template>
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="操作" width="140">
            <template slot-scope="scope">
              <div class="statusbigbox" v-if="scope.row.status === 2 || scope.row.status === 3">
                <el-button type="text" size="small" @click="detailBtn(scope)">
                  查看
                </el-button>
              </div>
              <div class="statusbigbox" v-if="scope.row.status === 1 || scope.row.status === 4">
  ...
  methods: {
    ...
    list () {
      ...
      clarifyFile.queryLandList(url).then((res) => {
        console.log(res)
        this.projectData = res.data.clarifyFileList.list
    ...
    // -----------------------------------------查看----------------------------------------------
    detailBtn (scope) {
      this.$router.push({path: `/system-management/land-transaction/tender-clarify/detail/${scope.row.objectId}`})
    },
    // -----------------------------------------审批----------------------------------------------
    examineBtn (scope) {
      this.$router.push({path: `/system-management/land-transaction/tender-clarify/examine/${scope.row.objectId}`})
    },
    // ***************************************************************自己方法**********************************************************************
    // 搜索选择
    selectSeacher (val) {
