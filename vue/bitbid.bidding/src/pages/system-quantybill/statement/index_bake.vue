<template>
  <el-container id="statement" class="statement">
    <el-header>
      <div class="l_header">
        <div class="wrap">
    ...
    <el-container>
      <el-aside width="300px" class="setheight">
        <div class="toptree">
          <span>工程结构</span>
        ...
        <div class="pdflist">
          ...
          <template v-if="Number(this.$route.query.type) === 1">
            <div v-if="this.rank === '1'">
              <span v-for="(item, index) in zb" :class="[ { 'first-active': index === 0 },  { 'active': item.value === currentVal }]" :key="item.value" @click="getCurrentType(item.value, index)" v-if="item.rank === '1'" :title="item.label">{{item.label}}</span>
            </div>
      ...
      <el-main>
        <div class="maintop">
          ...
          <el-button class="lookbtn" type="primary" @click="lookDialogVisible = true"><img src="../../../../static/images/system-quantybill/ico2.png" /> 查看标书信息</el-button>
          <el-button type="primary" v-if="Number(this.$route.query.turn) === 0" @click="exportInterface"><img src="../../../../static/images/system-quantybill/jiekou.png"/> 导出招标接口</el-button>
        ...
        <div class="iframe">
          <iframe id="printIframe" :src="src" style="width: 99%;height: 100%;"></iframe>
        </div>
        <!---->
        <el-dialog
          title="生成标书"
        ...
        <el-dialog
          ...
          <div class="maincon">
            <div class="lefttree">
              <el-tree :data="allData" :props="defaultProps" ref="allDataTree" show-checkbox @check-change="handleCheckChange" default-expand-all
                       :expand-on-click-node="false"></el-tree>
            </div>
          </div>
          <span slot="footer" class="dialog-footer pl-button">
            <el-button type="primary" @click="submitAllData">导出Excel</el-button>
            <el-button type="primary" @click="submitAllData">导出PDF</el-button>
            <el-button type="primary" @click="submitAllData">打印</el-button>
          </span>
        </el-dialog>
        <!--批量弹窗-->
        <!--查看标书信息 弹窗-->
...
import { loadUrl, downloadFileMining } from '@/assets/js/common'
import $ from 'jquery'
export default {
  data () {
    return {
  ...
  methods: {
    ...
    setMain () {
      ...
      let iframeH = document.getElementsByClassName('iframe')
      // 设置主要内容区的高度
      topUl[0].style.height = (screenH - 60) + 'px'
      iframeH[0].style.height = (screenH - 154) + 'px'
    },
    // 查看标书信息读取数据
    _getBidData () {
      billPushData.getItemInfor(this.objectId).then((res) => {
        ...
        if (Number(this.$route.query.type) !== 1) {
          ...
          this.relateBidder = res.data.relateItemInfor.bidder
          this.relateBidderLegalPerson = res.data.relateItemInfor.bidderLegalPerson
    ...
    exportInterface () {
      billPushData.projectDetail(this.objectId).then((res) => {
        ...
        if (Number(this.$route.query.type) === 1) {
          relatedProjectId = this.objectId
        } else {
          relatedProjectId = res.data.project.relatedProjectId
        }
        billPushData.projectDetail(relatedProjectId).then((res) => {
          downloadFileMining(res.data.project.fileInformation.fileName, res.data.project.fileInformation.relativePath)
        })
      })
    },
    // 读取属性数据
    ...
    treeAllData () {
      ...
      billPushData.projectDetail(this.$route.query.projectId).then((res) => {
        ...
        if (Number(this.$route.query.type) === 2) {
          let childArr = res.data.project.treeModel.children
          let arr = this.getReportInfo(2, 1)
          for (let k = (arr.length - 1); k >= 0; k--) {
            arr[k].id = res.data.project.treeModel.id
            childArr.unshift(arr[k])
          }
          for (let i = (childArr.length - 1); i >= 4; i--) {
            let childLastArr = childArr[i].children
            let childarr = this.getReportInfo(2, 2)
            for (let k in childarr) {
              childarr[k].id = childArr[i].id
        ...
        if (Number(this.$route.query.type) === 3) {
          let childArr = res.data.project.treeModel.children
          let arr = this.getReportInfo(3, 1)
          for (let k = (arr.length - 1); k >= 0; k--) {
            arr[k].id = res.data.project.treeModel.id
            childArr.unshift(arr[k])
          }
    ...
    submitAllData () {
      ...
      this.transNode2Array(all, tree)
      billPushData.downloadAllExcels(tree, this.objectId).then((res) => {
        if (res.data.resCode === '0000') {
          let comurl = process.env.COMPONENTS_BASE_API
          let url = comurl + 'comptend/project/download/excels/' + this.objectId
          window.open(url)
        }
      })
    },
    transNode2Array (node, tree) {
      for (let key in node) {
        if (node[key].rank) {
          this.transNode2Array(node[key], tree)
        } else {
          if (node[key].id != null && node[key].id != '') {
            tree.push({
              id: node[key].id,
              type: node[key].type
            })
    ...
    _saveEndFile () {
      if (this.fileName) {
        let comurl = process.env.COMPONENTS_BASE_API
        let url = comurl + 'comptend/project/download/localFile/end/' + this.objectId + '?fileName=' + encodeURI(this.fileName)
        window.open(url)
      } else {
        this.$message({
          type: 'warning',
