<template>
  <div id="dynamic-info">
    ...
    <div class="contentbigbox">
      <!--搜索 & 添加按钮-->
      <el-row>
        <el-button type="success"  class="addbutton" @click="addbutton"> + 添加动态</el-button>
        <div class="anno_seacher_box">
          <span style="margin-left: 10px">标题/关键字：</span>
...
export default {
  ...
  methods: {
    list () {
      ...
      dynamicNotice.queryList(params).then((res) => {
        this.tableData = res.data.dynamicNoticePageInfo.list
        this.total = res.data.dynamicNoticePageInfo.total
      })
    },
    // 搜索
    search () {
    ...
    /** 新增 */
    addbutton () {
      this.$router.push({path: '/index/dynamic-info/add'})
    },
    /** 表格操作按钮 */
    handleTableBtn (row, type) {
      if (Object.is(type, 'del')) {
        ...
        this.$router.push({path: `/index/dynamic-info/detail/${row.objectId}`})
      } else if (Object.is(type, 'edit')) {
        this.$router.push({path: '/index/dynamic-info/update', query: {objectId: row.objectId}})
      }
    },
    /** 删除 */
    delete (row) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        // 执行删除
        dynamicNotice.remove(row.objectId).then((res) => {
          this.list()
        })
      }).catch(() => {
        this.$message({
    ...
    isEnable (type, row) {
      ...
      this.$confirm(str, '提示', {
        ...
        if (type === 'disable') {
          // 禁用
          dynamicNotice.disable(row.objectId).then((res) => {
            this.list()
          })
        } else {
          // 启用
          dynamicNotice.enabled(row.objectId).then((res) => {
            this.list()
          })
        }
      }).catch(() => {
