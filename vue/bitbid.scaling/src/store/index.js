...
import {getToken, setToken, removeToken, getProjectId, setProjectId, removeProjectId} from '@/utils/auth'
...
const getters = {
  ...
  isRefreshMenu: state => state.isRefreshMenu
}
...
const mutations = {
  ...
  SET_IS_REFRESH_MENU: (state, isRefreshMenu) => {
    state.isRefreshMenu = isRefreshMenu
  }
}
  ...
  GetLoginInfo: ({commit, state}) => {
    return new Promise((resolve, reject) => {
      ...
      auth.getLoginInfo(state.token).then(response => {
        ...
        if (state.authUser.userType === 2) {
          commit('SET_PROJECT_ID', state.authUser.uscCode)
        }
        // 保存权限信息到state中
