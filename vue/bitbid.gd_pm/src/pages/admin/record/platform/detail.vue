            ...
            <div class="dialog_cancel_infoson_father">
              <table class="detailtable">
                <tr>
                  <td>合作单位：</td>
                  <td colspan="2">
                    <span :title="projectForm.bulletinCode">{{projectForm.enterpriseName}}</span>
                  </td>
                  <td>订单号：</td>
                  <td colspan="2">
                    <span :title="projectForm.tenderOrganizeForm">{{projectForm.ordercode}}</span>
                  </td>
                </tr>
                <tr>
                  <td>申请渠道：</td>
...
export default {
  ...
  methods: {
    ...
    // 返回
    returnBtn () {
      this.$router.push({path: `/admin/record/platform/index`})
    }
  },
  mounted () {
    this.list()
