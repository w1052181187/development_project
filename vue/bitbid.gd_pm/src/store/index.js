...
import {getToken, setToken, removeToken, setIsKeepUser} from '@/utils/auth'
...
const getters = {
  ...
  bidInf: state => state.bidInf,
  newRooId: state => state.newRooId
}
...
const mutations = {
  ...
  SET_MESSAGES: (state, message) => {
    state.bidInf = message
  },
  SET_ROOMID: (state, message) => {
    state.newRooId = message
  }
}
...
const actions = {
    ...
    return new Promise((resolve, reject) => {
      auth.login(userInfo).then(response => {
        ...
        if (response.data.resCode === '0000') {
          ...
          setIsKeepUser(response.data.isKeepUser)
  ...
  LoginBitbid: ({commit}, userInfo) => {
    return new Promise((resolve, reject) => {
      auth.loginBitbid(userInfo).then(response => {
        // 登录成功
        if (response.data.resCode === '0000') {
          // state保存token
          // commit('SET_TOKEN', response.data.token)
