<template>
  <div class="center_box" id="operating-update">
    <el-form :model="projectForm" ref="projectForm" label-width="120px" class="demo-ruleForm">
      <div class="coninfor_box">
        ...
        <!--结算单-->
        <!--备注-->
        <div class="coninfor_con" v-if="Number(this.$route.query.status) === 2">
          <el-row>
            <el-col :span="20">
...
export default {
  ...
  data () {
    return {
      ...
      detailData: [], // 明细数据
      indexMethod: 1, // 序号
      submitStartDate: '', // 结算开始日期
      submitEndDate: '', // 结算结束日期
      subStatus: false, // 是否提交
      isAgency: false, // 是否修改代理
      isTime: false, // 是否修改结算日期
      zbOrderIdArray: [], // 明细里面的订单id集合
      agencyId: '',
      isGenerate: false // 是否再次生成
    }
  },
  methods: {
    // 结算合计方法
    ...
    particularsBtn () {
      ...
      if (this.isGenerate) {
        ...
        settlement.detailList(this.$route.query.id).then((res) => {
          this.detailData = res.data.zbOrderFlowPageInfo.list
          this.detailData.map((ite) => {
            ite.price = Number(ite.price).toFixed(2)
          })
    ...
    anencySelect (agencyId) {
      ...
      if (this.projectForm.agencyenterpriseid !== this.agencyId) { // 判断修改后的代理机构和之前的是否一样， 如果不一样进行提示，且让必须生成
        this.$message({
          type: 'warning',
          message: '修改合作代理后必须生成结算单，否则不能进行保存活提交'
        })
        this.isAgency = true
      } else {
        this.isAgency = false
    ...
    updateTime (val) {
      if (val) {
        if (val[0] !== this.submitStartDate || val[1] !== this.submitEndDate) {
          this.$message({
            type: 'warning',
            message: '修改结算日期后必须生成结算单，否则不能进行保存活提交'
          })
          this.isTime = true
        } else {
          this.isTime = false
    ...
    // 当前信息
    detailInfo () {
      settlement.queryDeail(this.$route.query.id).then((res) => {
        this.agencyId = res.data.settlementSummary.agencyenterpriseid // 代理机构id
        this.projectForm = res.data.settlementSummary
        this.settlSumPriceList = res.data.settlementSummary.settlSumPriceList // 结算单
        this.zbOrderIdArray = res.data.zbOrderIds // 明细里面的订单id集合
        this.unitPriceCapital = res.data.settlementSummary.priceUp // 合计代写
        ...
        // 结算日期
        this.submitStartDate = res.data.settlementSummary.submitStartDate
        this.submitEndDate = res.data.settlementSummary.submitEndDate
        let timeArray = [res.data.settlementSummary.submitStartDate, res.data.settlementSummary.submitEndDate]
        this.$set(this.projectForm, 'hangDataRange', timeArray)
        // 合作代理
        this.projectForm.agencyId = this.projectForm.agencyenterpriseid
    ...
    generateBtn () {
      if (!this.projectForm.agencyId) {
        ...
        let url = {
          cooperator: this.projectForm.agencyId,
    ...
    submitContract (form, status) {
      ...
      submitForm.settlSumPriceList.map((item) => {
        delete item.id
        delete item.settlementsummaryid
        return item
      })
      if (this.isAgency) {
        this.$message({
          type: 'warning',
          message: '合作代理已发生变化，必须先生成结算单后才能进行保存或提交'
        })
        return false
      }
      if (this.isTime) {
        this.$message({
          type: 'warning',
          message: '结算日期已发生变化，必须先生成结算单后才能进行保存或提交'
        })
        return false
      }
      submitForm.id = this.$route.query.id // 本数据id
      submitForm.remark = this.projectForm.remark // 备注
      ...
      settlement.queryUpdate(submitForm).then((res) => {
