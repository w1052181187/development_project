  ...
  methods: {
    ...
    submitForm (form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          shroffAccountNumber.update(this.projectForm).then((res) => {
    ...
    detail () {
      shroffAccountNumber.detail(this.$route.query.objectId).then((res) => {
        this.projectForm = res.data.shroffAccountNumber
        this.projectForm.agencyId = res.data.shroffAccountNumber.agency.id
      })
    }
  },
  mounted () {
