...
public class ImagePickHelper {
	...
	public void showPickDialog(final PermissionManager permissionManager){
		String[] items = new String[] {ImagePickHelper.this.activity.getResources().getString(R.string.myProfile_edit_activity_takephoto),
				ImagePickHelper.this.activity.getResources().getString(R.string.myProfile_edit_activity_localphoto)};
		DialogInterface.OnClickListener click = new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				switch(which){
					//拍照
					case 0:
						handleCameraClick(ImagePickHelper.this.activity, fileName, permissionManager);
						break;
					//从本地选择图片
					case 1:
						handleAbulmClick(ImagePickHelper.this.activity, permissionManager);
						break;
					default :
						break;
		...
		new AlertDialog.Builder(ImagePickHelper.this.activity).setItems(items,
				click).show();
	}
	...
	public void handleAbulmClick(final Activity activity, final PermissionManager permissionManager) {
		...
		permissionManager.init(activity, android.Manifest.permission.WRITE_EXTERNAL_STORAGE, REQUEST_CODE_PERMISSION_ABULM, callback);
