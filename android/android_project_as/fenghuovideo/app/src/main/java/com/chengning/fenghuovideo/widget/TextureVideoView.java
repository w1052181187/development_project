...
//    private float mVolumn = -1;
    ...
    private OnCacheDisplayListener mCacheDisplayListener;
    private long mCacheDisplayTime;
    private long mCacheDisplayPreTime;
    private int mCacheDisplayCount;
    private boolean mCacheDisplayHasCache;
...
//      }
    ...
    public void setOnCacheDisplayListener (OnCacheDisplayListener listener){
        this.mCacheDisplayListener = listener;
    }

    public void clearCacheDisplay(){
        mCacheDisplayTime = 0;
        mCacheDisplayPreTime = 0;
        mCacheDisplayCount = 0;
        mCacheDisplayHasCache = false;
        setOnCacheDisplayListener(null);
    }

    public void resumeCacheDisplay(OnCacheDisplayListener listener){
        mCacheDisplayTime = 0;
        mCacheDisplayPreTime = 0;
        mCacheDisplayCount = 0;
        mCacheDisplayHasCache = false;
        setOnCacheDisplayListener(listener);
    }
...
//                mMediaPlayer.setVolume(mVolumn, mVolumn);
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        if(Consts.DEBUG) Log.d(TAG, "onSurfaceTextureAvailable");
    ...
    public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width, int height) {
        if(Consts.DEBUG) Log.d(TAG, "onSurfaceTextureSizeChanged");

    }

    @Override
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
        if(Consts.DEBUG) Log.d(TAG, "onSurfaceTextureDestroyed");
        //画布失效
        mSurfaceHolder = null;
        release();
        return true;
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        if(Consts.DEBUG) Log.d(TAG, "onSurfaceTextureUpdated");

        mCacheDisplayCount++;
        long time = System.currentTimeMillis();
        if(mCacheDisplayCount == 1){
            if(time - mCacheDisplayTime > 200){
                if(mCacheDisplayListener != null){
                    // TODO(debug)
//                    可能是2次，准备中的状态
                    mCacheDisplayListener.onCache();
                }
                mCacheDisplayHasCache = true;
            }else{
                if(mCacheDisplayListener != null){
                    mCacheDisplayListener.onRefresh();
                }
            }
        }else if(mCacheDisplayCount == 2){
            if(mCacheDisplayHasCache){
                mCacheDisplayHasCache = false;
                if(mCacheDisplayListener != null){
                    mCacheDisplayListener.onRefresh();
                }
        ...
        mCacheDisplayPreTime = mCacheDisplayTime;
        mCacheDisplayTime = time;
    }
    ...
    public static interface OnCacheDisplayListener{
        public void onCache();
