...
import com.chengning.fenghuovideo.activity.ArticleReplyDetailActivity;
...
import com.chengning.fenghuovideo.widget.DynamicTextView;
...
//		convertView.setVisibility(!Common.isTrue(bean.getLocal_del_flag()) ? View.VISIBLE : View.GONE);
		...
		Utils.handleDefaultAvatar(bean.getFace(), image);
		...
		image.setTag(bean);
		image.setOnClickListener(new ArticleCommentListeners.NameOnClickListener());
		...
		comment.setTag(bean);
		comment.setOnClickListener(isCmtDetail ? new ArticleCommentListeners.CommentOnClickListener((BaseFragmentActivity)getContext()) : new View.OnClickListener() {
			@Override
...
//		like.setEnabled(true);
		...
		like.setOnClickListener(new ArticleCommentListeners.LikeOnClickListener(getContext(), new ArticleManagerUtils(),
				new ArticleManagerUtils.DataStateListener() {
		...
		delete.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				handleDelete((BaseFragmentActivity) getContext(), v, position);
			}
		});
...
//					View v = view.findViewById(R.id.item_article_comment_top_user_image);
					...
					dialog.showAllowingStateLoss((BaseFragmentActivity)getContext(), ((FragmentActivity)getContext()).getSupportFragmentManager(), CommentInputDialog.class.getSimpleName());
				} else {
					ArticleReplyDetailActivity.launch(getContext(), bean);
				}
			}
		});
		mView.setOnLongClickListener(new ArticleCommentListeners.ArticleCommentOnLongClickListener((BaseFragmentActivity)getContext(),
				null, content.getText().toString()));
	}

	private void handleDelete(final BaseFragmentActivity activity, final  View v, final int position) {
