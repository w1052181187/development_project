...
import com.chengning.fenghuo.db.provider.dbContent.table_userinfo_other;
...
public class UserinfoOtherServer extends BaseListDA<UserInfoBean> {
	...
	public ContentValues buildInsertValues(UserInfoBean bean) {
		...
		values.put("my_qun", SerializeUtil.serialize(bean.getMy_qun()));
	...
	public UserInfoBean buildQueryValues(Cursor cursor) {
		...
		bean.setMy_qun((ArrayList<CircleBean>) SerializeUtil.deSerialize(cursor.getString(cursor.getColumnIndex("my_qun"))));
