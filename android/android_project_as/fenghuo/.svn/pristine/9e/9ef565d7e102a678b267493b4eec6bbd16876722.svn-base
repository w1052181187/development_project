...
import com.chengning.fenghuo.event.NicknameModifyEvent;
...
import com.chengning.fenghuo.widget.NicknameModifyDialog;
...
public class MyprofileActivity extends BaseFragmentActivity {
	...
	public void initViews() {
		...
		mLabelBtn = (RelativeLayout) this.findViewById(R.id.infoedit_label_rl);
		mLabelTv = (TextView) this.findViewById(R.id.infoedit_label_tv);
	...
	private void handleInfoIsEditable(boolean isEditable) {
		...
		mIntroductionBoult.setVisibility(isEditable ? View.VISIBLE
				: View.INVISIBLE);
		if (!isEditable) {
			RelativeLayout.LayoutParams params = (android.widget.RelativeLayout.LayoutParams) mIntroductionBoult
					.getLayoutParams();
			params.width = 0;
			params.height = 0;
	...
	public void setDataForViews() {
		if (null != mUserInfoBean) {
			...
			ArrayList<CircleBean> mQun = mUserInfoBean.getMy_qun();
			if (!Common.isListEmpty(mQun)) {
				int size = mQun.size();
				for (int i = 0; i < size; i++) {

					label.append(mUserInfoBean.getMy_qun().get(i).getName());
					if (i != size -1) {
						label.append(",");
					}
				}
	...
	public void installListeners() {
		...
		mIntroBtn.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				EditMyDescriptionActivity.create(getActivity());
			}
		});
