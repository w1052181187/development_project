...
import com.chengning.fenghuo.wxapi.WXPayEntryActivity;
...
public class ArticleRewardDialog extends BaseDialogFragment{
	...
	public void initData() {
		...
		api = WXAPIFactory.createWXAPI(getActivity(), Consts.WX_APP_ID, false);
    	if (api.isWXAppInstalled()) {//判断是否安装微信客户端  
    		api.registerApp(Consts.WX_APP_ID);
    	} else {
    		UIHelper.showToast(getActivity(), getResources().getString(R.string.no_wx_no_pay));
...
//		params.put("money", "0.01");
		...
		HttpUtil.getClient().post(getActivity(), JUrl.SITE + JUrl.URL_DO_CREATEORDER, params, new MyJsonHttpResponseHandler() {
			@Override
