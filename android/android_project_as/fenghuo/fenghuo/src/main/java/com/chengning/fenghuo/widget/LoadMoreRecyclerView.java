		...
		super.addOnScrollListener(new OnScrollListener() {
			...
			public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
				...
				if (null != mListener && mIsFooterEnable && !mIsLoadingMore
					...
					if (lastVisiblePosition + 1 == mAutoLoadAdapter
							.getItemCount()) {
						setLoadingMore(true);
...
//				if (null != mListener && mIsHeaderEnable && !mIsRefresh
//						&& dy < 0) {
//					int firstVisiblePosition = getFirstVisiblePosition();
//					if (firstVisiblePosition ==0) {
//						setRefresh(true);
////						mLoadMorePosition = lastVisiblePosition;
//						mAutoLoadAdapter.addHeaderView(R.layout.pull_to_refresh_header_vertical);
//						mListener.onRefresh();
//					}
//				}
			}
		...
		public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
			if (viewType == TYPE_HEADER) {
				return new HeaderViewHolder(LayoutInflater.from(
						parent.getContext()).inflate(R.layout.list_foot_loading, parent,
						false));
			}
...
//		}
	...
	private int getFirstVisiblePosition() {
		int position;
		if (getLayoutManager() instanceof LinearLayoutManager) {
			position = ((LinearLayoutManager) getLayoutManager())
					.findFirstVisibleItemPosition();
		} else if (getLayoutManager() instanceof GridLayoutManager) {
			position = ((GridLayoutManager) getLayoutManager())
					.findFirstVisibleItemPosition();
		} else if (getLayoutManager() instanceof StaggeredGridLayoutManager) {
			StaggeredGridLayoutManager layoutManager = (StaggeredGridLayoutManager) getLayoutManager();
			int[] lastPositions = layoutManager
					.findFirstVisibleItemPositions(new int[layoutManager
							.getSpanCount()]);
	...
	private int getLastVisiblePosition() {
		...
		if (getLayoutManager() instanceof LinearLayoutManager) {
			...
			StaggeredGridLayoutManager layoutManager = (StaggeredGridLayoutManager) getLayoutManager();
			int[] lastPositions = layoutManager
					.findLastVisibleItemPositions(new int[layoutManager
							.getSpanCount()]);
