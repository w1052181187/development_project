...
import com.chengning.fenghuo.base.BaseRecyclerViewHolder;
...
public class CircleChannelItemAdapterRecyler extends BaseRecylerViewAdapter{
	...
	public CircleChannelItemAdapterRecyler(Activity activity, List list,
			boolean isShowDel, boolean isShowDelFav, RecyclerView recyclerView) {
...
//				.displayer(
//						new MyRoundedBitmapDisplayer(App
//								.getInst()
//								.getResources()
//								.getDimensionPixelSize(
//										R.dimen.common_round_corner)))
				.build();

		DisplayUtil.getInst().init(activity);
		isSelected = new HashMap<Integer, Boolean>();

		int gapWidth = activity.getResources().getDimensionPixelSize(
		...
		mImageThreeSmallHeight = activity.getResources().getDimensionPixelSize(
				R.dimen.channel_item_image_three_height);

		mImageTwoSmallWidth = (DisplayUtil.getInst().getScreenWidth() - leftMargin - gapWidth * 3) / 2;
		mImageTwoSmallHeight = activity.getResources().getDimensionPixelSize(
				R.dimen.channel_item_image_two_height);
	...
	public CircleChannelItemAdapterRecyler(Activity activity, List list,
			boolean isShowDel, TextView tv, RecyclerView recyclerView) {
		super(activity, list, recyclerView);
		this.mContext = activity;
		this.list = list;
		this.isShowDel = false;
...
//										R.dimen.common_round_corner)))
	...
	public int buildLayoutId() {
		return R.layout.item_circlechannel_item;
	}
	
	@Override
	public void handleLayout(final BaseRecyclerViewHolder holder, Object item, final int position) {
		
		ImageView head = (ImageView) holder.getView(R.id.item_circlechannel_head);
		TextView name = (TextView) holder.getView(R.id.item_circlechannel_name);
		TextView time = (TextView) holder.getView(R.id.item_circlechannel_time);
//		from = (TextView) view.findViewById(R.id.item_circlechannel_from);
		final TextView title = (TextView) holder.getView(R.id.item_circlechannel_title);
		final DynamicTextView contentDetail = (DynamicTextView) holder.getView(R.id.item_circlechannel_content_detail);
		final DynamicTextView content = (DynamicTextView) holder.getView(R.id.item_circlechannel_content);
		View view1 = holder.getView(R.id.item_circlechannel_image_ll1);
		final ImageView image1 = (ImageView) holder.getView(R.id.item_circlechannel_image1);
		ImageView image2 = (ImageView) holder.getView(R.id.item_circlechannel_image2);
		ImageView image3 = (ImageView) holder.getView(R.id.item_circlechannel_image3);
		View view2 = holder.getView(R.id.item_circlechannel_image_ll2);
		ImageView image4 = (ImageView) holder.getView(R.id.item_circlechannel_image4);
		ImageView image5 = (ImageView) holder.getView(R.id.item_circlechannel_image5);
		ImageView image6 = (ImageView) holder.getView(R.id.item_circlechannel_image6);
		View view3 = holder.getView(R.id.item_circlechannel_image_ll3);
		ImageView image7 = (ImageView) holder.getView(R.id.item_circlechannel_image7);
		ImageView image8 = (ImageView) holder.getView(R.id.item_circlechannel_image8);
		ImageView image9 = (ImageView) holder.getView(R.id.item_circlechannel_image9);
		TextView comment_tv = (TextView) holder.getView(R.id.item_circlechannel_comment_tv);
		final TextView zan_tv = (TextView) holder.getView(R.id.item_circlechannel_zan_tv);
		ImageView del = (ImageView) holder.getView(R.id.item_circlechannel_del);
		CheckBox del_favorite = (CheckBox) holder.getView(R.id.circle_list_item_check);
		
		
		final View zanOrCmtLayout = holder.getView(R.id.item_circlechannel_comment_zan_layout);
		final View zanLayout = holder.getView(R.id.item_circlechannel_zan_layout);
		final LinearLayout cmtLayout = (LinearLayout) holder.getView(R.id.item_circlechannel_cmt_layout);
		final View cmtLayoutLine = holder.getView(R.id.item_circlechannel_cmt_layout_divider);
		final View cmtMoreLine = holder.getView(R.id.item_circlechannel_cmt_more_divider);
		final TextView cmtMore = holder.getView(R.id.item_circlechannel_cmt_more);
		
		final FlowLayoutSingleLine zanFlow = (FlowLayoutSingleLine) holder.getView(R.id.item_circlechannel_zan_flow);
		final ImageView zanMore = (ImageView) holder.getView(R.id.item_circlechannel_zan_more);
		final FlowLayout zanMoreFlow = (FlowLayout) holder.getView(R.id.item_circlechannel_zan_more_flow);
		
		final TextView detail = (TextView) holder.getView(R.id.item_circlechannel_detail);
		final TextView duanpingState = (TextView) holder.getView(R.id.item_circlechannel_duanping);
		TextView shareContent = (TextView) holder.getView(R.id.item_circlechannel_share);
		...
		final DynamicItemBean bean = (DynamicItemBean) item;
		...
		if (Common.isTrue(SettingManager.getInst().getNightModel())) {
			head.setColorFilter(
					mContext.getResources().getColor(R.color.night_img_color),
					PorterDuff.Mode.MULTIPLY);
		}
		Common.handleUserNameDisplay(mContext, bean, name);
		name.setVisibility(!TextUtils.isEmpty(bean.getNickname())
				? View.VISIBLE
				: View.GONE);
		try {
...
//		mHolder.from.setText("来自" + bean.getFrom());
		if (TextUtils.isEmpty(bean.getTitle())) {
			...
			title.setOnLongClickListener(new DynamicCommentOnLongClickListener(mContext,new DeleteListener() {
				...
				public void deleteSuccess(String data) {
					...
					if(pos != list.size()){
						getAdapter().notifyItemRangeChanged(pos, list.size() - pos);
					}
					if (mCircleCommentHandleSuccess != null) {
						mCircleCommentHandleSuccess.deleteSuccess(bean, pos);
					}
				}
			}, bean.getTitle()));
		}
...
//									notifyDataSetChanged();
	...
	protected void showCmtPopupwindow(final Activity mContext, final View view, final int position, final FlowLayoutSingleLine zanFlow,
		...
		final DynamicItemBean bean = (DynamicItemBean)list.get(position);
	...
	private void handleDetailClick(DynamicItemBean bean) {
		...
		LocalStateDA.getInst(mContext).setReadStateRead(LocalStateDA.PREFIX_CHANNEL_ITEM, bean.getTid());
		DynamicDetailActivity.launch(mContext, bean);
	}
	...
	public static void setIsSelected(HashMap<Integer, Boolean> isSelected) {
		CircleChannelItemAdapterRecyler.isSelected = isSelected;
	}
	...
	public static void setAllSelected(boolean isChecked) {
		if (isSelected != null) {
			...
			while (iter.hasNext()) {
				...
				isSelected.put((Integer) key, isChecked);
			}
		}
	}
