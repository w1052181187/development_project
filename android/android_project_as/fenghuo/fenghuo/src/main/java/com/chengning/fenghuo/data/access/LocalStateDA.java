...
import com.chengning.fenghuo.data.bean.LocalStateBean;
import com.chengning.fenghuo.db.provider.dbContent.table_local_state;
...
public class LocalStateDA extends BaseListDA<LocalStateBean> {
	...
	public static final int COLLECT_DISCUSS = 0;
...
//	}
	...
	public boolean isArticleSaved(String prefix, String id){
		...
		if(!TextUtils.isEmpty(aStr)){
			try {
				...
				if(!TextUtils.isEmpty(bean.getTid())){
					return true;
				}
			} catch (Exception e) {
				e.printStackTrace();
	 ...
	 * @param data
	 */
	public void saveFavArticle(String prefix, String id, String data,ArticleType article){		
		LocalStateBean bean = queryLocalStateNotNull(prefix, id);
...
//		List<LocalStateBean> list = queryTarget("favorite_state = ? and collect_type = ?", new String[] { String.valueOf(STATE_TRUE),String.valueOf(COLLECT_ARTICLE)}, "collect_time desc");
	 ...
	 * @return
	 */
	public ArrayList<DynamicItemBean> getFavDiscuss(int offset) {
		String limit = table_local_state.Columns.COLLECT_TIME + " desc limit 20 offset " + offset;
		List<LocalStateBean> list = queryTarget("favorite_state = ? and collect_type = ?", new String[] { String.valueOf(STATE_TRUE),String.valueOf(COLLECT_DISCUSS)}, limit);
//		List<LocalStateBean> list = queryTarget("favorite_state = ? and collect_type = ?", new String[] { String.valueOf(STATE_TRUE),String.valueOf(COLLECT_DISCUSS)}, "collect_time desc");
		ArrayList<DynamicItemBean> beans = new ArrayList<DynamicItemBean>();
		String data = null;
		for (LocalStateBean bean:list) {
			data = bean.getData_item();
			if(!TextUtils.isEmpty(data)){
				try {
					DynamicItemBean itemBean = SerializeUtil.deSerialize(data, DynamicItemBean.class);
					beans.add(itemBean);
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			if(!TextUtils.isEmpty(data)){
				try {
					...
					if(isUpdate){
						if( itemBean.getShow_type() == Consts.SHOW_TYPE_ONE_SMALL_VIDEO || itemBean.getShow_type() == Consts.SHOW_TYPE_ONE_BIG_VIDEO ){
							itemBean.setContent_type(Consts.CONTENT_TYPE_VIDEO);
							saveFavArticle(LocalStateDA.PREFIX_CHANNEL_ITEM, itemBean.getTid(), SerializeUtil.serialize(itemBean), ArticleType.ARTICLE);
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
