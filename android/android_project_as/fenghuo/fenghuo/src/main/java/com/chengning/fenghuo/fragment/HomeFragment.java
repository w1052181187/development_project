...
import com.chengning.fenghuo.activity.DirectorateActivity;
import com.chengning.fenghuo.activity.MainActivity;
import com.chengning.fenghuo.activity.MyprofileActivity;
import com.chengning.fenghuo.activity.SearchNewsActivity;
import com.chengning.fenghuo.base.AbstractChannelItemListFragment;
import com.chengning.fenghuo.base.AbstractChannelItemListFragment.OnRecommendHttpListener;
...
import com.chengning.fenghuo.widget.HomeEditChannelList;
import com.chengning.fenghuo.widget.HomeEditChannelList.OnChannelListChangeListener;
...
public class HomeFragment extends BaseFragment implements IForceListenRefreshExtend {
	...
	private ImageButton zhihuibuBtn;
	...
	public void initViews() {
		...
		mUserImgLayout = mView.findViewById(R.id.top_user_btn_layout);
		mUserImg = (ImageView) mView.findViewById(R.id.top_user_btn);
		mLoginBtn = (ImageButton) mView.findViewById(R.id.top_login_btn);
		zhihuibuBtn = (ImageButton) mView.findViewById(R.id.top_zhihuibu_btn);
...
//		hideIndicator();
	...
	private void initUserAvatar() {
		if (App.getInst().isLogin()) {
			changeUserState(0);
			Utils.setCircleImage(App.getInst().getUserInfoBean().getFace(),
					mUserImg);
			if (Common.isTrue(SettingManager.getInst().getNightModel())) {
	...
	public void installListeners() {
		...
		zhihuibuBtn.setOnClickListener(new OnClickListener() {
		...
		if (!Common.isListEmpty(channelList)) {
			...
			for (int i = 0; i < channelCount; i++) {
				...
				if (i == 0) {
					...
					fragmentsList.add(f1);
				} else if (channelList.get(i).getId().equals("special")) {
					fragmentStrsList.add(b.getName());
					ColumnFragmentNew columnFragment = (ColumnFragmentNew) ColumnFragmentNew.newInstance(b);
					fragmentsList.add(columnFragment);
...
//		if (!App.getInst().isLogin() || null == channelList || !isUserSetted) {
	...
	public void onResume() {
		super.onResume();
		if (App.getInst().isLogin()) {
			if (App.getInst().isUserAvatarChange()) {
				App.getInst().setUserAvatarChange(false);
				Utils.setCircleImage(App.getInst().getUserInfoBean().getFace(),
						mUserImg);
				if (Common.isTrue(SettingManager.getInst().getNightModel())) {
					mUserImg.setColorFilter(getResources().getColor(R.color.night_img_color), PorterDuff.Mode.MULTIPLY);
	...
	private void changeUserState(int i) {
		...
		case 0:
			...
			mUserImgLayout.setVisibility(View.VISIBLE);
			mLoginBtn.setVisibility(View.GONE);
			break;
		case 1:
			mUserImg.setVisibility(View.GONE);
			mUserImgLayout.setVisibility(View.GONE);
