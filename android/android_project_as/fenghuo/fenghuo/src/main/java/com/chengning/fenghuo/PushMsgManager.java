...
import com.chengning.fenghuo.data.bean.XiaomiPushBean;
...
public class PushMsgManager {
	...
	public static boolean isUserLogin(Serializable lc){
		String uid = null;
		if (lc instanceof PushLCBean) {
			uid = ((PushLCBean)lc).getExtension().getUid();
		} else if (lc instanceof XiaomiPushBean) {
			uid = ((XiaomiPushBean)lc).getUid();
		}
		return LoginManager.getInst().isLogin() 
				&&  LoginManager.getInst().getLoginUserBean().getUserinfo().getUid().equals(uid);
	}
	...
	public Object[] filterLCPush(Context context, int type, Serializable lc){
		...
		case PushReceiver.TYPE_COMMENT:
			...
			resultIntent = new Intent(context, CommentAboutMeListActivity.class);
			break;
