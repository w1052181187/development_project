...
import com.chengning.fenghuo.db.provider.dbContent.table_circle_item_home;

@Deprecated
public class CircleItemHomeDA extends BaseListDA<CircleBean> {

	private static CircleItemHomeDA mInst;

	public static CircleItemHomeDA getInst(Context con) {
	    if (mInst == null) {
            synchronized (CircleItemHomeDA.class) {
                if (mInst == null) {
                    mInst = new CircleItemHomeDA(con);
                }
            }
        }
		return mInst;
	...
	private CircleItemHomeDA(Context con) {
		super(con, table_circle_item_home.CONTENT_URI);
	}
