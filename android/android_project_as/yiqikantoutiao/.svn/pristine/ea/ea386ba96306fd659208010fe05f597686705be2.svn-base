...
//		mFav.setImageResource(R.drawable.article_bottom_fav_state);
	...
	public void setChannelBean(final BaseArticlesBean bean, final Consts.ArticleCommentType type){
		if (bean instanceof ChannelItemBean) {
			this.mChannelBean = (ChannelItemBean) bean;
		}
...
//				if (null != mArticleBean && LoginManager.getInst().checkLoginWithNotice(mContext)) {
	...
	public void setData(ArticlesBean bean){
		...
		String img = mArticleBean.getImage_list();
		if (mChannelBean != null) {
			mArticleBean.setImage_arr(mChannelBean.getImage_arr());
			if (TextUtils.isEmpty(img)) {
				mArticleBean.setImage_list(mChannelBean.getImage_list());
			}
		}
