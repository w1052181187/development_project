...
//			mMoneyTv.setText("+" + bean.getExtcredits2());
	...
	private void getTaskDetail(final Activity context, String action) {
		...
		HttpUtil.get(JUrl.SITE + JUrl.URL_DIRECTORATE_DETAIL_INFO, params, new MyStatusResponseHandler(){
...
//				UIHelper.removePD();
				Gson gson = new Gson();
                DirectorateTaskBean bean = gson.fromJson(data, DirectorateTaskBean.class);
				//list = SetDbName(list);
                getHandler().obtainMessage(DATA_SUCCESS, bean).sendToTarget();
			}
