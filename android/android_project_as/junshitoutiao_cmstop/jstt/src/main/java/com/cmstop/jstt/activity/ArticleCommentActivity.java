			...
			Runnable skipRunnable = new Runnable() {
				...
				public void run() {
					if(getActivityInfo().getActivityState() != ActivityState.Killed){
						...
						dialog.setAid(mId);
						dialog.showAllowingStateLoss(ArticleCommentActivity.this, ((BaseFragmentActivity)getActivity()).getSupportFragmentManager(), 
...
//				if(CommentCheckUtil.checkLoginNotNeededOfComment(getActivity())){
					CommentInputDialog dialog = new CommentInputDialog();
					dialog.setAid(mId);
					dialog.showAllowingStateLoss(ArticleCommentActivity.this, getSupportFragmentManager(), CommentInputDialog.class.getSimpleName());
//				}
			}
		});
