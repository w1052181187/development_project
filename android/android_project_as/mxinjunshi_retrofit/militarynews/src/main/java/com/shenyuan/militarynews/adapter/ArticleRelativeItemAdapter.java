					...
					if (!isNoPic) {
						if(showTypeAd == Const.NEWS_TYPE_COMMON_AD_THREE_SMALL){
							...
							if(!Common.isListEmpty(adBean.getMultiPicUrls()) && adBean.getMultiPicUrls().size() > 2){
								...
								GlideHelper.getInst().loadImageWithPlace(getContext(),adBean.getMultiPicUrls().get(0), image, getImageOptions(finalBean));
								GlideHelper.getInst().loadImageWithPlace(getContext(),adBean.getMultiPicUrls().get(1), image, getImageOptions(finalBean));
								GlideHelper.getInst().loadImageWithPlace(getContext(),adBean.getMultiPicUrls().get(2), image, getImageOptions(finalBean));
								...
								GlideHelper.getInst().clear(getContext(), image1);
							...
							if (!TextUtils.isEmpty(obj.getImageUrl())) {
								image.setVisibility(View.VISIBLE);
								GlideHelper.getInst().loadImageWithPlace(getContext(), obj.getImageUrl(), image, mImageType);
							} else if (!TextUtils.isEmpty(obj.getIconUrl())) {
								image.setVisibility(View.VISIBLE);
								GlideHelper.getInst().loadImageWithPlace(getContext(),obj.getIconUrl(), image, mImageType);
							} else {
								GlideHelper.getInst().clear(getContext(), image);
								image.setVisibility(View.GONE);
							}
			...
			if (!isNoPic) {
				switch (itemType) {
					...
					case ThreeSmall:
						...
						updateSize(image1, mImageThreeSmallWidth, mImageThreeSmallHeight);
						updateSize(image2, mImageThreeSmallWidth, mImageThreeSmallHeight);
						GlideHelper.getInst().loadImageWithPlace(getContext(),imgs.get(0), image, getImageOptions(bean));
						GlideHelper.getInst().loadImageWithPlace(getContext(),imgs.get(1), image1, getImageOptions(bean));
						GlideHelper.getInst().loadImageWithPlace(getContext(),imgs.get(2), image2, getImageOptions(bean));
						if (Common.isTrue(SettingManager.getInst().getNightModel())) {
							image.setColorFilter(getContext().getResources().getColor(R.color.night_img_color), PorterDuff.Mode.MULTIPLY);
							image1.setColorFilter(getContext().getResources().getColor(R.color.night_img_color), PorterDuff.Mode.MULTIPLY);
							image2.setColorFilter(getContext().getResources().getColor(R.color.night_img_color), PorterDuff.Mode.MULTIPLY);
