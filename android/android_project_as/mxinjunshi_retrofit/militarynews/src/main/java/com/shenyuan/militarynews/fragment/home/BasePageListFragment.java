...
public abstract class BasePageListFragment<T> extends BaseFragment implements IForceListenRefreshExtend, IFragmentLazyLoadListener {
...
//		if (isSelected && !mHasLoadedOnce) {
		if (isSelected && isSupportVisible()) {
			getDataOnInit();
		}
	}
	...
	private void handleHttpFailure(final Throwable throwable){
//		getHandler().obtainMessage(DATA_FAILURE).sendToTarget();
...
//					}
			...
			public void onFailure(Call call, IOException e) {
				handleHttpFailure(e == null ? null : e.getCause());
				onHttpFailure();
				mUrlWorking.remove(url);
...
//		mContext = this.getContext();
	...
	public void initDatas() {
		...
		mListServer = ChannelItemListServer.getInst(getContext());
		...
		mAdapter = buildAdapter(getContext(), mDataList);
...
//				getListByHttp();
		...
		mPullListView.setOnScrollListener(new GlideHelper.PauseOnScrollListener(getContext(), mScrollListener));
