	...
	private class OfflineDownloadTask extends AsyncTask<String, Integer, String> {
		...
		private void privateDownloadImage(String url){
			if(!Common.isTrue(SettingManager.getInst().getNoPicModel())){
				Glide.with(App.getInst()).downloadOnly().load(url).submit();
			}
		}
		...
		private void downloadArticle(final MChannelItemBean bean){
            ...
            call.enqueue(new MyOkhttpCallback() {
				@Override
				public void onDataSuccess(int status, String mod, String message, String data, Response response) {
					Gson gson = new Gson();
				...
				@Override
				public void onDataFailure(int status, String mod, String message, String data, Response response) {
...
//            http.setTimeout(5000);
		...
		private boolean downloadChannel(final ChannelDownloadBean dBean){
			...
			if (!TextUtils.isEmpty(dBean.getUrl())) {
                Request request = new Request.Builder()
                        .url(JUrl.appendPage(JUrl.SITE + JUrl.API + dBean.getUrl(), JUrl.PAGE_START))
                        .build();;
                OkHttpClient okHttpClient = new OkHttpClient.Builder()
                        .connectTimeout(5, TimeUnit.SECONDS)
                        .readTimeout(5, TimeUnit.SECONDS)
