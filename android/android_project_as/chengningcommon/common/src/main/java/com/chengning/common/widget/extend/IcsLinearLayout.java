...
//    	Log.d("IcsLinearLayout", "1 mode: " + mode + ", width: " + width + ", childSubWidth: " + childSubWidth + ", childMaxWidth: " + childMaxWidth + ", childCount: " + childCount);
    	if(childCount > 1){
    		for(int i = 0; i< childCount; i++){
    			View child = getChildAt(i);
    			int cWidth = child.getMeasuredWidth();
    			childSubWidth += cWidth;
    			childMaxWidth = Math.max(childMaxWidth, cWidth);
//            	Log.d("IcsLinearLayout", "index: " + i + ", cWidth: " + cWidth);
    		}

    		int ava = width / (childCount + 1);
        	if(childSubWidth == parentWidth && childMaxWidth > (ava + 2)){
            	super.onMeasure(MeasureSpec.makeMeasureSpec(widthMeasureSpec, MeasureSpec.UNSPECIFIED), heightMeasureSpec);
            	int newWidth = getMeasuredWidth();
            	if(newWidth < parentWidth && (parentWidth - newWidth) <  (ava / 4)){
                	super.onMeasure(MeasureSpec.makeMeasureSpec(widthMeasureSpec, MeasureSpec.AT_MOST), heightMeasureSpec);
            	}else{
                	super.onMeasure(widthMeasureSpec, heightMeasureSpec);
            	}
        	}
    	}
//    	Log.d("IcsLinearLayout", "2 width: " + width + ", childSubWidth: " + childSubWidth + ", childMaxWidth: " + childMaxWidth + ", childCount: " + childCount);
    }
