...
public class CollectAdapter extends PageActivityAdapter {
    private static final int TYPE_ITEM = 0;  //普通Item View
    private final LayoutInflater inflater;
    private Activity context;
    private List<InformationItemsModel> socailInfos;

    public CollectAdapter(Activity context, List<InformationItemsModel> socailInfos) {
        super(context);
    ...
    protected RecyclerView.ViewHolder onChildCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_ITEM) {
            View view = inflater.inflate(R.layout.adapter_collect, parent, false);
            //这边可以做一些属性设置，甚至事件监听绑定
            //view.setBackgroundColor(Color.RED);
            CollectItemHolder itemViewHolder = new CollectItemHolder(view);
    ...
    protected void onChildBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        if (holder instanceof CollectItemHolder) {
            CollectItemHolder h = (CollectItemHolder) holder;
    ...
    class CollectItemHolder extends RecyclerView.ViewHolder {
